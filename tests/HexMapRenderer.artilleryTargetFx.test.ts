...
registerTest("HEXMAP_ARTILLERY_TARGETS_SKIP_LARGE_IMPACT_FLASH", async ({ Given, When, Then }) => {
  ...
  await Given("an arcing-artillery attack against an artillery defender", async () => {
    ...
    renderer.getUnitClassAt = (hexKey) => (hexKey === "0,0" ? "artillery" : "artillery");
    renderer.getUnitScenarioTypeAt = (hexKey) => (hexKey === "0,0" ? "Howitzer_105" : "SP_Artillery");
  ...
  await Then("the small barrage still plays but the oversized opening flash is skipped", async () => {
    if (flashCalls !== 0) {
      throw new Error(`Expected no large impact flash for artillery defenders, received ${flashCalls} flash call(s).`);
    }
