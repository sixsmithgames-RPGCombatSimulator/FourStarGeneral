...
registerTest("AIR_SHOW_BOMBER_REACHES_STANDOFF_AT_PROGRESS_1_0", async ({ Given, When, Then }) => {
  ...
  await Then("the bomber position should be at the standoff point (2 hexes before target)", async () => {
    ...
    const bomberPhase = inspection.report.phases.find(
      p => p.assignments.some(a => a.role === "bomber")
    );
    if (!bomberPhase) {
    ...
    for (const assignment of bomberAssignments) {
      ...
      if (!Number.isFinite(endSample.cx) || !Number.isFinite(endSample.cy)) {
        throw new Error(
          `Bomber ${assignment.actorId} has invalid end position at progress ${endSample.progress}: ` +
...
registerTest("AIR_SHOW_CORRIDOR_PHASE_DURATIONS_ARE_NOT_FIXED_FRACTION_ARTIFACTS", async ({ Given, When, Then }) => {
  ...
  await Then("bomber-ingress should consume a readable share of pre-target playback instead of a tiny fixed slice", async () => {
    ...
    if (ingressShare < 0.12) {
      throw new Error(
        `bomber-ingress collapsed to ${(ingressShare * 100).toFixed(1)}% of pre-target playback; ` +
        `expected a readable window derived from planned paths, not a tiny fixed corridor fraction.`
      );
    }

    console.log(
      `[CORRIDOR TIMING] bomber-ingress=${bomberIngress.durationMs}ms ` +
...
registerTest("AIR_SHOW_FLAK_TIMING_OPENS_ON_MID_APPROACH_AND_TAPERS_AFTER_BOMB_RELEASE", async ({ Given, When, Then }) => {
  ...
  await Then("flak should activate on the approach, persist through bomb release, and taper before egress", async () => {
    const hasTargetRunFlak = (entry: NonNullable<typeof result>["airshowInspections"][number]): boolean =>
      entry.report.phases.some((phase) => phase.label === "target-run" && (phase.flakBursts?.length ?? 0) > 0);
    const hasPreTargetFlak = (entry: NonNullable<typeof result>["airshowInspections"][number]): boolean =>
      entry.report.phases.some((phase) =>
        (phase.label === "bomber-ingress" || phase.label === "bomber-defense-pass") &&
    ...
    if (
      ...
      throw new Error(
        `Flak starts too late: first phase with flak is ${firstFlakPhase.label} ` +
    ...
    if (lastFlakPhase.label !== "target-run") {
      throw new Error(
        `Flak ends too early: last phase with flak is ${lastFlakPhase.label} ` +
...
registerTest("AIR_SHOW_ESCORTS_PRESENT_IN_EARLY_CONTESTED_PACKAGE_PHASES", async ({ Given, When, Then }) => {
  ...
  await Then("escorts should be present in early phases of the contested package", async () => {
    ...
    const phasesWithEscorts = inspection.report.phases.filter(
      p => p.assignments.some(a => a.role === "escort")
    );
...
registerTest("AIR_SHOW_DOGFIGHT_OCCURS_BETWEEN_PROGRESS_0_20_AND_0_50", async ({ Given, When, Then }) => {
  ...
  await Then("CAP vs Escort dogfight should occur within progress window 0.20-0.50", async () => {
    ...
    const dogfightPhases = inspection.report.phases.filter(
      p => p.label.includes("clash") || p.label.includes("merge") || p.label.includes("scramble")
    );
...
registerTest("AIR_SHOW_FIGHTER_EGRESS_AT_PROGRESS_0_80_PLUS", async ({ Given, When, Then }) => {
  ...
  await Then("CAP and escorts should begin egress at or after progress 0.80", async () => {
    ...
    const fighterAssignments = egressPhase.assignments.filter(
      a => a.role === "interceptor" || a.role === "escort"
    );
...
registerTest("AIR_SHOW_BOMB_RELEASE_AT_TURN_PROGRESS_0_50", async ({ Given, When, Then }) => {
  ...
  await Then("bombs should release at turnProgress = 0.50 (midpoint of arc turn)", async () => {
    ...
    const targetRun = inspection.report.phases.find(
    ...
    for (const assignment of bomberAssignments) {
      const midSample = assignment.sampledPositions.find(
        s => Math.abs(s.progress - 0.5) < 0.1
      );
      if (!midSample) {
      ...
      if (!Number.isFinite(midSample.cx) || !Number.isFinite(midSample.cy)) {
        throw new Error(
          `Bomber ${assignment.actorId} invalid position at turn midpoint: ` +
...
registerTest("AIR_SHOW_SCENARIO_2_STRIKE_ONLY_FOLLOWS_PROGRESS_MODEL", async ({ Given, When, Then }) => {
  ...
  await Then("strike-only package should follow progress-based choreography without fighter phases", async () => {
    const inspection = result?.airshowInspections.find(
      (entry) => entry.missionId === "synthetic-scenario-2-strike-only"
    );
    if (!inspection) {
    ...
    const fighterPhases = phases.filter(p =>
      p.label.includes("fighter") || p.label.includes("clash") || p.label.includes("merge") || p.label.includes("scramble")
    );
...
registerTest("AIR_SHOW_SCENARIO_4_CAP_CLASH_DISTANCE_BASED_TIMING", async ({ Given, When, Then }) => {
  ...
  await Then("CAP clash should use distance-based timing with fighters meeting at center", async () => {
    const inspection = result?.airshowInspections.find(
      (entry) => entry.missionId === "synthetic-scenario-4-cap-clash"
    );
    if (!inspection) {
    ...
    const combatPhases = phases.filter(p =>
