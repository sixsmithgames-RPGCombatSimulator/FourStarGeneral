...
const truckDef: UnitTypeDefinition = {
  class: "vehicle",
...
function buildScenario(tilePalette: TilePalette, tiles: ScenarioData["tiles"]): ScenarioData {
  return {
...
): { engine: GameEngine; config: GameEngineConfig } {
...
registerTest("ENGINEER_FIELDWORKS_REQUIRE_A_FRESH_START_AND_STILL_END_THE_TURN", async ({ Then }) => {
  ...
  for (const [type, facing] of [
    ...
    const built = freshEngine.buildHexModification(
      engineerTemplate.hex,
      type,
      facing ?? undefined
    );
    if (!built) {
...
registerTest("FORTIFICATIONS_AND_TANK_TRAPS_CAN_SHARE_THE_SAME_EDGE", async ({ Then }) => {
  ...
  const engineerB: ScenarioUnit = { ...engineerA, unitId: "eng-b", facing: "SE" as ScenarioUnit["facing"] };
  const engineerC: ScenarioUnit = { ...engineerA, unitId: "eng-c", facing: "E" as ScenarioUnit["facing"] };
...
registerTest("CLEARED_PATHS_STACK_TO_LEVEL_THREE_AND_CUT_MARSH_COST_TOWARD_ROADS", async ({ Then }) => {
  ...
  const tiles: ScenarioData["tiles"] = [
    [{ tile: "PLAINS" }, { tile: "MARSH" }, { tile: "PLAINS" }],
    [{ tile: "PLAINS" }, { tile: "PLAINS" }, { tile: "PLAINS" }]
  ];

  const truck: ScenarioUnit = {
    type: "TestTruck" as unknown as ScenarioUnit["type"],
    unitId: "truck",
    hex: { q: 0, r: 0 },
    strength: 100,
    experience: 0,
