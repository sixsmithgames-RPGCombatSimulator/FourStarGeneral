...
function buildScene(fixture: AirshowHarnessFixture): ResolvedAirShowScene {
...
interface RenderHarness {
  readonly renderer: HexMapRenderer;
  readonly svg: SVGSVGElement;
  readonly canvas: HTMLDivElement;
  readonly restoreFetch: () => void;
}

function renderFixture(fixture: AirshowHarnessFixture): RenderHarness {
...
function disposeHarness(harness: RenderHarness): void {
  harness.restoreFetch();
  harness.svg.remove();
  harness.canvas.remove();
}

function inspect(renderer: HexMapRenderer, scene: ResolvedAirShowScene): AirShowInspectionReport {
...
function timeline(renderer: HexMapRenderer, scene: ResolvedAirShowScene): AirShowTimeline {
  const planned = (renderer as unknown as {
    planResolvedAirCombatTimeline: (candidate: ResolvedAirShowScene) => AirShowTimeline | null;
...
describe("Air show renderer timeline", () => {
  ...
  test.each([
    ...
    try {
      ...
      const playerDistancePx = Math.hypot(
        (report.originPlan?.playerOrigin.cx ?? 0) - (report.originPlan?.playerBoundary.cx ?? 0),
        (report.originPlan?.playerOrigin.cy ?? 0) - (report.originPlan?.playerBoundary.cy ?? 0)
      );
      const botDistancePx = Math.hypot(
        (report.originPlan?.botOrigin.cx ?? 0) - (report.originPlan?.botBoundary.cx ?? 0),
        (report.originPlan?.botOrigin.cy ?? 0) - (report.originPlan?.botBoundary.cy ?? 0)
      );
  ...
  test("CAP clash has no bomber, flak, or target-run work", () => {
    ...
    try {
      ...
      const scene: ResolvedAirShowScene = {
        ...source,
        kind: "capClash",
        bombers: [],
        bomber: null,
  ...
  test("one bomber sprite identity remains visible while impact promises are unresolved", async () => {
    ...
    let clockMs = 0;
    let firstBomberElement: Element | null = null;
    let visibleDuringImpact = false;
    let resolveImpact: () => void = () => {};
    const pendingImpact = new Promise<void>((resolve) => {
      resolveImpact = resolve;
    });
    const nowSpy = jest.spyOn(performance, "now").mockImplementation(() => clockMs);
    const internals = harness.renderer as unknown as {
      scheduleAnimationFrame: (step: FrameRequestCallback) => void;
      playAirShowFlakWave: () => void;
      playAirShowTracerBurst: () => void;
      playExplosion: () => Promise<void>;
      playDustCloud: () => Promise<void>;
      playAirDamageSmokeTrailAt: () => Promise<void>;
    };
