...
const AIRSHOW_CHOREOGRAPHY_TIMEOUT_MS = 140_000;
...
function centroid(actors: ReadonlyArray<Actor>): { cx: number; cy: number } {
  return {
...
function nearestOpposingDistance(
  playerSide: ReadonlyArray<Actor>,
  botSide: ReadonlyArray<Actor>
): number {
...
function midpointSample(samples: ReadonlyArray<Sample>, phaseLabel: string): Sample {
...
function activeFighterSides(sample: Sample): {
  playerSide: ReadonlyArray<Actor>;
  botSide: ReadonlyArray<Actor>;
} {
...
test.describe("AirShow timeline-v2 choreography", () => {
  test("20x20 full engagement preserves origins, speed, continuity, merge, scramble, formation, and egress @temporal-certificate", async ({ page }, testInfo) => {
    ...
    const startResult = await page.evaluate(async () => {
      ...
      if (!harness) throw new Error("Airshow e2e hooks were not installed.");
    ...
    const capture = await page.evaluate(() => {
      const harness = (window as Window & {
        __FSG_AIRSHOW_E2E__?: {
          ...
          getSpawnSnapshot: () => ReadonlyArray<{
            actorId: string;
            role: string;
            active: boolean;
            cx: number;
    ...
    const originPlan = capture.originPlan as OriginPlan | null;
    ...
    for (const actor of capture.spawn) {
      const insideTileEnvelope =
        actor.cx >= originPlan.mapBounds.minX
        && actor.cx <= originPlan.mapBounds.maxX
        && actor.cy >= originPlan.mapBounds.minY
        && actor.cy <= originPlan.mapBounds.maxY;
      expect(
        insideTileEnvelope,
        `${actor.role} ${actor.actorId} must originate outside the rendered tile envelope`
      ).toBe(false);
    }

    const actorIds = new Set(capture.timeline.flatMap((sample) =>
      sample.actors.map((actor) => actor.actorId)
    ));
    ...
    for (const actorId of actorIds) {
      const actorSamples = capture.timeline
        ...
        .filter((entry): entry is { elapsedMs: number; actor: Actor } => !!entry.actor);
      const firstActive = actorSamples.findIndex((entry) => entry.actor.active);
      let lastActive = -1;
      actorSamples.forEach((entry, index) => {
        if (entry.actor.active) lastActive = index;
      });
      if (firstActive < 0 || lastActive <= firstActive) continue;

      const activeWindow = actorSamples.slice(firstActive, lastActive + 1);
      expect(
        activeWindow.every((entry) => entry.actor.active),
        `${actorId} must use one uninterrupted visible lifecycle`
      ).toBe(true);

      const observedSpeeds: number[] = [];
      for (let index = 1; index < activeWindow.length; index += 1) {
        ...
        expect(
          observedSpeed,
          `${actorId} moved too far for its ${current.actor.role} speed at ${Math.round(current.elapsedMs)}ms`
        ).toBeLessThanOrEqual(expectedSpeed * 1.35 + 0.01);
    ...
    const strikeSamples = capture.timeline.filter((sample) =>
    ...
    const bomberPairDistances = strikeSamples.flatMap((sample) => {
      ...
      return bombers.flatMap((left, leftIndex) =>
        bombers.slice(leftIndex + 1).map((right) => distance(left, right))
      );
    });
    expect(bomberPairDistances.length).toBeGreaterThan(0);
    expect(Math.min(...bomberPairDistances)).toBeGreaterThanOrEqual(48);

    const lastActiveByActor = new Map<string, Actor>();
    ...
    for (const actor of lastActiveByActor.values()) {
      const exitProjection =
        (actor.cx - mapCenter.cx) * originPlan.axis.cx
