...
test.describe("AirShow Browser Harness", () => {
  ...
  test("target-run keeps bombers on the strike lane while fighters peel away toward egress", async ({ page }) => {
    ...
    const nearestBomberPairDistancePx = Math.min(
      ...activeBomberActors.flatMap((left, leftIndex) =>
        activeBomberActors
          .slice(leftIndex + 1)
          .map((right) => Math.hypot(left.cx - right.cx, left.cy - right.cy))
      )
    );
    const bomberSpreadWidthPx =
      Math.max(...activeBomberActors.map((actor) => actor.cx))
      - Math.min(...activeBomberActors.map((actor) => actor.cx));
    const bomberSpreadHeightPx =
      Math.max(...activeBomberActors.map((actor) => actor.cy))
      - Math.min(...activeBomberActors.map((actor) => actor.cy));
    expect(
      nearestBomberPairDistancePx,
      `target-run bomber sprites should not collapse into one painted clump. nearest pair=${nearestBomberPairDistancePx.toFixed(1)}px`
    ).toBeGreaterThan(48);
    expect(
...
test.describe("AirShow Browser Replay Harness", () => {
  ...
  test("replay harness applies live camera focus so bomber ingress is painted inside the focused viewport", async ({ page }) => {
    ...
    const result = await page.evaluate(() => {
      const svg = document.getElementById("battleHexMap") as SVGSVGElement | null;
      const viewportRoot = document.getElementById("viewportRoot") as SVGGElement | null;
      if (!svg || !viewportRoot) {
      ...
      const size = 32;
      const bombers = Array.from(
        document.querySelectorAll<SVGImageElement>('[data-testid="airshow-actor"][data-airshow-role="bomber"]')
      ).map((el) => {
    ...
    const paintedBombers = activeBombers.filter((actor) =>
      actor.opacity !== "0"
    ...
    expect(
      Math.abs(result.panX) + Math.abs(result.panY) + Math.abs(result.zoomX - 1) + Math.abs(result.zoomY - 1),
      `Expected replay harness to exercise a focused camera, but viewportRoot remained near identity: ${result.transformValue}`
    ).toBeGreaterThan(1);
    expect(activeBombers).toHaveLength(4);
    expect(
      paintedBombers.length,
      `Expected bomber ingress to be painted inside the focused viewport. Visible bounds=${JSON.stringify(result.visibleBounds)} bombers=${JSON.stringify(result.bombers)}`
