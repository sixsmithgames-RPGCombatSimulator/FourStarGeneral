...
registerTest("SPRITESHEET_ANIMATOR_DOES_NOT_ADVERTISE_MISSING_LARGE_EXPLOSION_SHEET", async ({ Given, Then }) => {
  await Given("large bomb impacts use the characterized multi-impact small-explosion path", async () => {
  });

  await Then("the sprite registry does not expose the nonexistent legacy large sheet", async () => {
    if ("explosionLarge" in COMBAT_ANIMATIONS) {
      throw new Error("Expected the missing FSG_Explosion_Large sheet to remain outside the runtime registry.");
    }
  });
});
