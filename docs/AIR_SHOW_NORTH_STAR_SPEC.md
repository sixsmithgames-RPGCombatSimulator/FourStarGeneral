...
- A minimum zero-strength visual seed is lifecycle-only: it must stay visible through target release, impact effects, and egress so tutorial/live replay does not blink bombers out during the explosion window. Real bomber losses with a nonzero recorded pre-loss strength still drop before egress.
- Standalone/tutorial strike playback must use one persistent bomber sortie sprite across ingress, bomb release, impact effects, and egress. It must not split the sequence into an ingress sprite, an empty explosion window, and a later return sprite.
...
| ~~**Tutorial zero-strength bombers disappear while explosion animations play**~~ | ~~Critical~~ | ✅ **FIXED 2026-06-26** | Visual-seed-only bombers now keep their seeded active strength through target release and bomber egress, while real flak/interception kills with recorded pre-loss strength still drop before egress. Regression coverage checks tutorial-style zero-strength bombers receive visible egress assignments. |
| ~~**Standalone/tutorial strike bombers disappear between ingress and return while impact FX play**~~ | ~~Critical~~ | ✅ **FIXED 2026-06-26** | `BattleScreen` now routes non-aborted strike-only playback through `HexMapRenderer.animateAircraftSortie`, keeping one persistent bomber sprite alive while `playResolvedAirStrikeImpact` runs at target pass. Regression coverage holds the explosion promise open and verifies the sortie remains visible during impact. |
