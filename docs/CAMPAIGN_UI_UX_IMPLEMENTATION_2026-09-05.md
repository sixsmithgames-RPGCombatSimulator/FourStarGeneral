...
The first full R4 attempt at clean ec41704 stopped in professional UI after18 passes. Unchanged FSG_CAM_045_FIELD_FORMATION_SELECTION caught the new recovery-visibility exception exposing the selection section on a formation already committed to battle. The domain still rejected recovery, but the prior UI contract was violated. Parent corrected the single projection expression to use canonical present-at-location and shattered/refitting posture; ordinary ready controls keep their existing authority. The full and isolated RED evidence is retained in diagnostics/campaign-audit-release/authenticated-r4-release-results.json and recovery-visibility-red.log. Verification continues under a distinct R5 namespace after focused checks and independent review.

The unchanged field-formation contract and four actual recovery Screen cases now pass5/5; full TypeScript and scoped zero-warning lint pass. Goodall independently reviewed the exact projection and returned LOCAL architecture/contract PASS: current recovery remains visible, committed assets no longer bypass the existing gate, and State remains the sole eligibility authority.

### R5 deployment and authentic follow-up audit
Clean27b0b516f5849479dfcce03777f2d78f24f6e616 passed all eight sequential release gates: professionalUI159/159,campaign381/381,campaignbrowser26/26,tacticalgeometry19/19,reports/picker12/12,productionbuildTS0/staticcyclePASS,zero-warninglint,unfiltered774/774 with774matchingpasslines andoneexactsummary. No source changes, filters, skips or waivers during validation. Evidence: diagnostics/campaign-audit-release/authenticated-r5-release-results.json and distinct r5 browser artifacts. After a fresh shared-team capacity check (three deployments in24hours,noneactive), one intended push deployed READY dpl_8ZC43MExVte8Qf3PYzV4uiTr5Lwq to fsg.sixsmithgames.com. The external browser verified exactSHA and CSS/assets/index-_bcrCa6H.css.

The live checkpoint chooser restored only this audit's post-battle campaign8719ded1 at03:00. The primary checkpoint still shows August29,3:57:09PM, and all13original tactical checkpoints remain preserved. Native200% keyboard selection, full checkpoint text and44px actions passed at this boundary. The Omaha AAR and formation history now retain the authored location. Native AAR disclosure traversal, acknowledgement and forward/reverse focus containment passed; final close/invoker and broader live checks remain required.

The real5thEngineerSpecialBrigade recovery quote is154supply/123hours for150survivors and5equipment, preserving10killed. Draft held154 without debit; commit deducted once and marked the same formation_afb981ea Refitting. The ordinary03→06 advance executed1/41segments and restored one equipment item without another debit. HoldNormandy awarded15000supply; total214732. The repaired Caen-Orne counterattack naturally interrupted into the18-formation Playerdefense at31,22, with no support selections. The audit saved a new named defense checkpoint atturn1; recovery completion and the natural defensive outcome remain unproved.

Live018 exposed AAR→Recover/Continue leaving stale legacyselection state despite correct canonicalinspector. The bounded Screen repair now calls its existing canonical synchronizer. Live019 exposed unstyled21.2px recovery actions and focus loss after drafting; controls now reuse existing44px action styling and preserve focus on the exact new Review action or refreshed rejected context. Actual HQload→AAR→realquotedraft regressions cover prior-sibling selection and state/storage purity; focused27/27,TypeScript0 andlint0 pass. Wegener independently returned LOCAL architecture/testintegrity/boundedUXPASS. These changes still need the complete release sequence and exact deployed acceptance.

Native200% tactical testing additionally found that expanded activity log consumes280px beside a316px map pane at753x356/DPR2.5. The long defense title wraps excessively and three turn actions shrink to30px wide; the populated intel card is clipped. The nineteen local geometry cases had forced the activity log collapsed. Thus FSG-CAM-008 geometry remains OPEN; the modified-Tab correction itself passes real native traversal afterDismiss. Screenshots and source/fixture diagnosis are retained in the run ledger. No local test count closes a failed live state. Enemy-estimate legacy Defender wording on this Playerdefense is recorded separately as020; only its frozen-context presentation repair is authorized.

FSG-CAM-020's bounded Precombat consumer now relabels four exact frozen legacy Defender unknowns as Enemy uncertainty, without modifying saved packages or inferring opposing strength. Four real-State-committed, frozen-context consumer cases pass for attacker/defender andzero/twocontacts; each fails against the original display. ScopedTypeScript/lint pass. Wegener independently returned LOCAL architecture/test-integrityPASS; parent registered the four cases in the shared audit entry and extended the professional prefix through100. Deployment/livecopy remain pending.
The additional scoped review exposed two stale assertions in the previously unregistered precombatAllocs test file: medical/maintenance are now implemented in the current catalog, and the training profile's authored minimum is13 rather than12. These fail identically before andafter020 and are absent from the prior774-test runner; they are not020regressions and must not be described as having passed that runner. Registration/contract reconciliation is tracked separately from the product wording fix.
