...
The deployed campaign-led landing and real `/play?mode=campaign` entry render correctly. The visible session is a guest; normal return-to-landing, Sign In and Google authentication reach the existing-account chooser. Account selection is pending user input. A visible existing tactical save was preserved.
