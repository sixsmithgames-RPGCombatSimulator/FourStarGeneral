...
import {
  planResolvedAirCombatShowScene,
  type AirShowPlannerActor,
  type AirShowPlannerFlight,
  type PlanResolvedAirCombatShowHost
...
export class HexMapRenderer implements IMapRenderer {
  ...
  private createResolvedAirCombatShowPlannerHost(): PlanResolvedAirCombatShowHost {
    const bind = <T extends (...args: any[]) => any>(fn: T): T => fn.bind(this) as T;
    return {
  ...
  private applyPlannedAirShowAssignments(
  ...
  private async animatePlannedResolvedAirCombatShow(scene: ResolvedAirShowScene): Promise<void> {
    ...
    let runtimeTraceStatus: "success" | "error" = "success";
    let runtimeTraceError: string | null = null;
    ...
    try {
      ...
      completeAirShowRuntimeTrace(
        this.activeAirShowRuntimeTrace,
        runtimeTraceStatus,
        runtimeTraceError
      );
  ...
  private playAirTracerExchange(
    ...
    const visibleLengthPx = this.clamp(
      options.visibleLengthPx ?? Math.min(104, distance * 0.24),
      28,
      Math.min(distance, 112)
    );
  ...
  private harmonizeAirShowPhaseEntryWithPreviousPhase(
    ...
    const adjustedAssignments = currentAssignments.map((assignment) => {
      ...
      if (
        ...
        const turnHomePath = this.softenAirShowEntryWindowTurns(
  ...
  private resolveAirShowTracerBurstGeometry(
    ...
    const visibleLengthPx = this.clamp(
      burst.visibleLengthPx ?? Math.min(54, streakLengthPx * 0.14),
      24,
      Math.min(streakLengthPx, 96)
    );
  ...
  private resolveAirShowFlakBurstWave(
    ...
    const smokePuffCount = Math.max(
      8,
      Math.min(puffCount + 4, burst.smokePuffCount ?? Math.round(puffCount * 1.15))
    );
