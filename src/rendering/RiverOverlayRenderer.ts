...
export class RiverOverlayRenderer {
   ...
   */
  hasRiver(tile: TileDetails | null | undefined): boolean {
    if (!tile) {
      return false;
    }
  ...
  drawRiverOverlay(
    ...
    const hasRiverNeighbor: boolean[] = [];
    ...
    const curvatureSigns = [1, -1, 1, -1, 1, -1] as const;
    ...
    edgeVectors.forEach(({ dx, dy }, index) => {
      ...
      const curvature = 0.22 * curvatureSigns[index as 0 | 1 | 2 | 3 | 4 | 5];
