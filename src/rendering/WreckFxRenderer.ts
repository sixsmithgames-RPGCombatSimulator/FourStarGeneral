...
type LifetimeBand = Readonly<{
  minMs: number;
  maxMs: number;
}>;

type ParticleLifetimes = Readonly<Record<ParticleFamily, LifetimeBand>>;
type EmitterIntervals = Readonly<Record<ParticleFamily, number>>;

interface WreckSeverityCounts {
  readonly flame: TieredCount;
  readonly smokeLow: TieredCount;
  readonly smokeMid: TieredCount;
  readonly smokeHigh: TieredCount;
  readonly emberAir: TieredCount;
  readonly heatHaze: TieredCount;
}

interface WreckSeverityProfile {
  readonly counts: WreckSeverityCounts;
  readonly intervalsMs: EmitterIntervals;
  readonly lifetimes: ParticleLifetimes;
}

export interface WreckFxPreset {
  readonly wreckClass: WreckFxClass;
  readonly hotspotOffsets: ReadonlyArray<{ x: number; y: number }>;
  readonly emberBedCount: TieredCount;
  readonly emberGlowCount: TieredCount;
  readonly heatHazeEnabled: boolean;
  readonly severityWindowsMs: {
    readonly freshEnd: number;
    readonly burningEnd: number;
    readonly settlingEnd: number;
  };
  readonly severities: Readonly<Record<WreckFxSeverity, WreckSeverityProfile>>;
}

export interface WreckFxMountOptions {
  readonly hexKey: string;
  readonly parentGroup: SVGGElement;
...
interface EmberAirParticle {
  readonly element: SVGEllipseElement;
  active: boolean;
  bornAtMs: number;
  lifeMs: number;
  hotspotIndex: number;
...
interface WreckFxInstance {
  ...
  readonly flameGroup: SVGGElement;
  readonly smokeLowGroup: SVGGElement;
  readonly smokeMidGroup: SVGGElement;
  readonly smokeHighGroup: SVGGElement;
  readonly emberAirGroup: SVGGElement;
  readonly heatHazeGroup: SVGGElement;
  readonly emberBed: SVGElement[];
  readonly emberGlows: SVGElement[];
  readonly flames: FlameParticle[];
  readonly smokeLow: SmokeParticle[];
  readonly smokeMid: SmokeParticle[];
  readonly smokeHigh: SmokeParticle[];
  ...
  anchorX: number;
  anchorY: number;
  zoomTier: WreckFxZoomTier;
}

function counts(far: number, mid: number, near: number): TieredCount {
  return { far, mid, near };
}

function range(minMs: number, maxMs: number): LifetimeBand {
  return { minMs, maxMs };
}

function createIntervals(flame: number, smokeLow: number, smokeMid: number, smokeHigh: number, emberAir: number, heatHaze: number): EmitterIntervals {
  return { flame, smokeLow, smokeMid, smokeHigh, emberAir, heatHaze };
}

function createLifetimes(
  flame: LifetimeBand,
  smokeLow: LifetimeBand,
  smokeMid: LifetimeBand,
  smokeHigh: LifetimeBand,
  emberAir: LifetimeBand,
  heatHaze: LifetimeBand
): ParticleLifetimes {
  return { flame, smokeLow, smokeMid, smokeHigh, emberAir, heatHaze };
}

function createSeverityProfile(
  countsConfig: WreckSeverityCounts,
  intervalsMs: EmitterIntervals,
  ...
  infantry: {
    ...
    severities: {
      fresh: createSeverityProfile(
        ...
        createIntervals(460, 620, 920, 1680, 1320, 999999),
        createLifetimes(range(170, 230), range(620, 880), range(980, 1500), range(1500, 2200), range(180, 340), range(1500, 2200))
      ),
      burning: createSeverityProfile(
        ...
        createIntervals(520, 720, 1080, 1840, 1480, 999999),
        createLifetimes(range(170, 240), range(640, 900), range(1100, 1600), range(1700, 2300), range(190, 360), range(1500, 2200))
      ),
      settling: createSeverityProfile(
        ...
        createIntervals(760, 940, 1260, 1940, 1800, 999999),
        createLifetimes(range(180, 240), range(680, 940), range(1200, 1750), range(1900, 2500), range(180, 320), range(1500, 2200))
      ),
      smoldering: createSeverityProfile(
        ...
        createIntervals(999999, 1320, 1680, 2200, 999999, 999999),
        createLifetimes(range(180, 240), range(760, 1080), range(1300, 1900), range(2100, 2700), range(180, 320), range(1500, 2200))
      )
    }
  },
  truck: {
    ...
    severities: {
      fresh: createSeverityProfile(
        ...
        createIntervals(190, 250, 420, 980, 860, 999999),
        createLifetimes(range(180, 280), range(700, 1020), range(1160, 1760), range(1700, 2400), range(180, 420), range(1400, 2200))
      ),
      burning: createSeverityProfile(
        {
          flame: counts(1, 3, 4),
        ...
        createIntervals(220, 280, 460, 1040, 940, 999999),
        createLifetimes(range(190, 290), range(760, 1080), range(1220, 1820), range(1840, 2480), range(200, 460), range(1500, 2300))
      ),
      settling: createSeverityProfile(
        {
          flame: counts(0, 2, 3),
        ...
        createIntervals(320, 380, 580, 1180, 1320, 999999),
        createLifetimes(range(200, 310), range(800, 1140), range(1300, 1880), range(1960, 2550), range(190, 390), range(1500, 2300))
      ),
      smoldering: createSeverityProfile(
        {
          flame: counts(0, 0, 1),
        ...
        createIntervals(999999, 760, 920, 1400, 999999, 999999),
        createLifetimes(range(200, 300), range(820, 1200), range(1380, 1960), range(2100, 2680), range(180, 360), range(1500, 2300))
      )
    }
  },
  artillery: {
    ...
    severities: {
      fresh: createSeverityProfile(
        ...
        createIntervals(170, 235, 390, 900, 760, 1200),
        createLifetimes(range(180, 300), range(760, 1080), range(1180, 1840), range(1760, 2440), range(180, 480), range(1800, 2600))
      ),
      burning: createSeverityProfile(
        {
          flame: counts(1, 3, 5),
        ...
        createIntervals(195, 255, 430, 980, 860, 1320),
        createLifetimes(range(190, 310), range(800, 1120), range(1260, 1880), range(1880, 2520), range(190, 460), range(1900, 2700))
      ),
      settling: createSeverityProfile(
        {
          flame: counts(0, 2, 3),
        ...
        createIntervals(300, 360, 560, 1140, 1240, 1560),
        createLifetimes(range(200, 320), range(820, 1180), range(1360, 1960), range(2000, 2580), range(180, 420), range(2000, 2800))
      ),
      smoldering: createSeverityProfile(
        {
          flame: counts(0, 0, 1),
        ...
        createIntervals(999999, 760, 980, 1420, 999999, 1780),
        createLifetimes(range(200, 300), range(860, 1220), range(1420, 2040), range(2140, 2720), range(180, 360), range(2100, 2900))
      )
    }
  },
  tank: {
    ...
    severities: {
      fresh: createSeverityProfile(
        ...
        createIntervals(140, 210, 340, 780, 640, 1040),
        createLifetimes(range(190, 320), range(820, 1180), range(1280, 1940), range(1960, 2580), range(180, 500), range(2000, 2800))
      ),
      burning: createSeverityProfile(
        {
          flame: counts(2, 5, 7),
        ...
        createIntervals(155, 225, 360, 820, 720, 1180),
        createLifetimes(range(200, 330), range(860, 1220), range(1340, 2020), range(2040, 2640), range(180, 480), range(2100, 2900))
      ),
      settling: createSeverityProfile(
        {
          flame: counts(1, 3, 4),
        ...
        createIntervals(250, 320, 500, 980, 1080, 1360),
        createLifetimes(range(210, 340), range(900, 1260), range(1440, 2100), range(2140, 2720), range(180, 420), range(2200, 3000))
      ),
      smoldering: createSeverityProfile(
        {
          flame: counts(0, 1, 1),
        ...
        createIntervals(860, 620, 860, 1320, 999999, 1640),
        createLifetimes(range(210, 300), range(940, 1320), range(1500, 2180), range(2240, 2840), range(180, 360), range(2300, 3100))
      )
    }
  },
  convoy: {
    ...
    severities: {
      fresh: createSeverityProfile(
        ...
        createIntervals(105, 180, 320, 760, 560, 980),
        createLifetimes(range(210, 340), range(920, 1280), range(1380, 2040), range(2100, 2720), range(180, 520), range(2000, 2900))
      ),
      burning: createSeverityProfile(
        {
          flame: counts(3, 6, 9),
        ...
        createIntervals(120, 200, 340, 820, 620, 1100),
        createLifetimes(range(220, 350), range(960, 1320), range(1440, 2120), range(2180, 2780), range(190, 500), range(2100, 3000))
      ),
      settling: createSeverityProfile(
        {
          flame: counts(1, 4, 6),
        ...
        createIntervals(200, 280, 460, 980, 980, 1320),
        createLifetimes(range(220, 360), range(980, 1360), range(1520, 2200), range(2260, 2860), range(180, 420), range(2200, 3100))
      ),
      smoldering: createSeverityProfile(
        {
          flame: counts(0, 1, 2),
        ...
        createIntervals(720, 520, 760, 1240, 999999, 1560),
        createLifetimes(range(220, 320), range(1040, 1400), range(1600, 2280), range(2360, 2940), range(180, 360), range(2300, 3200))
      )
    }
  }
};
...
function pickTierValue(countsByTier: TieredCount, zoomTier: WreckFxZoomTier): number {
  return countsByTier[zoomTier];
}

export function resolveWreckSeverity(preset: WreckFxPreset, elapsedMs: number): WreckFxSeverity {
...
export function getWreckFxPreset(wreckClass: WreckFxClass): WreckFxPreset {
...
function createGroup(className: string): SVGGElement {
  const group = document.createElementNS(SVG_NS, "g");
  group.setAttribute("class", className);
  return group;
}

function hideElement(element: SVGElement): void {
  element.style.display = "none";
}

function showElement(element: SVGElement): void {
  element.style.display = "";
}

function ensureSharedDefs(svgRoot: SVGSVGElement): void {
  const existing = svgRoot.querySelector("#wreck-fx-shared-defs");
  if (existing) {
    return;
  }

  let defs = svgRoot.querySelector("#battleDefs") as SVGDefsElement | null;
  if (!defs) {
...
function createBlobPath(cx: number, cy: number, rx: number, ry: number, variant: number): string {
...
function hotspotPositions(preset: WreckFxPreset): ReadonlyArray<{ x: number; y: number }> {
...
function lifetimeFromBand(band: LifetimeBand, seed: number, salt: number): number {
  return Math.round(lerp(band.minMs, band.maxMs, hashUnit(seed, salt)));
}

function createEmitterState(): Record<ParticleFamily, EmitterState> {
  return {
...
export class WreckFxRenderer {
  ...
  private readonly zoomResolver: () => WreckFxZoomTier;
  private readonly instances = new Map<string, WreckFxInstance>();
  private rafHandle: number | null = null;
  private lastTimestamp = 0;

  constructor(svgRoot: SVGSVGElement, zoomResolver: () => WreckFxZoomTier) {
    this.svgRoot = svgRoot;
    this.zoomResolver = zoomResolver;
    ensureSharedDefs(svgRoot);
  }

  bindSvg(svgRoot: SVGSVGElement): void {
    this.svgRoot = svgRoot;
    ensureSharedDefs(svgRoot);
  }
  ...
  removeWreck(hexKey: string): void {
  ...
  stepForTests(timestamp: number): void {
    this.tick(timestamp);
  }
  ...
  private createInstance(hexKey: string, seed: number, preset: WreckFxPreset): WreckFxInstance {
    ...
    const emberBed: SVGElement[] = [];
    const emberGlows: SVGElement[] = [];
    for (let i = 0; i < 8; i += 1) {
  ...
  private createSmokeParticle(parent: SVGGElement, band: "low" | "mid" | "high"): SmokeParticle {
  ...
  private renderShadow(instance: WreckFxInstance, nowMs: number): void {
  ...
  private updateFlames(instance: WreckFxInstance, nowMs: number): void {
    const hotspots = hotspotPositions(instance.preset);
  ...
  private updateSmokeParticles(particles: readonly SmokeParticle[], family: "smokeLow" | "smokeMid" | "smokeHigh", nowMs: number): void {
    particles.forEach((particle) => {
      ...
      if (particle.element.tagName === "ellipse") {
        const ellipse = particle.element as SVGEllipseElement;
        ...
        const path = particle.element as SVGPathElement;
  ...
  private updateEmberAir(instance: WreckFxInstance, nowMs: number): void {
  ...
  private updateHeatHaze(instance: WreckFxInstance, nowMs: number): void {
  ...
  private emitFamily(instance: WreckFxInstance, family: ParticleFamily, dtMs: number, targetCount: number, profile: WreckSeverityProfile): void {
  ...
  private countActiveParticles(instance: WreckFxInstance, family: ParticleFamily): number {
  ...
  private spawnParticle(instance: WreckFxInstance, family: ParticleFamily, sequence: number, profile: WreckSeverityProfile): void {
    const hotspots = hotspotPositions(instance.preset);
