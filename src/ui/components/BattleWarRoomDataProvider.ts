...
interface ForceDamageTotals {
  ...
  equipmentReadiness: number | null;
}
...
export class BattleWarRoomDataProvider implements WarRoomDataProvider {
  ...
  private getActiveRosterUnits(roster: BattleRosterSnapshot): RosterUnitSummary[] {
    return [
      ...roster.frontline,
      ...roster.support,
      ...roster.reserves
  ...
  private summarizeForceDamage(roster: BattleRosterSnapshot): ForceDamageTotals {
    ...
    const averageReadiness = this.average(
      activeStatusUnits.map((unit) => unit.statusSummary?.readiness ?? unit.strength),
      0
    );
    const averageStrength = this.average(
      activeStatusUnits.map((unit) => unit.strength),
      0
    );
    const personnelReadiness = this.weightedAverage(
      activeStatusUnits.map((unit) => ({
  ...
  private weightedAverage(values: Array<{ value: number; weight: number }>, fallback: number): number {
  ...
  private composeReadinessComment(forceDamage: ForceDamageTotals): string {
  ...
  private composeRequisitions(
    ...
    const careRequests = (logisticsSnapshot?.careTargets ?? []).slice(0, 5).map((entry) => ({
      ...
      status: entry.assignedAssets > 0 ? "approved" as const : "pending" as const,
    ...
    const supplyRequests = (logisticsSnapshot?.priorityTargets ?? [])
      ...
      .map((entry) => ({
        ...
        status: entry.status === "direct" || entry.status === "delivering" ? "approved" as const : "pending" as const,
        requestedBy: `${entry.hex} priority ${entry.priority}`,
        updatedAt
      }));
  ...
  private composeLiveLogisticsBoard(
    logisticsSnapshot: LogisticsSnapshot | null,
    supplySnapshot: SupplySnapshot | null
  ): Pick<WarRoomData, "supplyStatus" | "logisticsSummary"> {
    ...
    const stockLevel = Math.max(0, Math.min(100, Math.round((
      (ammoCategory?.averagePerUnit ?? 0) +
      (fuelCategory?.averagePerUnit ?? 0) +
      Math.min(8, logisticsSnapshot.depotStock.ammo + logisticsSnapshot.depotStock.fuel)
    ) * 6)));
  ...
  private findSupplyCategory(snapshot: SupplySnapshot, resource: "ammo" | "fuel"): SupplyCategorySnapshot | undefined {
    return snapshot.categories.find((category) => category.resource === resource);
  }

  private resolveSupplySummaryStatus(
    ammoCategory: SupplyCategorySnapshot | undefined,
    fuelCategory: SupplyCategorySnapshot | undefined,
    logisticsSnapshot: LogisticsSnapshot
  ): WarRoomData["supplyStatus"]["status"] {
  ...
  private composeUnitDamageLine(unit: RosterUnitSummary): string {
    const summary = unit.statusSummary;
  ...
  private composeFieldReports(
    ...
    const damagedUnits = this.getActiveRosterUnits(roster)
      .filter((unit) => unit.statusSummary && (
        unit.statusSummary.readiness < 90 ||
