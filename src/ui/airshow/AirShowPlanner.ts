export interface AirShowPlannerPoint {
  readonly cx: number;
  readonly cy: number;
}

export type AirShowPlannerRole = "interceptor" | "escort" | "bomber";

export interface AirShowMapBounds {
...
export interface AirShowHqAxis {
  readonly playerOrigin: AirShowPlannerPoint;
  readonly botOrigin: AirShowPlannerPoint;
  readonly axis: { readonly x: number; readonly y: number };
  readonly playerBoundary: AirShowPlannerPoint;
  readonly botBoundary: AirShowPlannerPoint;
  readonly mapBounds: AirShowMapBounds;
}

export interface AirShowInspectionOriginPlan {
  readonly offsetPx: number;
  readonly axis: AirShowPlannerPoint;
  readonly mapBounds: AirShowMapBounds;
  readonly playerBoundary: AirShowPlannerPoint;
  readonly botBoundary: AirShowPlannerPoint;
  readonly playerOrigin: AirShowPlannerPoint;
...
export interface AirShowInspectionPhaseTimingRoleAudit {
...
export interface AirShowInspectionPhaseTimingAudit {
  readonly label: string;
  readonly durationMs: number;
  readonly roles: ReadonlyArray<AirShowInspectionPhaseTimingRoleAudit>;
}
...
export function buildAirShowMapBounds(
  centers: ReadonlyArray<AirShowPlannerPoint>,
  hexWidth: number,
  hexHeight: number
): AirShowMapBounds | null {
  if (centers.length === 0) {
    return null;
  }
...
export function resolveAirShowBoundsRayIntersection(
  origin: AirShowPlannerPoint,
  direction: { readonly x: number; readonly y: number },
  bounds: AirShowMapBounds
): AirShowPlannerPoint | null {
  const candidates: Array<{ t: number; point: AirShowPlannerPoint }> = [];
...
export function resolveAirShowFallbackOrigin(
  center: AirShowPlannerPoint,
  faction: string | undefined,
  bounds: AirShowMapBounds | null,
  options: {
    readonly offsetPx?: number;
...
export function resolveAirShowHqAxis(
  playerHq: AirShowPlannerPoint | null,
  botHq: AirShowPlannerPoint | null,
  mapBounds: AirShowMapBounds | null,
  offsetPx = AIR_SHOW_OFF_MAP_DISTANCE_PX
): AirShowHqAxis | null {
  ...
  const playerBoundary =
    resolveAirShowBoundsRayIntersection(playerHq, axis, mapBounds)
    ?? {
      cx: mapBounds.minX,
      cy: playerHq.cy
    };
  const botBoundary =
    resolveAirShowBoundsRayIntersection(botHq, { x: -axis.x, y: -axis.y }, mapBounds)
    ?? {
      cx: mapBounds.maxX,
      cy: botHq.cy
    };
...
export function buildAirShowInspectionOriginPlan(
  hqAxis: AirShowHqAxis,
  offsetPx = AIR_SHOW_OFF_MAP_DISTANCE_PX
...
export function buildAirShowPhaseTimingAudit(
  label: string,
  durationMs: number,
  samples: ReadonlyArray<AirShowPhaseTimingSample>,
  roleTargetSpeeds: ReadonlyMap<AirShowPlannerRole, number>
