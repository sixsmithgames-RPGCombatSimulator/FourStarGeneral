...
export function planResolvedAirCombatShowScene(
  ...
  const isVisualSeedOnlyBomberFlight = (flight: AirShowPlannerFlight): boolean =>
    flight.spec.role === "bomber"
    && Math.max(
      0,
      flight.spec.strengthBefore,
      flight.spec.strengthAfterEscortPhase ?? 0,
      flight.spec.finalStrength ?? 0
    ) <= 0
    && flight.currentStrength > 0;
  const resolvePostTargetRunBomberStrength = (flight: AirShowPlannerFlight): number =>
    isVisualSeedOnlyBomberFlight(flight)
  ...
  const buildCorridorContestedAirShowPlan = (): PlannedAirShowScene | null => {
    ...
    type BomberCorridorPlan = {
      ...
      targetCenter: AirShowPoint;
      attackForward: { x: number; y: number };
    };
    ...
    const roundPathCorners = (
      ...
      const rounded: AirShowPoint[] = [points[0]!];
    ...
    const buildCubicAirShowPath = (
      start: AirShowPoint,
      firstControl: AirShowPoint,
      secondControl: AirShowPoint,
      end: AirShowPoint,
      steps = 12
    ): AirShowPoint[] => {
    ...
    const matchPathLengthWithCarry = (
      points: ReadonlyArray<AirShowPoint>,
    ...
    const matchAssignmentPathLength = (
      assignment: AirShowPhaseAssignment,
      durationMs: number,
      lateralSign = 1
    ): AirShowPhaseAssignment => {
    ...
    const buildEndpointPreservingLengthMatchedPath = (
      start: AirShowPoint,
      end: AirShowPoint,
      targetLengthPx: number,
      lateralSign = 1
      ...
      const normal = { x: -forward.y, y: forward.x };
      const sideSign = lateralSign < 0 ? -1 : 1;
      const buildArc = (lateralPx: number): AirShowPoint[] =>
        dedupePath([
          start,
          {
    ...
    const buildGovernedPassRail = (
      ...
      const approachLead = host.offsetAirShowPoint(
        start,
        forward.x * leadDistancePx + normal.x * sideSign * lateralDriftPx * 0.35,
        forward.y * leadDistancePx + normal.y * sideSign * lateralDriftPx * 0.35
      );
      const prePass = host.offsetAirShowPoint(
        passPoint,
        -forward.x * Math.min(72, approachBudgetPx * 0.2) + normal.x * sideSign * lateralDriftPx * 0.26,
        -forward.y * Math.min(72, approachBudgetPx * 0.2) + normal.y * sideSign * lateralDriftPx * 0.26
      );
      const prePassAlongPx =
        (prePass.cx - start.cx) * forward.x + (prePass.cy - start.cy) * forward.y;
      const keepPrePass =
        directToPassPx >= 96
      ...
      const baseLengthPx = measurePathLength(basePath);
      const smoothMatchedPath = (points: ReadonlyArray<AirShowPoint>): AirShowPoint[] =>
        matchPathLengthWithCarry(
          roundPathCorners(points, Math.min(34, Math.max(10, targetLengthPx * 0.08)), 5),
          targetLengthPx,
          sideSign
        );
      ...
      const exitPoint = host.offsetAirShowPoint(
        passPoint,
        forward.x * exitDistancePx + normal.x * sideSign * Math.min(42, exitDistancePx * 0.12),
    ...
    const buildGovernedMergeTurnRail = (
      ...
      const approachLead = host.offsetAirShowPoint(
        start,
        forward.x * approachLeadDistancePx,
        forward.y * approachLeadDistancePx
      );
      ...
      const centerPoint = host.offsetAirShowPoint(
        passPoint,
        normal.x * turnSign * radiusPx,
        normal.y * turnSign * radiusPx
      );
      const startAngleRad = Math.atan2(passPoint.cy - centerPoint.cy, passPoint.cx - centerPoint.cx);
      const buildArc = (angleRad: number): AirShowPoint[] => {
    ...
    const bomberPlans = bomberFlights.map((bomberFlight, index): BomberCorridorPlan => {
      ...
      const attackAlongSign =
        Math.abs(targetDeltaAlongPx) > 1
      ...
      const turnEntry = host.offsetAirShowPoint(
        targetCenter,
        -attackForward.x * 92 + attackNormal.x * laneOffsetPx,
        -attackForward.y * 92 + attackNormal.y * laneOffsetPx
      );
      const nearTarget = host.offsetAirShowPoint(
        targetCenter,
        -attackForward.x * 48 + attackNormal.x * laneOffsetPx,
        -attackForward.y * 48 + attackNormal.y * laneOffsetPx
      );
      ...
      const targetRunExit = host.offsetAirShowPoint(
        targetCenter,
        attackForward.x * 112 + attackNormal.x * laneOffsetPx,
        attackForward.y * 112 + attackNormal.y * laneOffsetPx
      );
      const targetRunPath = roundPathCorners(
        dedupePath([turnEntry, nearTarget, releasePoint, targetRunExit]),
        18,
        6
      );
      return {
    ...
    const alignClashFightersThroughSharedFightSpace = (
      ...
      return assignments.map((assignment) => {
        ...
        const convergencePoint = host.offsetAirShowPoint(
          fightSpaceCenter,
          corridor.axis.x * roleSide * 3 + corridor.normal.x * (lane * 12 + actorSlot * 5),
          corridor.axis.y * roleSide * 3 + corridor.normal.y * (lane * 12 + actorSlot * 5)
        );
        const roleSpeedPxPerMs = roleSpeeds.get(assignment.actor.role) ?? host.airShowFighterSpeedPxPerMs;
        const points = buildGovernedMergeTurnRail(
          start,
          convergencePoint,
          durationMs,
          roleSpeedPxPerMs,
    ...
    const alignScrambleFightersThroughChasePocket = (
      ...
      const fighterAssignmentsByFlightId = new Map<string, AirShowPhaseAssignment[]>();
      fighterAssignments.forEach((assignment) => {
      ...
      fighterGroups.forEach((group, groupIndex) => {
        ...
        const groupForward = interceptorStart && escortStart
          ? normalizeVector(
              escortStart.cx - interceptorStart.cx,
              escortStart.cy - interceptorStart.cy,
              fallbackForward.x,
              fallbackForward.y
            )
          : fallbackForward;
      ...
      return assignments.map((assignment) => {
        ...
        const initialTurnPoint = rawTurnAlongPx < minimumTurnAlongPx
          ? host.offsetAirShowPoint(
              rawTurnPoint,
              chaseForward.x * (minimumTurnAlongPx - rawTurnAlongPx),
              chaseForward.y * (minimumTurnAlongPx - rawTurnAlongPx)
            )
          : rawTurnPoint;
        const turnDistancePx = Math.hypot(initialTurnPoint.cx - start.cx, initialTurnPoint.cy - start.cy);
        const minimumTurnDistancePx =
          assignment.actor.role === "escort"
            ? Math.min(68, Math.max(52, targetLengthPx * 0.2))
            : 0;
        const turnPoint =
          minimumTurnDistancePx > 0 && turnDistancePx > 0.5 && turnDistancePx < minimumTurnDistancePx
            ? host.offsetAirShowPoint(
                start,
                (initialTurnPoint.cx - start.cx) * (minimumTurnDistancePx / turnDistancePx),
                (initialTurnPoint.cy - start.cy) * (minimumTurnDistancePx / turnDistancePx)
              )
            : initialTurnPoint;
        ...
        const turnEntryForward = normalizeVector(
          turnPoint.cx - start.cx,
          turnPoint.cy - start.cy,
          chaseForward.x,
          chaseForward.y
        );
        ...
        const needsEscortRecovery =
          assignment.actor.role === "escort"
          && turnToChaseAngleDeg > 108;
        const escortRecoveryForward = needsEscortRecovery
          ? normalizeVector(
              turnExitForward.x + turnEntryForward.x * 0.86,
              turnExitForward.y + turnEntryForward.y * 0.86,
              turnExitForward.x,
              turnExitForward.y
            )
          : turnExitForward;
        ...
        const recoveryPoint = needsEscortRecovery
          ? host.offsetAirShowPoint(
              turnPoint,
              escortRecoveryForward.x * escortRecoveryDistancePx,
        ...
        const chaseExit = recoveryPoint
          ? host.offsetAirShowPoint(
              ...
              chaseForward.x * Math.max(72, targetLengthPx * 0.28)
                + chaseNormal.x * roleLaneOffsetPx * 0.35,
    ...
    const finalizeCorridorPhaseAssignments = (
        ...
        label === "bomber-defense-pass"
          ? finalSpeedAdjustedAssignments.map((assignment) => {
              ...
              const points = buildGovernedPassRail(
                ...
                interceptPoint,
                durationMs,
                roleSpeeds.get("interceptor") ?? host.airShowFighterSpeedPxPerMs,
                0.46,
                attackSideSign
              );
              return {
      ...
      const bomberIngressFighterGovernedAssignments =
        label === "bomber-ingress"
          ? (() => {
              const governedFighterAssignments = host.extendAirShowPhaseAssignmentsForSpeed(
                endpointAlignedAssignments.filter(
                  (assignment) =>
                    assignment.actor.role === "interceptor"
                    || assignment.actor.role === "escort"
                ),
              ...
              const governedFighterAssignmentByActorId = new Map(
                governedFighterAssignments.map((assignment) => [assignment.actor.id, assignment] as const)
              );
              return endpointAlignedAssignments.map(
                (assignment) => {
    ...
    const buildHomeSideTargetRunFighterPath = (
      ...
      return {
        ...assignment,
        points: matchPathLengthWithCarry(
          buildCubicAirShowPath(start, firstControl, secondControl, sideExit, 16),
          targetLengthPx,
          sideSign
        ),
        progressTimeline: undefined
    ...
    const targetRunPackageForward = normalizeVector(
      bomberPlans.reduce((sum, plan) => sum + plan.attackForward.x, 0),
      bomberPlans.reduce((sum, plan) => sum + plan.attackForward.y, 0),
      corridor.axis.x,
      corridor.axis.y
    );
    const targetRunBomberActorCountByFlightId = new Map<string, number>();
    rawTargetRunBomberAssignments.forEach((assignment) => {
      if (assignment.actor.role !== "bomber") {
        return;
      }
      targetRunBomberActorCountByFlightId.set(
        assignment.actor.flightId,
        Math.max(
          targetRunBomberActorCountByFlightId.get(assignment.actor.flightId) ?? 0,
          assignment.actor.formationIndex + 1
        )
      );
    });
    const buildTargetRunPhaseAssignments = (durationMs: number): AirShowPhaseAssignment[] => {
      ...
      return host.extendAirShowPhaseAssignmentsForSpeed(
        ...
        const targetForward = normalizeVector(
          releaseBase.cx - preparedStart.cx,
          releaseBase.cy - preparedStart.cy,
          sourcePlan?.attackForward.x ?? targetRunPackageForward.x,
          sourcePlan?.attackForward.y ?? targetRunPackageForward.y
        );
        const packageForwardAlignment =
          targetRunPackageForward.x * targetForward.x
        ...
        const inheritedForward = previousActorMotion
          ? normalizeVector(
              previousActorMotion.dx,
              previousActorMotion.dy,
              targetForward.x,
              targetForward.y
            )
          : null;
        ...
        const entryForward = inheritedForward && inheritedAlignment > -0.15
          ? inheritedForward
          : targetForward;
        const entryCommitDistancePx = Math.min(
          host.clamp(releaseDistancePx * 0.28, 18, 72),
          Math.max(8, releaseDistancePx * 0.55)
        );
    ...
    const governedTargetRunDurationMs = host.clamp(
      Math.round(maxBomberTargetRunDurationMs || meanBomberTargetRunDurationMs),
      1,
      60000
    );
    ...
    targetRunAssignments = targetRunAssignments.map((assignment) => {
      ...
      const currentPathLengthPx = host.resolveAirShowAssignmentTraversedPathLengthPx(
        assignment,
        targetRunDurationMs
      );
      ...
      if (pathLengthDeficitPx > 8 && assignment.points.length >= 2) {
        ...
        const extensionForward = normalizeVector(
          endPoint.cx - previousPoint.cx,
          endPoint.cy - previousPoint.cy,
          targetRunPackageForward.x,
          targetRunPackageForward.y
        );
        adjustedAssignment = {
          ...assignment,
          points: dedupePath([
            ...
            host.offsetAirShowPoint(
              endPoint,
              extensionForward.x * pathLengthDeficitPx,
              extensionForward.y * pathLengthDeficitPx
            )
          ])
        };
      }
