...
export class BattleScreen {
    ...
    if (flight.kind === "strike") {
      if (typeof (renderer as any).animateAircraftSortie === "function") {
        await (renderer as any).animateAircraftSortie(
          ...
          flight.unitType,
          {
            ingressDurationMs: this.scaleAirSequenceMs(2200),
  ...
  private async playDamagedAircraftReturn(
    ...
    await this.animateAircraftLeg(renderer, fromKey, toKey, unitType, this.scaleAirSequenceMs(1900), (progress, centerX, centerY) => {
      if (damage <= 0) {
        return;
      }
      while (progress >= nextSmokeProgress && nextSmokeProgress < 0.96) {
