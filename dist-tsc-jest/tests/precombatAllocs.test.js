...
import { allocationOptions, ALLOCATION_BY_KEY, ALLOCATION_BY_CATEGORY, isAllocationKey, getAllocationOption } from "../src/data/unitAllocation";
...
registerTest("PRECOMBAT_UNIMPLEMENTED_LOGISTICS_RENDER_AS_PENDING", async ({ Given, When, Then }) => {
  ...
  `;
    ...
    await When("the logistics cards are queried for unavailable rows", async () => {
        medicCard = document.querySelector('#allocationLogisticsList [data-key="medic"]');
        maintenanceCard = document.querySelector('#allocationLogisticsList [data-key="maintenance"]');
    });
...
registerTest("PRECOMBAT_RIVER_WATCH_USES_AUTHORED_MISSION_PACKAGE", async ({ Given, When, Then }) => {
    ...
    let doctrineElement = null;
    let turnLimitElement = null;
    let supplyList = null;
    ...
    `;
        ...
        supplyList = document.getElementById("baselineSupplyList");
        const fakeScreenManager = {
            showScreen: () => { },
            showScreenById: () => { },
            getCurrentScreen: () => null
...
registerTest("PRECOMBAT_ALLOCATIONS_LOOKUP_GUARDS", async ({ Given, When, Then }) => {
    ...
    await Given("the category lookup map and key guard", async () => {
        missingCategories = Array.from(allowedCategories).filter((category) => !ALLOCATION_BY_CATEGORY.has(category));
        unexpectedCategories = Array.from(ALLOCATION_BY_CATEGORY.keys()).filter((category) => !allowedCategories.has(category));
    });
