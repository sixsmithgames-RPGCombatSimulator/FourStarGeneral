...
function createFrameCanvases(frameSources, frameWidth, frameHeight) {
...
function createCachedFrames(frameSources, frameWidth, frameHeight) {
    return {
        frameWidth,
        frameHeight,
...
registerTest("FRAME_SEQUENCE_ANIMATOR_REUSES_ONE_NODE_AND_RESOLVES_AFTER_CLEANUP", async ({ Given, When, Then }) => {
    ...
    const frameSources = ["frame-0", "frame-1", "frame-2", "frame-3"];
    ...
    await When("the animation plays through successive frame ticks", async () => {
        ...
        const initialSurface = overlay.querySelector("foreignObject[data-frame-sequence-surface='true']");
        const initialCanvas = overlay.querySelector("canvas");
        ...
        for (let index = 0; index < expectedFrames.length; index += 1) {
            ...
            const surface = overlay.querySelector("foreignObject[data-frame-sequence-surface='true']");
            if (!surface) {
...
registerTest("FRAME_SEQUENCE_ANIMATOR_REJECTS_FULL_SHEET_URL_FRAME_SOURCES", async ({ Given, When, Then }) => {
    ...
    const frameSources = [resolvedSpec.imagePath, "frame-1", "frame-2", "frame-3"];
    const frames = createCachedFrames(frameSources, 64, 64);
    const animator = new FrameSequenceAnimator(overlay, {
        resolveSpec: async () => resolvedSpec,
        resolveFrames: async () => frames
