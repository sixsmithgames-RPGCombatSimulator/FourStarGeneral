...
import { planHeuristicBotTurn } from "../src/game/bot/BotPlanner";
...
const terrain = { plains, woods, roughWoods };
const playerInfantryDef = {
    class: "infantry",
...
function createPlannedAction(type, definition, origin, destination, path, score) {
    return {
...
registerTest("BOT_EXECUTION_MOVES_LANE_CLEARING_UNITS_BEFORE_THEIR_FOLLOWERS", async ({ Given, When, Then }) => {
    ...
    await Given("a rear tank plan that depends on a lead element clearing the lane first", async () => {
        ...
        const rearTankPlan = createPlannedAction("RearTank", playerTankDef, { q: 0, r: 0 }, { q: 3, r: 0 }, [
            { q: 0, r: 0 },
            { q: 1, r: 0 },
            { q: 2, r: 0 },
            { q: 3, r: 0 }
        ], 25);
        const leadTankPlan = createPlannedAction("LeadTank", playerTankDef, { q: 1, r: 0 }, { q: 2, r: -1 }, [
            { q: 1, r: 0 },
            { q: 2, r: -1 }
        ], 12);
        orderedOrigins = engine.prioritizeHeuristicPlansForExecution([
            rearTankPlan,
            leadTankPlan
        ]).map((plan) => axialKey(plan.origin));
    });
...
registerTest("BOT_LEVEL_MULTIPLE_BOMBERS_QUEUE_MULTIPLE_STRIKES", async ({ Given, When, Then }) => {
    ...
    await Then("both bombers should resolve strike missions against meaningful targets", async () => {
        const strikeReports = engine.getAirMissionReports().filter((entry) => entry.faction === "Bot" && entry.kind === "strike" && entry.event === "resolved");
