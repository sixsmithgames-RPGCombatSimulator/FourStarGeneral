...
registerTest("BATTLE_SCREEN_SELECTION_INTEL_NOTES_SKIP_REDUNDANT_SENTRY_AND_FORTIFICATION_COPY", async ({ Then }) => {
    ...
    const notes = BattleScreen.prototype.buildBattleIntelNotes.call(fakeScreen, { type: "AT_Gun_50mm", entrench: 0 }, {
...
registerTest("BATTLE_SCREEN_STACK_UNIT_SELECTOR_SWITCHES_THE_ACTIVE_UNIT_ON_A_SHARED_HEX", async ({ Then }) => {
    ...
    const fakeScreen = {
        ...
        updateSelectionFeedback: (hexKey) => {
            refreshedHexKey = hexKey;
        }
    };
    await BattleScreen.prototype.executeSelectionIntelAction.call(fakeScreen, "selectUnit:infantry_2");
...
registerTest("BATTLE_SCREEN_MOVE_FAILURE_MESSAGE_EXPLAINS_THE_PROBLEM_AND_FIX", async ({ Then }) => {
    const message = BattleScreen.prototype.buildMoveFailureMessage(new Error("This battery must choose Move Out before it can be towed."));
...
registerTest("BATTLE_SCREEN_AMMO_HELPERS_DISTINGUISH_LOW_AMMO_FROM_ALREADY_ATTACKED", async ({ Then }) => {
    const helpers = BattleScreen.prototype;
    const fakeScreen = {
    ...
    const standardAmmoMessage = helpers.buildBattleAmmoStatusMessage.call(fakeScreen, { ammo: 1 }, { moveType: "tracked", class: "tank", traits: [] });
    const lowAmmoMessage = helpers.buildBattleAmmoStatusMessage.call(fakeScreen, { ammo: 1 }, { moveType: "towed", class: "artillery", traits: [] });
...
registerTest("BATTLE_SCREEN_DOES_NOT_RENDER_SPOTTED_CONTACT_MARKERS_OVER_FRIENDLY_STACKS", async ({ Then }) => {
    ...
    BattleScreen.prototype.renderEngineUnits.call(fakeScreen);
