...
registerTest("CAMPAIGN_RENDERER_RENDERS_LAYERS", async ({ Given, When, Then }) => {
    ...
    await Then("background, hexes, sprites, and fronts are present", async () => {
        ...
        if (!bg)
            throw new Error("Background image layer missing");
        ...
        if (!front)
            throw new Error("Front polyline not rendered");
    });
    await When("a campaign hex is clicked", async () => {
        ...
        const anyHex = svg.querySelector(".campaign-hex:not(.campaign-hex-padding)");
        if (!anyHex)
            throw new Error("No campaign hex rendered for click test");
