...
import { GameEngine, buildScenarioUnitsFromAllocation } from "../src/game/GameEngine";
...
const testUnitTypes = unitTypesData;
const testTerrain = terrainData;
...
function makeContexts(attackerHex, defenderHex) {
    return {
...
const terrainDictionary = terrainDataJson;
const unitTypesDictionary = unitTypesJson;
const smokeScenario = {
...
function cloneSide(side) {
    return JSON.parse(JSON.stringify(side));
}
...
registerTest("ENGINE_INITIALIZE_FROM_ALLOCATIONS_RESETS_RESERVES", async ({ Given, When, Then }) => {
    ...
    await Given("an engine seeded with allocations", async () => {
        ...
        const units = buildScenarioUnitsFromAllocation([{ hex: { q: 0, r: 0 }, unitKey: "infantryBattalion" }], deploymentTemplates, testUnitTypes);
...
registerTest("ENGINE_END_TURN_REQUIRES_BASE_CAMP", async ({ Given, When, Then }) => {
    ...
    let reportDuringDeployment = null;
    let reportAfterStart = null;
    let subsequentReport = null;
    ...
    await When("finalizing deployment", async () => {
        ...
        try {
            ...
            if (!error.message.includes("base camp")) {
                throw error;
            }
        }
