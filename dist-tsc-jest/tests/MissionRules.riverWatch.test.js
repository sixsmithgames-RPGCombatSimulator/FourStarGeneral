...
function makeSnapshot(controllerKey, turnNumber, occupancyEntries, playerUnits, botUnits) {
    const controller = createMissionRulesController(controllerKey, riverWatchScenario);
    const turnSummary = { phase: "playerTurn", activeFaction: "Player", turnNumber };
    const occupancy = new Map(occupancyEntries);
...
registerTest("missionRules: secondary and tertiary flags", async ({ Then }) => {
    const playerRecon = { type: "Recon_Bike", hex: { q: 0, r: 0 }, strength: 100, experience: 0, ammo: 0, fuel: 0, entrench: 0, facing: "NE" };
    const { status: withRecon } = makeSnapshot("patrol_river_watch", 1, [], [playerRecon], [
...
registerTest("missionRules: river watch phases escalate on Normal", async ({ Given, When, Then }) => {
    ...
    await When("the battle reaches turn 4 and then holds all three fords for two turns", async () => {
        ...
        controller.onTurnAdvanced({
            ...
            scenario: riverWatchScenario,
            occupancy: new Map([["6,2", "Player"], ["6,4", "Player"], ["5,9", "Player"]]),
            playerUnits: [],
            botUnits: []
        });
