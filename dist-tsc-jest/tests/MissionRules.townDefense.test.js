...
function makeUnit(type, q, r, strength = 100) {
    return {
        type,
        hex: { q, r },
        strength,
...
function makeOccupancy(entries) {
...
function offsetObjectiveKey(col, row) {
    return `${col},${row - Math.floor(col / 2)}`;
}
function makeHistoricalScenario(name, objectives, turnLimit) {
    return {
        name,
