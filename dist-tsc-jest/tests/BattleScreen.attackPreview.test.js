...
registerTest("BATTLESCREEN_ATTACK_DIALOG_EXPLAINS_AT_GUN_RANGE_AND_PENETRATION_MATH", async ({ Given, When, Then }) => {
  ...
  `;
    ...
    const fakeEngine = {
        ...
        previewAttack(_attacker, _defender) {
            const result = {
                accuracy: 22.38705,
                shots: 120,
                damagePerHit: 0.2163,
    ...
    await When("the commander opens the attack dialog", async () => {
        screen.promptAttackConfirmation({ q: 0, r: 0 }, { q: 2, r: 0 });
    });
...
registerTest("BATTLESCREEN_ATTACK_DETAILS_CAP_DAMAGE_DISPLAY_AT_100_PERCENT", async ({ Given, When, Then }) => {
    ...
    await Given("a battle screen with an overkill preview and resolution", async () => {
        ...
        sections = screen.buildPlayerAttackDetails(resolution, preview, {
            attackerHex: "10,2",
            defenderHex: "10,4",
            inflictedDamage: 468,
            retaliationDamage: 0
...
registerTest("BATTLESCREEN_ATTACK_DIALOG_USES_RESOLVED_FORMATION_LABEL_AND_DECIMAL_EXPECTED_HITS", async ({ Given, When, Then }) => {
  ...
  `;
    ...
    await When("the attack confirmation dialog is opened for that unit", async () => {
        screen.promptAttackConfirmation({ q: 0, r: 0 }, { q: 2, r: 0 }, { attackerUnitId: "u_tank_1" });
    });
