...
function inspectCoordinatedScene(scene, subjectLabel, strikeSortieMissionIds) {
    if (!scene) {
        return null;
    }
    ...
    try {
        ...
        if (fighterIngress) {
            ...
            if (bomberAssignments.length > 0 && fighterAssignments.length > 0) {
                const averageDistance = (assignments, progress) => assignments.reduce((sum, assignment) => {
                    const nearest = assignment.sampledPositions.reduce((closest, sample) => Math.abs(sample.progress - progress) < Math.abs(closest.progress - progress) ? sample : closest);
                ...
                if (bomberMeanDistance + 20 < fighterMeanDistance) {
                    findings.push({
                        ...
                        message: `${subjectLabel} places bombers ${Math.round(fighterMeanDistance - bomberMeanDistance)}px deeper than fighters ` +
...
function buildPlaybackProjection(engine, arrivals, engagements) {
    ...
    const coordinatedPlans = clusterOperationGroups
        ...
        const fighterSceneInspection = inspectCoordinatedScene(plan.scene, `coordinated cluster #${clusterIndex + 1}`, Array.from(plan.strikeMissionIds));
        return {
            clusterIndex,
...
function buildInspectableScene(engine, event, flakEvent) {
    ...
    const linkedEscortFlights = event.type === "airToAir"
        ? engine
            ...
            originKey: (entry.originHexKey ? CoordinateSystem.axialKeyToOffsetKey(entry.originHexKey) : null)
...
function resolveSampledBoundaryVector(sampledPositions, edge) {
    if (sampledPositions.length < 2) {
        return null;
    }
    if (edge === "start") {
        const boundary = sampledPositions[0];
        const reference = sampledPositions.find((sample, index) => index > 0 && Math.hypot(sample.cx - boundary.cx, sample.cy - boundary.cy) > 0.5);
        if (!reference) {
            return null;
        }
        return {
...
function detectAnomalies(engine, missionReports, engagements, expectedFlakCoverageByMissionId) {
    const anomalies = [];
    const internals = engine;
    ...
    missionReports
        ...
        if (report.outcome?.result !== "destroyed" && expectedCoverage.length > 0 && !hasFlakEvent) {
            anomalies.push({
                ...
                message: `${describeMission(report)} entered a target hex covered by sentry flak (${expectedCoverage.join(", ")}), but no flak engagement event was recorded.`
            });
        }
            ...
            anomalies.push({
                ...
                message: `${describeMission(report)} appears to have defending flak coverage in the resolved state, but no flak engagement event was recorded.`
            });
        }
    });
    return anomalies;
...
export function formatAirScenarioReport(result) {
    ...
    const formatHeading = (headingDegrees) => typeof headingDegrees === "number" && Number.isFinite(headingDegrees)
        ? `${Math.round(headingDegrees)}deg`
        : "n/a";
    const formatPxPer100Ms = (speedPxPer100Ms) => typeof speedPxPer100Ms === "number" && Number.isFinite(speedPxPer100Ms)
        ? `${speedPxPer100Ms.toFixed(1)}px/100ms`
        : "n/a";
    const describeSpeedWindow = (sampledPositions) => {
    ...
    const describeSampleTrack = (sampledPositions) => sampledPositions
        .map((sample, index) => {
        ...
        .join(" | ");
    const describeActor = (inspectionReport, assignment) => {
    ...
    if (coordinatedGeometryPlans.length > 0) {
        ...
        coordinatedGeometryPlans.forEach((plan) => {
            ...
            lines.push(`- coordinated cluster #${plan.clusterIndex + 1} focus=${plan.focusKey ?? "<none>"} center=(${Math.round(report.center.cx)},${Math.round(report.center.cy)}) phases=${report.phases.length} sorties=${plan.strikeSortieMissionIds.join("|") || "<none>"}`);
            if (report.flights.length > 0) {
                ...
                report.flights.forEach((flight) => {
                    ...
                    const actorSummary = flight.actors
                        .map((actor) => `${actor.actorId}@${formatPoint(actor.position)} ${formatHeading(actor.headingDegrees)} ${actor.active ? "active" : "inactive"}`)
                        .join(" | ");
                    lines.push(`    ${flight.id} ${flight.faction ?? "Unknown"} ${flight.scenarioType} ${flight.role}/${flight.combatRole ?? "unknown"} ` +
            ...
            report.phases.forEach((phase) => {
                ...
                phase.flakBursts.forEach((flak, index) => {
                    ...
                    lines.push(`    flak ${Math.round(flak.progress * 100)}% bomber=${flak.bomberUnitKey ?? "<none>"} targetHex=${flak.targetHexKey ?? "<none>"} ` +
                    ...
                    lines.push(`      points=${flak.points.map((point) => formatPoint(point)).join(" | ")}`);
                });
            });
        });
        lines.push("");
...
function sanitizeDiagnosticFileSegment(value) {
    return value
...
function formatSceneTimeMs(value) {
    return `${Math.round(value).toString().padStart(5, "0")}ms`;
}
function describeSampleSpeedPxPer100Ms(current, previous) {
    if (!previous) {
        return "start";
    }
...
function collectAirScenarioDiagnosticAnimations(result) {
    const animations = [];
...
function buildAirScenarioAnimationIndexText(animation) {
    const phaseStartTimes = animation.report.phases.reduce((starts, phase, index) => {
        if (index === 0) {
            starts.push(0);
            return starts;
        }
    ...
    animation.report.phases.forEach((phase, index) => {
        ...
        lines.push(`- ${phase.label} sceneT=${formatSceneTimeMs(startMs)}..${formatSceneTimeMs(endMs)} duration=${phase.durationMs}ms ` +
        ...
        phase.tracers.forEach((tracer, tracerIndex) => {
            ...
            lines.push(`  tracer#${tracerIndex + 1} sceneT=${formatSceneTimeMs(sceneTimeMs)} src=${tracer.sourceActorId} ` +
        ...
        phase.flakBursts.forEach((flak, flakIndex) => {
            ...
            lines.push(`  flak#${flakIndex + 1} sceneT=${formatSceneTimeMs(sceneTimeMs)} bomber=${flak.bomberUnitKey ?? "<none>"} ` +
    ...
    animation.report.flights.forEach((flight) => {
        flight.actors.forEach((actor) => {
            lines.push(`- sprites/${animation.id}__${sanitizeDiagnosticFileSegment(actor.actorId)}.txt :: ${actor.actorId} ${flight.faction ?? "Unknown"} ${flight.scenarioType} ${flight.role}/${flight.combatRole ?? "unknown"}`);
        });
    });
    return lines.join("\n");
}
function buildAirScenarioSpriteTimelineText(animation, flight, actor) {
    const phaseStartTimes = animation.report.phases.reduce((starts, phase, index) => {
        if (index === 0) {
            starts.push(0);
            return starts;
    ...
    animation.report.phases.forEach((phase, phaseIndex) => {
        ...
        assignment.sampledPositions.forEach((sample, sampleIndex) => {
            ...
            lines.push(`  sceneT=${formatSceneTimeMs(sceneTimeMs)} phaseT=${formatSceneTimeMs(sample.timeMs)} progress=${sample.progress.toFixed(2)} ` +
...
export function buildAirScenarioDiagnosticTextFiles(result) {
    ...
    animations.forEach((animation) => {
        ...
        indexLines.push(`- ${animationFile} :: phases=${animation.report.phases.length} sprites=${animation.report.flights.reduce((sum, flight) => sum + flight.actors.length, 0)} findings=${animation.findings.length}`);
        files.push({
...
export function formatAirScenarioSummary(result) {
    const animations = collectAirScenarioDiagnosticAnimations(result);
    const lines = [];
    lines.push(`Scenario: ${result.scenarioName}`);
    lines.push(`Mission arrivals: ${result.arrivals.length}`);
