...
registerTest("BOMBER_CONTINUOUS_FORWARD_PROGRESS_DURING_PRE_TARGET_PHASES", async ({ Given, When, Then }) => {
    ...
    await Then("bombers must keep advancing through the pre-target phases", async () => {
        ...
        const along = (point) => (point.cx - corridor.center.cx) * corridorAxis.x + (point.cy - corridor.center.cy) * corridorAxis.y;
        ...
        const bomberPhaseProgress = phases
            ...
            .map((phase) => ({
            phase: phase.label,
            maxForwardProgressPx: Math.max(...phase.assignments
                .filter((assignment) => assignment.role === "bomber")
                .flatMap((assignment) => assignment.sampledPositions.map((sample) => along(sample) * progressSign)))
        }));
        ...
        for (const entry of bomberPhaseProgress) {
            if (entry.maxForwardProgressPx < furthestProgressPx - PRE_TARGET_DISTANCE_TOLERANCE_PX) {
                violations.push(`strike-group: REGRESSED in axial progress from ${furthestProgressPx.toFixed(1)}px to ${entry.maxForwardProgressPx.toFixed(1)}px ` +
