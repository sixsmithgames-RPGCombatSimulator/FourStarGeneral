            ...
            for (const phase of ingressPhases) {
                ...
                if (fighterAssignments.length > 0) {
                    ...
                    if (phase.label === "fighter-ingress") {
                        ...
                        const minimumFighterSpeed = phase.label === "bomber-ingress"
                        ...
                        const bomberAudit = phaseAudit?.roles.find((role) => role.role === "bomber" && role.assignmentCount > 0);
                        const bomberReferenceSpeed = bomberAudit?.realizedSpeedPxPerMs ?? null;
                        if (meanFighterSpeed < minimumFighterSpeed
                        ...
                        else if (phase.label === "bomber-ingress"
                            && bomberReferenceSpeed !== null
                            && meanFighterSpeed <= bomberReferenceSpeed * 1.12) {
                            violations.push(`${inspection.missionId}/${phase.label}: bomber-ingress fighter cover collapsed too close to bomber speed ` +
                                `(fighter=${meanFighterSpeed.toFixed(3)} px/ms bomber=${bomberReferenceSpeed.toFixed(3)} px/ms)`);
                        }
                    }
                }
...
registerTest("AIR_SHOW_CAP_COMBAT_PHASES_STAY_IN_PURPOSEFUL_MOTION", async ({ Given, When, Then }) => {
    ...
    await Then("CAP should keep moving through combat phases without falling into slow loiter outside compact merge work", async () => {
        const inspection = getAuthoritativeContestedInspection(result)
            ?? result?.airshowInspections.find((entry) => entry.eventType === "airToAir" &&
        ...
        const violations = speeds.flatMap(({ phase, speed }) => {
            ...
            const minimumSpeed = AIR_SHOW_FIGHTER_SPEED_PX_PER_MS * (isCompactMergePhase ? 0.25 : 0.55);
            const maximumSpeed = AIR_SHOW_FIGHTER_SPEED_PX_PER_MS * 1.35;
            const failures = [];
            if (speed < minimumSpeed) {
                failures.push(`${phase}: CAP speed ${speed.toFixed(2)} px/ms below minimum ${minimumSpeed.toFixed(2)} px/ms`);
            }
            if (speed > maximumSpeed) {
                failures.push(`${phase}: CAP speed ${speed.toFixed(2)} px/ms above maximum ${maximumSpeed.toFixed(2)} px/ms`);
            }
            return failures;
        });
        if (violations.length > 0) {
