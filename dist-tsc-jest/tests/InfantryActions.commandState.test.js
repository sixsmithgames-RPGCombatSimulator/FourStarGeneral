...
function createEngine(playerUnits, botUnits = []) {
    const config = {
        ...
        botSide: side({ q: 4, r: 2 }, botUnits.map((unit) => ({ ...unit, preDeployed: true }))),
...
registerTest("RECON_BIKES_CAN_ASSAULT_BUT_CANNOT_DIG_IN", async ({ Then }) => {
    ...
    if (assault.finalExpectedDamage <= suppressive.finalExpectedDamage) {
        throw new Error(`Expected assault to materially improve recon bike damage. Saw suppressive=${suppressive.finalExpectedDamage}, assault=${assault.finalExpectedDamage}.`);
    }
...
registerTest("BOT_ATTACK_SUMMARY_INCLUDES_PLAYER_RETALIATION", async ({ Then }) => {
    ...
    const botAttack = engine.resolveBotAttack(attacker, { q: 0, r: 1 }, { q: 0, r: 0 }, "suppressive");
...
registerTest("ENTRENCHED_SENTRY_COVER_REDUCES_INCOMING_FIRE_BEFORE_COUNTERFIRE", async ({ Then }) => {
    const resolveExchange = (entrench) => {
        const defender = {
            type: "TestInfantry",
            hex: { q: 0, r: 0 },
            strength: 100,
        ...
        const exchange = engine.resolveBotAttack(attacker, attacker.hex, defender.hex, "assault");
...
registerTest("SENTRY_DEFENDERS_RETURN_FIRE_SIMULTANEOUSLY_DURING_BOT_ATTACKS", async ({ Then }) => {
    ...
    const botAttack = engine.resolveBotAttack(attacker, { q: 0, r: 1 }, { q: 0, r: 0 });
...
registerTest("TOWED_GUNS_CANNOT_RETALIATE_UNTIL_DEPLOYED", async ({ Then }) => {
    ...
    const towedBotAttack = botVsTowed.resolveBotAttack(botAttacker, botAttacker.hex, playerTowedGun.hex);
    ...
    const playerDeployedGun = {
        ...playerTowedGun,
        towState: "deployed"
    };
    const { engine: botVsDeployed } = createEngine([playerDeployedGun], [botAttacker]);
    const deployedBotAttack = botVsDeployed.resolveBotAttack(botAttacker, botAttacker.hex, playerDeployedGun.hex);
