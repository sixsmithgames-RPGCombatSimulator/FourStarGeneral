...
registerTest("CAMPAIGN_STATE_NOTIFIES", async ({ Given, When, Then }) => {
    ...
    await Then("listeners receive notifications", async () => {
        if (!calls.includes("scenarioLoaded"))
            throw new Error("scenarioLoaded not emitted");
        if (!calls.includes("engagementsUpdated"))
            throw new Error("engagementsUpdated not emitted");
        if (!calls.includes("headquartersStatusUpdated"))
            throw new Error("headquartersStatusUpdated not emitted");
    });
});
