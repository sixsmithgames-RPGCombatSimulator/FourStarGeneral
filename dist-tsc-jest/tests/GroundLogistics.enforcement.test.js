...
const routingTerrain = { plains, road };
const vehicleDef = {
    class: "vehicle",
...
function findPlayerUnit(engine, hex) {
...
function createDepotSeeder(hex = { q: 3, r: 0 }) {
    return {
        type: "TestVehicle",
        hex,
        strength: 10,
...
function createSupplyTruck(hex = { q: 1, r: 0 }) {
    return {
        type: "Supply_Truck",
        hex,
        strength: 10,
...
registerTest("CONVOYS_ROTATE_EQUAL_PRIORITY_DELIVERIES_ACROSS_TURNS", async ({ Given, When, Then }) => {
    ...
    await Then("both battalions should have received at least one convoy delivery instead of one being serviced twice first", async () => {
        ...
        if (!alphaServiced || !bravoServiced) {
            throw new Error(`Expected the convoy to rotate equal-priority service across both battalions by the second pass, saw alpha ammo=${alphaAfterSecond.ammo}, fuel=${alphaAfterSecond.fuel}; bravo ammo=${bravoAfterSecond.ammo}, fuel=${bravoAfterSecond.fuel}.`);
        }
    });
});
...
registerTest("CONVOY_PATHFINDER_RETAINS_THE_CHEAPEST_ROUTE_BREADCRUMBS", async ({ Then }) => {
    ...
    const plan = planner.findCheapestPathToAny({ q: 0, r: 0 }, [{ q: 2, r: 0 }], "wheel", new Set());
    if (!plan) {
