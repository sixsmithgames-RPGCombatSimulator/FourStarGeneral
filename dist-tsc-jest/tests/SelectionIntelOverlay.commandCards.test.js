...
registerTest("SELECTION_INTEL_OVERLAY_RENDERS_COMMAND_CARDS_AND_NOTES", async ({ Given, When, Then }) => {
  ...
  `;
    ...
    await Then("the overlay surfaces actionable command buttons and context notes", async () => {
        ...
        const actions = Array.from(root.querySelectorAll("[data-selection-action]"));
        if (actions.length !== 2) {
...
registerTest("SELECTION_INTEL_OVERLAY_SHOWS_FRACTIONAL_AMMO_AND_FUEL_PRECISELY", async ({ Given, When, Then }) => {
  ...
  `;
    ...
    await Then("the stat cards keep those fractional values instead of rounding them away", async () => {
        const stats = Array.from(document.querySelectorAll(".battle-intel-overlay__stat"));
        const values = new Map(stats.map((stat) => [
            stat.querySelector(".battle-intel-overlay__stat-label")?.textContent?.trim() ?? "",
            stat.querySelector(".battle-intel-overlay__stat-value")?.textContent?.trim() ?? ""
        ]));
...
registerTest("SELECTION_INTEL_OVERLAY_SHOWS_RANGE_AND_DETAILS_WITHOUT_REDUNDANT_BATTLE_SUMMARY", async ({ Given, When, Then }) => {
  ...
  `;
    ...
    await When("battle intel is expanded to inspect the unit details tab", async () => {
        ...
        container.querySelector("[data-selection-intel-tab='unit']")?.click();
    });
...
registerTest("SELECTION_INTEL_OVERLAY_RENDERS_STACK_UNIT_TABS_FOR_SHARED_HEX_CONTROL", async ({ Given, When, Then }) => {
  ...
  `;
    ...
    await Then("the overlay renders selector tabs so the commander can switch units", async () => {
        const root = document.getElementById("battleIntelOverlay");
        const unitTabs = Array.from(root?.querySelectorAll("[data-selection-action^='selectUnit:']") ?? []);
