 ...
 */
export async function loadTerrainTints(jsonPath) {
    try {
        const response = await fetch(jsonPath);
        if (!response.ok) {
            throw new Error(`Failed to load terrain tints: ${response.statusText}`);
        }
        const tintConfigs = await response.json();
 ...
 */
export function getTerrainTint(terrainType) {
    return {
 ...
 */
export function shouldUseTerrainResponse(effectType) {
