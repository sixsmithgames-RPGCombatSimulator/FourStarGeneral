import { CAMPAIGN_HEX_SCALE_KM } from "../core/campaignTypes";
...
export class CampaignMapRenderer {
     ...
     */
    static estimatePixelBounds(cols, rows) {
    ...
    estimatePixelBoundsWithMargin(cols, rows, margin) {
        const { x: maxX } = CoordinateSystem.axialToPixel(cols - 1, Math.floor((rows - 1) / 2));
        const { y: maxY } = CoordinateSystem.axialToPixel(0, rows - 1);
        return { width: maxX + margin, height: maxY + margin };
    }
     ...
     */
    derivePixelDimensions(scenario) {
     ...
     */
    resolveGridPadding(scenario, margin, density) {
    ...
    clearHighlight(hexKey, className) {
        const group = this.hexGroups.get(hexKey);
        if (!group) {
            return;
        }
    ...
    setTerrainOverlayVisible(visible) {
        if (!this.svgElement)
            return;
        const layer = this.svgElement.querySelector(`#${TERRAIN_OVERLAY_LAYER_ID}`);
        if (layer) {
            layer.style.display = visible ? "block" : "none";
        }
    }
    /** Updates the terrain overlay to reflect current waterHexes data. */
    refreshTerrainOverlay() {
        if (!this.svgElement || !this.scenario)
            return;
        const layer = this.svgElement.querySelector(`#${TERRAIN_OVERLAY_LAYER_ID}`);
        if (!layer)
            return;
        ...
        layer.querySelectorAll("polygon[data-hex-key]").forEach(polygon => {
     ...
     */
    renderHexGrid(layer, scenario, density) {
        const { cols, rows } = scenario.dimensions;
     ...
     */
    renderTerrainOverlay(layer, scenario) {
    ...
    renderForceGroups(layer, scenario) {
        scenario.tiles.forEach((instance) => {
            ...
            forces.slice(0, FORCE_POSITIONS.length).forEach((force, index) => {
                const spriteFaction = instance.factionControl === "Bot"
                    ? "Bot"
        ...
        scenario.fronts.forEach((front) => {
            ...
            front.hexKeys.forEach((hexKey) => {
                ...
                if (!center) {
                    ...
                    if (maybeOffset)
                        center = this.getHexCenter(maybeOffset);
                }
                if (center) {
