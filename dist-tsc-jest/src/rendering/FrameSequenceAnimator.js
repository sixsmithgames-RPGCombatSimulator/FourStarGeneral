import { COMBAT_ANIMATIONS, getSpriteSheetFrameDuration, getSpriteSheetFrameOpacity, loadSpriteSheetImage, resolveSpriteSheetSpecAsync, sliceSpriteSheet } from "./SpriteSheetAnimator";
let nextFrameSequencePlaybackId = 1;
function readCanvasFrameSource(canvasElement) {
...
class FrameSequenceAnimation {
    constructor() {
        ...
        this.xhtmlWrapper = document.createElementNS(XHTML_NS, "div");
    ...
    configure(animationType, spec, frames, svgParent, x, y, scale = 1) {
        this.release();
        if (frames.frameDataUrls.length < spec.frameCount) {
            throw new Error(`[FrameSequenceAnimator] ${animationType} resolved ${frames.frameDataUrls.length} cached frames, but playback requires ${spec.frameCount}.`);
        }
        if (frames.frameCanvases.length < spec.frameCount) {
            throw new Error(`[FrameSequenceAnimator] ${animationType} resolved ${frames.frameCanvases.length} cached frame surfaces, but playback requires ${spec.frameCount}.`);
        }
        ...
        if (!(svgParent instanceof SVGElement)) {
            const receivedType = svgParent?.constructor?.name ?? typeof svgParent;
            throw new Error(`[FrameSequenceAnimator] Combat effects must mount to an SVG element. ` +
    ...
    updateFrame(frameIndex) {
        ...
        if (!frameSource) {
            throw new Error(`[FrameSequenceAnimator] ${this.animationType} is missing cached frame ${frameIndex} of ${this.resolvedFrames.length}.`);
        }
        if (!frameSurface) {
            throw new Error(`[FrameSequenceAnimator] ${this.animationType} is missing cached frame surface ${frameIndex} of ${this.resolvedFrameCanvases.length}.`);
        }
        if (frameSource === this.spec.imagePath) {
            throw new Error(`[FrameSequenceAnimator] ${this.playbackId} resolved frame ${frameIndex} back to the full sprite sheet URL. ` +
    ...
    drawFrame(frameSurface, frameSource, frameIndex) {
    ...
    assertVisibleSourceNeverUsesSheetUrl() {
        ...
        if (visibleSource === this.spec.imagePath) {
            throw new Error(`[FrameSequenceAnimator] ${this.playbackId} attempted to render cached playback using the full sprite sheet URL ${this.spec.imagePath}.`);
        }
    }
    assertVisibleFrameState(frameIndex, frameSource) {
        if (!this.spec) {
            return;
        }
        const visibleNodeCount = this.foreignObject.querySelectorAll("canvas").length;
        if (visibleNodeCount !== 1) {
            throw new Error(`[FrameSequenceAnimator] ${this.playbackId} expected exactly one visible canvas node, found ${visibleNodeCount}.`);
        }
        const visibleSource = readCanvasFrameSource(this.canvasElement);
        if (visibleSource !== frameSource) {
            throw new Error(`[FrameSequenceAnimator] ${this.playbackId} expected visible frame source ${summarizeFrameSource(frameSource)}, ` +
                `received ${summarizeFrameSource(visibleSource)}.`);
        }
        if ((this.canvasElement.dataset.frameIndex ?? "") !== String(frameIndex)) {
            throw new Error(`[FrameSequenceAnimator] ${this.playbackId} expected visible frame index ${frameIndex}, ` +
                `received ${this.canvasElement.dataset.frameIndex ?? "<missing>"}.`);
        }
        if (visibleSource === this.spec.imagePath) {
            throw new Error(`[FrameSequenceAnimator] ${this.playbackId} reverted to full sprite sheet source ${this.spec.imagePath} during cached playback.`);
        }
        if (this.spec.frameCount > 1 &&
            this.lastRenderedFrameIndex >= 0 &&
            frameIndex !== this.lastRenderedFrameIndex &&
            frameSource === this.lastRenderedFrameSource) {
            throw new Error(`[FrameSequenceAnimator] ${this.playbackId} advanced from frame ${this.lastRenderedFrameIndex} to ${frameIndex} without changing the visible frame source.`);
        }
        console.log(`[FrameSequenceAnimator] Frame swap - animation: ${this.playbackId}, frame: ${frameIndex}, ` +
    ...
    assertLayoutInvariant() {
        if (this.resolvedFrames.length === 0 || !this.layoutSnapshot || !this.animationType) {
            return;
        }
        const currentSnapshot = this.captureLayoutSnapshot();
        const changedField = Object.entries(this.layoutSnapshot).find(([key, value]) => currentSnapshot[key] !== value);
        if (changedField) {
            throw new Error(`[FrameSequenceAnimator] ${this.animationType} mutated layout field ${changedField[0]} during frame-sequence playback. ` +
...
export class FrameSequenceAnimator {
    constructor(svgElement, dependencies = {}) {
        this.activeAnimations = new Set();
        this.animationPool = new Map();
        this.resolvedSpecCache = new Map();
        this.resolveSpecInternal = async (animationType) => {
            const spec = COMBAT_ANIMATIONS[animationType];
            return resolveSpriteSheetSpecAsync(spec);
        };
        this.resolveFramesInternal = async (spec) => {
    ...
    async playAnimation(animationType, x, y, scale = 1) {
    ...
    getAnimationPool(animationType) {
    ...
    async getResolvedSpec(animationType) {
        const cacheKey = String(animationType);
    ...
    async ensureResolvedFrames(spec) {
