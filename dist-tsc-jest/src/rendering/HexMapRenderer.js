...
export class HexMapRenderer {
    ...
    inspectResolvedAirCombatShow(scene) {
        ...
        const buildBandAssignments = (flights, label, options) => flights.flatMap((flight, index) => {
            ...
            const holdTarget = corridorPoint(options.alongPx +
                lane * (options.alongStepPx ?? 34) +
                (rand() - 0.5) * (options.jitterAlongPx ?? 28), options.lateralPx +
                lane * (options.lateralStepPx ?? 48) +
                (rand() - 0.5) * (options.jitterLateralPx ?? 24));
            return this.buildAirShowFlightAssignments(flight, this.buildAirShowCurvedPath(current, holdTarget, (lane >= 0 ? 1 : -1) * (options.arcPx ?? 76), (rand() - 0.5) * (options.driftPx ?? 34), this.resolveAirShowFlightHeadingDegrees(flight)), options.headingBlend ?? 0.26);
        });
        const phases = [];
        ...
        try {
            ...
            const inspBomberIngressBandAssignments = bomberFlights.flatMap((bomberFlight, bomberIndex) => {
                ...
                const holdTarget = corridorPoint(trailingAlongPx + laneIndex * (bandPlan.alongStepPx ?? 22) + (rand() - 0.5) * 12, trailingLateralPx);
                return this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowCurvedPath(current, holdTarget, (laneIndex >= 0 ? 1 : -1) * 52, (rand() - 0.5) * 22, this.resolveAirShowFlightHeadingDegrees(bomberFlight)), 0.18);
            });
            const ingressAssignments = [
                ...buildBandAssignments(interceptorFlights, "ingress:interceptors", this.resolveAirShowIngressBandPlan(scene.kind, "interceptor")),
                ...buildBandAssignments(escortFlights, "ingress:escorts", this.resolveAirShowIngressBandPlan(scene.kind, "escort")),
            ...
            if (escortExchanges.length > 0) {
                ...
                const uniqueEscortPairs = Array.from(new Map(rawEscortPairs.map((entry) => [
                    ...
                    entry
                ])).values()).map((entry, pairIndex, allPairs) => {
                    const rand = stageRandom(`escort:pair:${pairIndex}:${entry.escortFlight.spec.id}:${entry.interceptorFlight.spec.id}`);
                    return {
                        ...entry,
                        pairIndex,
                        focusPoint: corridorPoint(-4 + (rand() - 0.5) * 10, (pairIndex - (allPairs.length - 1) / 2) * 74 + (rand() - 0.5) * 18)
                    };
                });
                const pairFocusesByFlightId = new Map();
                ...
                if (uniqueEscortPairs.length > 0) {
                    ...
                    for (let beat = 0; beat < escortBeatCount; beat += 1) {
                        ...
                        activeInterceptorFlights.forEach((flight, index) => {
                            const current = this.averageAirShowPosition(flight.actors) ?? flight.anchor;
                            const focusPoint = this.averageAirShowPoints(pairFocusesByFlightId.get(flight.spec.id) ?? []) ??
            ...
            if (survivingBombers.length > 0) {
                const bomberIngressAssignments = [
                    ...survivingBombers.flatMap((flight, index) => {
                        ...
                        const ingressTarget = corridorPoint(-58 + laneIndex * 26 + (rand() - 0.5) * 10, laneIndex * 52 + (rand() - 0.5) * 12);
                        return this.buildAirShowFlightAssignments(flight, this.buildAirShowBomberRunPath(this.averageAirShowPosition(flight.actors) ?? flight.anchor, ingressTarget, {
                            lateralSign: this.resolveAirShowRouteSideSign(this.averageAirShowPosition(flight.actors) ?? flight.anchor, ingressTarget, this.resolveAirShowFlightHeadingDegrees(flight), rand() > 0.5 ? 1 : -1),
                ...
                if (bomberAttackEntries.length > 0) {
                    ...
                    const passStart = -36;
                    const passEnd = 100;
                    const phaseAssignments = survivingBombers.flatMap((bomberFlight, bomberIndex) => {
                        ...
                        return this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowBomberContinuationPath(bomberCurrent, corridorPoint(passEnd, bomberLateral), {
                    ...
                    activeAttackEntries.forEach((entry, attackIndex) => {
                        ...
                        const direction = this.resolveAirShowCorridorSideSign(interceptorCurrent, corridor, attackIndex % 2 === 0 ? -1 : 1);
                    ...
                    phaseAssignments.push(...buildBandAssignments(holdingInterceptors, `bomber-stack:other-interceptors:${passIndex}`, {
            ...
            if (postPassBombers.length > 0) {
                ...
                const bomberTargetRuns = postPassBombers.map((bomberFlight, index) => {
                    ...
                    const targetCenter = bomberTargetCentersById.get(bomberFlight.spec.id)
                        ...
                        ?? corridor.strike;
                    const bomberCurrent = this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor;
                    const laneIndex = postPassBombers.length <= 1 ? 0 : index - (postPassBombers.length - 1) / 2;
                ...
                const strikeRunAssignments = [
                    ...bomberTargetRuns.flatMap(({ bomberFlight, targetApproach, lateralSign }) => this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowBomberTargetRunPath(this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor, targetApproach, {
            ...
            if (survivingBombers.length > 0) {
                ...
                const bomberIngressAssignments = [
                    ...survivingBombers.flatMap((bomberFlight, bomberIndex) => {
                        ...
                        const laneIndex = survivingBombers.length <= 1 ? 0 : bomberIndex - (survivingBombers.length - 1) / 2;
                        const ingressTarget = corridorPoint(-58 + laneIndex * 26 + (rand() - 0.5) * 10, laneIndex * 52 + (rand() - 0.5) * 12);
                        return this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowBomberRunPath(this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor, ingressTarget, {
                            lateralSign: this.resolveAirShowRouteSideSign(this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor, ingressTarget, this.resolveAirShowFlightHeadingDegrees(bomberFlight), rand() > 0.5 ? 1 : -1),
                            corridorWidthPx: 20 + rand() * 6,
                            driftPx: 28 + rand() * 12,
                            startHeadingDegrees: this.resolveAirShowFlightHeadingDegrees(bomberFlight)
                        }), 0.22);
    ...
    buildAirShowBomberInterceptPassPath(start, corridor, options) {
        ...
        const approachPoint = pointOnCorridor(options.passStartAlongPx - 112, laneIndex * 22 + attackSideSign * 92);
        const alignmentPoint = pointOnCorridor(options.passStartAlongPx - 40, laneIndex * 12 + attackSideSign * 42);
        const firingPoint = pointOnCorridor(options.passStartAlongPx + 18, laneIndex * 6 + attackSideSign * 12);
        const crossingPoint = pointOnCorridor(options.passEndAlongPx - 12, -laneIndex * 8 - attackSideSign * 10);
        const extendPoint = pointOnCorridor(options.passEndAlongPx + 54, -laneIndex * 14 - attackSideSign * 42);
        const exitPoint = pointOnCorridor(options.passEndAlongPx + 136, -laneIndex * 24 - attackSideSign * 92);
        ...
        return this.pruneAirShowEarlyTurnWaypoints(this.pruneAirShowTurnWaypoint(path, headingLeadIndex, 52), {
            maxTurnDeg: 52,
            strongTurnDeg: 116,
            maxFirstSegmentPx: 60,
            maxWaypointsToRemove: 2
    ...
    sampleAirShowAssignmentAtProgress(assignment, progress) {
        const sample = this.sampleAircraftWaypointPath(assignment.points, this.clamp(progress + (assignment.progressOffset ?? 0), 0, 1));
    ...
    applyInspectionAirShowAssignments(assignments) {
        assignments.forEach((assignment) => {
            ...
            if (assignment.points.length >= 2) {
                ...
                assignment.actor.headingDegrees = this.resolveAircraftHeadingDegrees(finalPoint.cx - previousPoint.cx, finalPoint.cy - previousPoint.cy);
            }
        ...
        await new Promise((resolve) => {
            ...
            const step = (now) => {
                ...
                assignments.forEach((assignment, assignmentIndex) => {
                    const sample = this.sampleAircraftWaypointPath(assignment.points, this.clamp(easedProgress + (assignment.progressOffset ?? assignmentIndex * 0.003), 0, 1));
                    assignment.actor.headingDegrees = this.interpolateAircraftHeadingDegrees(assignment.actor.headingDegrees, this.resolveAircraftHeadingDegrees(sample.derivative.dx, sample.derivative.dy, assignment.actor.headingDegrees), assignment.headingBlend ?? 0.34);
                    assignment.actor.position = sample.point;
                    this.positionAircraftImageGhost(assignment.actor.image, assignment.actor.size, sample.point.cx, sample.point.cy, assignment.actor.headingDegrees);
                });
                ...
                if (rawProgress >= 1) {
                    assignments.forEach((assignment) => {
                        ...
                        assignment.actor.headingDegrees = this.resolveAircraftHeadingDegrees(finalPoint.cx - previousPoint.cx, finalPoint.cy - previousPoint.cy, assignment.actor.headingDegrees);
                        assignment.actor.position = {
                            cx: finalPoint.cx,
                            cy: finalPoint.cy
                        };
                        this.positionAircraftImageGhost(assignment.actor.image, assignment.actor.size, finalPoint.cx, finalPoint.cy, assignment.actor.headingDegrees);
                    });
