import { normalizeFacingDirection } from "../core/types";
...
const UNKNOWN_CONTACT_SPRITE = `data:image/svg+xml;utf8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
...
export class HexMapRenderer {
    constructor() {
        this.hexElementMap = new Map();
        this.hexPolygonMap = new Map();
        this.hexLabelMap = new Map();
        this.hexUnitImageMap = new Map();
        this.hexUnitFacingAngleMap = new Map();
        this.baseCampMarker = null;
        this.baseCampHexKey = null;
     ...
     */
    async animateAircraftPathByHex(fromKey, toKey, scenarioType, segmentMs = 350) {
        if (fromKey === toKey) {
            return;
        }
        const parseKey = (key) => {
        ...
        const path = hexLine(a, b);
        if (path.length <= 1) {
            await this.animateAircraftFlyover(fromKey, toKey, scenarioType, Math.max(200, segmentMs));
            return;
        }
        const toOffsetKey = (ax) => {
     ...
     */
    async playTargetMarker(hexKey, durationMs = 600) {
        if (!this.svgElement)
            return;
        ...
        await new Promise((resolve) => setTimeout(resolve, Math.max(100, durationMs)));
        group.remove();
    }
    syncQueuedTargetMarkers(markers) {
     ...
     */
    async animateAircraftFlyover(fromKey, toKey, scenarioType, durationMs = 2800, onProgress, endProgress = 1, strength, laneOffsetPx = 0, faction, role = "interceptor") {
        if (!this.svgElement) {
     ...
     * When strength is provided, renders as a formation with 1-4 sprites based on strength.
     */
    async animateAircraftArc(fromKey, toKey, scenarioType, durationMs = 2800, onProgress, endProgress = 1, strength, laneOffsetPx = 0, faction, role = "interceptor") {
        if (!this.svgElement) {
        ...
        await new Promise((resolve) => {
            ...
            const step = (now) => {
                ...
                const tangentX = 2 * (1 - progressedDistance) * (controlCenterX - startCenterX) +
                    2 * progressedDistance * (endCenterX - controlCenterX);
                const tangentY = 2 * (1 - progressedDistance) * (controlCenterY - startCenterY) +
     ...
     */
    async animateAirDogfightShowAt(hexKey, flights, durationMs = 3200) {
        if (!this.svgElement) {
            return;
        }
        const cell = this.hexElementMap.get(hexKey);
        ...
        const combatFlights = flights.filter((flight) => flight.team === "interceptor" || flight.team === "escort");
        if (combatFlights.length === 0) {
            return;
        }
        const spriteStates = [];
        const buildDogfightPath = (team, flightIndex, spriteIndex, laneOffsetPx, biasX, biasY) => {
        ...
        combatFlights.forEach((flight, flightIndex) => {
            ...
            const ghosts = this.createAircraftSpriteGhosts(spriteHref, HexMapRenderer.AIRCRAFT_GHOST_ICON_SIZE, flight.strength, flight.team);
            ghosts.forEach((ghostSpec, spriteIndex) => {
                layer.appendChild(ghostSpec.image);
                const path = buildDogfightPath(flight.team, flightIndex, spriteIndex, flight.laneOffsetPx ?? 0, ghostSpec.biasX, ghostSpec.biasY);
            ...
            await new Promise((resolve) => {
                ...
                const step = (now) => {
                    ...
                    spriteStates.forEach((sprite, spriteIndex) => {
                        ...
                        sprite.lastHeadingDegrees = this.interpolateAircraftHeadingDegrees(sprite.lastHeadingDegrees, this.resolveAircraftHeadingDegrees(sample.derivative.dx, sample.derivative.dy, sprite.lastHeadingDegrees), 0.34);
                    ...
                    while (nextTracerWindow < tracerWindows.length &&
    ...
    async animateBomberInterceptionShowAt(hexKey, bomber, interceptors, durationMs = 3600) {
        ...
        const bomberSprites = this.createAircraftSpriteGhosts(bomberHref, HexMapRenderer.AIRCRAFT_GHOST_ICON_SIZE, bomber.strength, "bomber").map((sprite, index) => {
            ...
            await new Promise((resolve) => {
                ...
                const step = (now) => {
                    ...
                    bomberSprites.forEach((sprite, index) => {
                        ...
                        sprite.lastHeadingDegrees = this.interpolateAircraftHeadingDegrees(sprite.lastHeadingDegrees, this.resolveAircraftHeadingDegrees(sample.derivative.dx, sample.derivative.dy, sprite.lastHeadingDegrees), 0.22);
                        sprite.position = sample.point;
                        this.positionAircraftImageGhost(sprite.image, sprite.size, sample.point.cx, sample.point.cy, sprite.lastHeadingDegrees);
                    });
                    interceptorSprites.forEach((sprite, index) => {
                        ...
                        sprite.lastHeadingDegrees = this.interpolateAircraftHeadingDegrees(sprite.lastHeadingDegrees, this.resolveAircraftHeadingDegrees(sample.derivative.dx, sample.derivative.dy, sprite.lastHeadingDegrees), 0.38);
                        sprite.position = sample.point;
                        this.positionAircraftImageGhost(sprite.image, sprite.size, sample.point.cx, sample.point.cy, sprite.lastHeadingDegrees);
                    });
                    while (nextTracerWindow < tracerWindows.length &&
    ...
    inspectResolvedAirCombatShow(scene) {
        ...
        const sceneSeed = this.seedFromHexKey(`${scene.hexKey}:airshow:${scene.interceptors.length}:${scene.escorts.length}:${bomberSpecs.map((spec) => spec.id).join(",") || "none"}`);
        const stageRandom = (label) => this.seededRandom(this.seedFromHexKey(`${sceneSeed}:${label}`));
        const bomberTargetCentersById = new Map(bomberSpecs
            .map((spec) => [spec.id, this.resolveAirShowBomberTargetCenter(spec, scene)])
        ...
        const phases = [];
        const recordPhase = (label, assignments, durationMs, tracerBursts = [], flakBursts = []) => {
            ...
            phases.push({
                ...
                tracers: tracerBursts.flatMap((burst) => {
                    ...
                    const targetAssignment = "image" in burst.target
                        ? assignmentsByActorId.get(burst.target.id)
                        : undefined;
                    const sampledTargetPoint = "image" in burst.target
                        ? (targetAssignment
                    ...
                    return [{
                            ...
                            targetActorId: "image" in burst.target ? burst.target.id : undefined,
                ...
                flakBursts: flakBursts.map((burst) => {
                    const targetHexCenter = burst.targetHexKey ? this.resolveHexCenterByKey(burst.targetHexKey) : null;
                    const bomberTargetCenter = burst.bomberUnitKey ? bomberTargetCentersById.get(burst.bomberUnitKey) ?? null : null;
                    const targetSource = targetHexCenter
                        ? "targetHex"
                        : bomberTargetCenter
                            ? "bomberTarget"
                            : averageBomberTargetCenter
                                ? "averageBomberTarget"
                                : "corridorStrike";
                    const scopedTargetCenter = targetHexCenter
                        ?? bomberTargetCenter
                        ?? averageBomberTargetCenter
                        ?? corridor.strike;
                    const wave = this.resolveAirShowFlakBurstWave(corridor, scopedTargetCenter, burst);
        ...
        try {
            ...
            const inspBomberIngressBandAssignments = bomberFlights.flatMap((bomberFlight, bomberIndex) => {
                ...
                const holdTarget = this.resolveAirShowBomberIngressBandHoldTarget(corridor, current, scene.kind, laneIndex, bomberFlights.length, (rand() - 0.5) * 12, (rand() - 0.5) * 14);
                return this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowCurvedPath(current, holdTarget, (laneIndex >= 0 ? 1 : -1) * 52, (rand() - 0.5) * 22, this.resolveAirShowFlightHeadingDegrees(bomberFlight)), 0.18, bomberIndex, bomberFlights.length);
            });
            const hasFighterIngressParticipants = interceptorFlights.length > 0 || escortFlights.length > 0;
            const ingressAssignments = hasFighterIngressParticipants
                ? [
            ...
            if (ingressAssignments.length > 0) {
                ...
                // and bombers (with ~half-length chords) run at V/2 in the same phase window.
                const fighterIngressDurationMs = this.resolveAirShowPhaseDurationFromRoleSpeeds(ingressAssignments, ingressRoleSpeeds, scene.fighterIngressDurationMs ?? 2520, 1875, 10500, ["interceptor"]);
                const spacedIngressAssignments = this.finalizeAirShowPhaseAssignments(ingressAssignments, fighterIngressDurationMs, [0.18, 0.42, 0.68, 0.9], undefined, ingressRoleSpeeds);
            ...
            if (escortExchanges.length > 0) {
                ...
                if (uniqueEscortPairs.length > 0) {
                    ...
                    for (let beat = 0; beat < escortBeatCount; beat += 1) {
                        const defaultEscortBeatDurationMs = beat === 0
                        ...
                        const useCloseScrambleTracerProfile = scene.kind === "airToAir"
                        ...
                        uniqueEscortPairs.forEach((pair) => {
                            ...
                            tracerBursts.push(...this.buildAirShowDynamicTracerVolley(phaseAssignments, pair.interceptorFlight, pair.escortFlight, {
                                emitter: "nose",
                                color: "#fff5cf",
                                width: 0.14,
                                ...
                                timings: baseTimings
                            }), ...this.buildAirShowDynamicTracerVolley(phaseAssignments, pair.escortFlight, pair.interceptorFlight, {
                                emitter: "nose",
                                color: "#ffd98a",
                                width: 0.14,
                                lifetimeMs: beat === 0 ? 24 : 22,
                        ...
                        if (activeBomberFlights.length > 0) {
                            phaseAssignments.push(...activeBomberFlights.flatMap((bomberFlight, bomberIndex) => {
                                const rand = stageRandom(`escort:bomber-transit:${beat}:${bomberFlight.spec.id}`);
                                const laneIndex = activeBomberFlights.length <= 1 ? 0 : bomberIndex - (activeBomberFlights.length - 1) / 2;
                                const bomberCurrent = this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor;
                                const transitTarget = this.resolveAirShowBomberClashTransitTarget(corridor, bomberCurrent, beat, laneIndex);
                                return this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowBomberTransitPath(bomberCurrent, transitTarget, {
                                    lateralSign: this.resolveAirShowRouteSideSign(bomberCurrent, transitTarget, this.resolveAirShowFlightHeadingDegrees(bomberFlight), rand() > 0.5 ? 1 : -1),
                                    corridorWidthPx: 14 + rand() * 4,
                                    driftPx: 10 + rand() * 5,
                                    startHeadingDegrees: this.resolveAirShowFlightHeadingDegrees(bomberFlight)
                                }), 0.22, bomberIndex, activeBomberFlights.length);
                            }));
                        }
                        ...
                        const escortBeatDurationMs = this.resolveAirShowPhaseDurationFromRoleSpeeds(phaseAssignments, escortClashRoleSpeeds, defaultEscortBeatDurationMs, beat === 0 ? 900 : 760, 5000, ["interceptor", "escort"]);
                        ...
                        recordPhase(beat === 0 ? "escort-clash-merge" : "escort-clash-scramble", resolvedPhaseAssignments, escortBeatDurationMs, tracerBursts);
                        updateFlightAnchors([...interceptorFlights, ...escortFlights, ...bomberFlights]);
                    }
                    ...
                    escortFlights.forEach((flight) => this.syncAirShowFlightStrengthForInspection(flight, Math.max(0, flight.spec.strengthAfterEscortPhase ?? flight.currentStrength)));
                    updateFlightAnchors([...interceptorFlights, ...escortFlights, ...bomberFlights]);
                }
            }
            ...
            const bomberApproachProfilesById = new Map(survivingBombers.map((bomberFlight, index) => {
                const rand = stageRandom(`bomber-defense:approach:${bomberFlight.spec.id}`);
                ...
                const targetCenter = bomberTargetCentersById.get(bomberFlight.spec.id)
                    ?? averageBomberTargetCenter
                    ?? corridor.strike;
                const targetApproach = this.offsetAirShowPoint(targetCenter, -corridor.axis.x * 14 + corridor.normal.x * (laneIndex * 16 + (rand() - 0.5) * 8), -corridor.axis.y * 14 + corridor.normal.y * (laneIndex * 16 + (rand() - 0.5) * 8));
                return [bomberFlight.spec.id, { targetCenter, targetApproach, laneIndex }];
            }));
            ...
            if (survivingBombers.length === 0
                ...
                const bomberGapAssignments = [
                    ...survivingInterceptors.flatMap((flight, index) => this.buildAirShowFlightAssignments(flight, this.buildAirShowScreenRunPath(this.averageAirShowPosition(flight.actors) ?? flight.anchor, corridor, {
                        endAlongPx: -96,
                        baseLateralPx: -118,
                    ...
                    ...survivingEscorts.flatMap((flight, index) => this.buildAirShowFlightAssignments(flight, this.buildAirShowScreenRunPath(this.averageAirShowPosition(flight.actors) ?? flight.anchor, corridor, {
                        endAlongPx: -18,
                        baseLateralPx: 132,
                ...
                recordPhase("bomber-gap", bomberGapAssignments, Math.max(180, scene.bomberArrivalDelayMs ?? 0));
                updateFlightAnchors([...survivingInterceptors, ...survivingEscorts]);
            }
            if (survivingBombers.length > 0) {
                const bomberIngressAssignments = [
                    ...survivingBombers.flatMap((flight, index) => {
                        const rand = stageRandom(`ingress:bomber:${flight.spec.id}`);
                        const laneIndex = survivingBombers.length <= 1 ? 0 : index - (survivingBombers.length - 1) / 2;
                        const current = this.averageAirShowPosition(flight.actors) ?? flight.anchor;
                        const approachProfile = bomberApproachProfilesById.get(flight.spec.id);
                        const ingressTarget = this.resolveAirShowBomberIngressContinuationTarget(corridor, current, approachProfile?.targetApproach ?? corridor.strike, laneIndex);
                        return this.buildAirShowFlightAssignments(flight, this.buildAirShowBomberContinuationPath(current, ingressTarget, {
                            lateralSign: this.resolveAirShowRouteSideSign(current, ingressTarget, this.resolveAirShowFlightHeadingDegrees(flight), rand() > 0.5 ? 1 : -1),
                            corridorWidthPx: 18 + rand() * 4,
                            driftPx: 12 + rand() * 6,
                            startHeadingDegrees: this.resolveAirShowFlightHeadingDegrees(flight)
                        }), 0.22, index, survivingBombers.length);
                    })
                ];
                const bomberIngressRoleSpeeds = this.resolveAirShowRoleSpeedMap({
                    bomber: HexMapRenderer.AIR_SHOW_BOMBER_SPEED_PX_PER_MS
                });
                const bomberIngressDurationMs = this.resolveAirShowPhaseDurationFromRoleSpeeds(bomberIngressAssignments, bomberIngressRoleSpeeds, scene.bomberIngressDurationMs ?? 3500, 3000, 7000, ["bomber"]);
                const spacedBomberIngressAssignments = this.finalizeAirShowPhaseAssignments(bomberIngressAssignments, bomberIngressDurationMs, [0.22, 0.5, 0.78, 0.94], undefined, bomberIngressRoleSpeeds);
                recordPhase("bomber-ingress", spacedBomberIngressAssignments, bomberIngressDurationMs);
                ...
                if (bomberAttackEntries.length > 0 || survivingInterceptors.length > 0) {
                    const defaultBomberPassBeatDurationMs = Math.max(760, Math.round(scene.bomberPassDurationMs ?? 2360));
                    const bomberDefenseTargets = survivingBombers.map((bomberFlight, bomberIndex) => {
                        ...
                        const bomberCurrent = this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor;
                        const approachProfile = bomberApproachProfilesById.get(bomberFlight.spec.id);
                        const defenseTarget = this.resolveAirShowBomberDefenseContinuationTarget(corridor, bomberCurrent, approachProfile?.targetApproach ?? corridor.strike, laneIndex);
                        return {
                            bomberFlight,
                            laneIndex,
                            defenseTarget
                    ...
                    const phaseAssignments = bomberDefenseTargets.flatMap(({ bomberFlight, laneIndex, defenseTarget }, bomberIndex) => {
                        const bomberLateral = laneIndex * 22 + (bomberIndex % 2 === 0 ? -6 : 6);
                        const bomberCurrent = this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor;
                        return this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowBomberContinuationPath(bomberCurrent, defenseTarget, {
                            lateralSign: bomberLateral >= 0 ? 1 : -1,
                            corridorWidthPx: 16,
                            driftPx: 10,
                            startHeadingDegrees: this.resolveAirShowFlightHeadingDegrees(bomberFlight)
                        }), 0.22, bomberIndex, bomberDefenseTargets.length);
                    });
                    const tracerBursts = [];
                    const activeAttackEntries = bomberAttackEntries.filter((entry) => entry.interceptorFlight.actors.some((actor) => actor.active)
                    ...
                    attackEntriesForShow.forEach((entry, attackIndex) => {
                        ...
                        const attackCorridor = this.resolveAirShowCorridor(center, bomberCurrent, bomberApproachProfile?.targetCenter ?? corridor.strike);
                        const attackPassEnd = Math.round(this.resolveAirShowCorridorCoordinates(attackCorridor, bomberDefenseTargets.find((candidate) => candidate.bomberFlight.spec.id === entry.bomberFlight.spec.id)?.defenseTarget
                            ?? bomberApproachProfile?.targetApproach
                            ?? corridor.strike).alongPx);
                        const attackPassStart = attackPassEnd - 116;
                        const direction = this.resolveAirShowCorridorSideSign(interceptorCurrent, attackCorridor, attackIndex % 2 === 0 ? -1 : 1);
                        ...
                        phaseAssignments.push(...this.buildAirShowFlightAssignments(entry.interceptorFlight, interceptorPath, 0.3, attackIndex, attackEntriesForShow.length));
                        tracerBursts.push(...this.buildAirShowDynamicTracerVolley(phaseAssignments, entry.interceptorFlight, entry.bomberFlight, {
                            emitter: "nose",
                            width: 0.18,
                            lifetimeMs: 30,
                            spreadPx: 0,
                        ...
                        if (entry.retaliationDamage > 0) {
                            tracerBursts.push(...this.buildAirShowDynamicTracerVolley(phaseAssignments, entry.bomberFlight, entry.interceptorFlight, {
                                emitter: "center",
                                color: "#fff1c8",
                                width: 0.17,
                                lifetimeMs: 28,
                    ...
                    if (tracerBursts.length === 0) {
                        ...
                        if (fallbackAttackerFlight && fallbackBomberFlight) {
                            tracerBursts.push(...this.buildAirShowDynamicTracerVolley(phaseAssignments, fallbackAttackerFlight, fallbackBomberFlight, {
                                emitter: "nose",
                                width: 0.18,
                                lifetimeMs: 30,
                                spreadPx: 0,
                                ...
                                fallbackToNearest: true
                            }), ...this.buildAirShowDynamicTracerVolley(phaseAssignments, fallbackBomberFlight, fallbackAttackerFlight, {
                                emitter: "center",
                                color: "#fff1c8",
                                width: 0.17,
                                lifetimeMs: 28,
                            ...
                            tracerBursts.push(...this.buildAirShowTracerVolley(fallbackInterceptor, fallbackBomber, {
                                emitter: "nose",
                                width: 0.18,
                                lifetimeMs: 30,
                                spreadPx: 0,
                    ...
                    const bomberPassBeatDurationMs = this.resolveAirShowPhaseDurationFromRoleSpeeds(phaseAssignments, bomberDefenseRoleSpeeds, defaultBomberPassBeatDurationMs, 900, 5000, ["bomber"]);
                    ...
                    survivingBombers.forEach((flight) => this.syncAirShowFlightStrengthForInspection(flight, Math.max(0, flight.spec.finalStrength ?? flight.currentStrength)));
                    interceptorFlights.forEach((flight) => this.syncAirShowFlightStrengthForInspection(flight, Math.max(0, flight.spec.finalStrength ?? flight.currentStrength)));
            ...
            if (postPassBombers.length > 0) {
                ...
                const bomberTargetRuns = postPassBombers.map((bomberFlight, index) => {
                    ...
                    const targetApproach = cachedProfile?.targetApproach
                        ?? this.offsetAirShowPoint(targetCenter, -corridor.axis.x * 14 + corridor.normal.x * laneIndex * 16, -corridor.axis.y * 14 + corridor.normal.y * laneIndex * 16);
                    const targetExit = this.resolveAirShowBomberTargetRunExitPoint(corridor, this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor, targetCenter, targetApproach, laneIndex);
                    return {
                        bomberFlight,
                        targetCenter,
                        targetApproach,
                        targetExit,
                        laneIndex,
                        lateralSign: this.resolveAirShowRouteSideSign(this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor, targetExit, this.resolveAirShowFlightHeadingDegrees(bomberFlight), laneIndex >= 0 ? 1 : -1)
                    };
                });
                const strikeRunAssignments = [
                    ...bomberTargetRuns.flatMap(({ bomberFlight, targetExit, lateralSign }, bomberIndex) => this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowBomberTargetRunPath(this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor, targetExit, {
                        lateralSign: this.resolveAirShowRouteSideSign(this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor, targetExit, this.resolveAirShowFlightHeadingDegrees(bomberFlight), lateralSign),
                        corridorWidthPx: 10,
                        startHeadingDegrees: this.resolveAirShowFlightHeadingDegrees(bomberFlight)
                    ...
                    ...(keepEscortsOnTargetRun ? postPassEscorts.flatMap((flight, index) => {
                        ...
                        return this.buildAirShowFlightAssignments(flight, this.buildAirShowTargetRunEscortPath(this.averageAirShowPosition(flight.actors) ?? flight.anchor, assignedRun.targetApproach, corridor, {
                            ...
                            driftPx: 18,
                            startHeadingDegrees: this.resolveAirShowFlightHeadingDegrees(flight)
                        }), 0.24, index, postPassEscorts.length);
                    }) : []),
                    ...(keepInterceptorsOnTargetRun ? postPassInterceptors.flatMap((flight, index) => {
                        const assignedRun = bomberTargetRuns[index % bomberTargetRuns.length];
                        const attackCorridor = this.resolveAirShowCorridor(center, this.averageAirShowPosition(assignedRun.bomberFlight.actors) ?? assignedRun.bomberFlight.anchor, assignedRun.targetCenter);
                        return this.buildAirShowFlightAssignments(flight, this.buildAirShowBomberInterceptPassPath(this.averageAirShowPosition(flight.actors) ?? flight.anchor, attackCorridor, {
                            passStartAlongPx: 18,
                            passEndAlongPx: 96,
                            laneIndex: index - (postPassInterceptors.length - 1) / 2,
                            attackSideSign: this.resolveAirShowCorridorSideSign(this.averageAirShowPosition(flight.actors) ?? flight.anchor, attackCorridor, -1),
                            startHeadingDegrees: this.resolveAirShowFlightHeadingDegrees(flight)
                        }), 0.34, index, postPassInterceptors.length);
                    }) : [])
                ];
                ...
                const strikeRunFlakBursts = bomberTargetRuns.flatMap(({ bomberFlight }) => this.resolveAirShowBomberFlakBursts(scene, bomberFlight.spec.id));
    ...
    async animateResolvedAirCombatShow(scene) {
        ...
        const roles = [
            ...interceptorFlights.map(() => "interceptor"),
            ...escortFlights.map(() => "escort"),
            ...bomberFlights.map(() => "bomber")
        ];
        logAirShowPackageStart(packageId, "ResolvedAirCombat", "HexMapRenderer", allFlightIds, roles, scene.hexKey);
        ...
        const scheduleTimedFlakBursts = (targetCenter, durationMs, bursts) => {
            ...
            const handles = bursts.map((burst) => window.setTimeout(() => {
        ...
        const scheduleBombRelease = (durationMs, targetHexKey, progress) => {
            if (!targetHexKey) {
        ...
        try {
            ...
            const bomberIngressBandAssignments = bomberFlights.flatMap((bomberFlight, bomberIndex) => {
                const rand = stageRandom(`ingress:bomber-band:${bomberFlight.spec.id}`);
                const laneIndex = bomberFlights.length <= 1 ? 0 : bomberIndex - (bomberFlights.length - 1) / 2;
                const current = this.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor;
                const holdTarget = this.resolveAirShowBomberIngressBandHoldTarget(corridor, current, scene.kind, laneIndex, bomberFlights.length, (rand() - 0.5) * 12, (rand() - 0.5) * 14);
            ...
            if (escortExchanges.length > 0) {
                ...
                if (uniqueEscortPairs.length > 0) {
                    ...
                    await Promise.all([
                        ...interceptorFlights.map((flight) => this.syncAirShowFlightStrength(flight, Math.max(0, flight.spec.strengthAfterEscortPhase ?? flight.currentStrength), { x: -0.88, y: 0.68 })),
                        ...escortFlights.map((flight) => this.syncAirShowFlightStrength(flight, Math.max(0, flight.spec.strengthAfterEscortPhase ?? flight.currentStrength), { x: 0.88, y: -0.68 }))
                    ]);
                    updateFlightAnchors([...interceptorFlights, ...escortFlights, ...bomberFlights]);
                }
            }
                ...
                ], Math.max(520, Math.round((scene.escortClashDurationMs ?? 1500) * 0.55)), [], { easing: "linear", sceneActors });
                updateFlightAnchors([...interceptorFlights, ...escortFlights]);
            }
            const survivingInterceptors = activeFlights(interceptorFlights);
            const survivingEscorts = activeFlights(escortFlights);
            ...
            if (survivingBombers.length === 0
                ...
                logAirShowBeatStart(packageId, 2, "bomberGap", survivingFighters.map((f) => f.spec.id));
                ...
                ], Math.max(180, scene.bomberArrivalDelayMs ?? 0), [], { easing: "linear", sceneActors });
                updateFlightAnchors([...survivingInterceptors, ...survivingEscorts]);
            }
            if (survivingBombers.length > 0) {
                ...
                const bomberIngressAssignments = [
                    ...survivingBombers.flatMap((bomberFlight, bomberIndex) => {
                        ...
                        const approachProfile = bomberApproachProfilesById.get(bomberFlight.spec.id);
                        const ingressTarget = this.resolveAirShowBomberIngressContinuationTarget(corridor, current, approachProfile?.targetApproach ?? corridor.strike, laneIndex);
                        return this.buildAirShowFlightAssignments(bomberFlight, this.buildAirShowBomberContinuationPath(current, ingressTarget, {
                            lateralSign: this.resolveAirShowRouteSideSign(current, ingressTarget, this.resolveAirShowFlightHeadingDegrees(bomberFlight), rand() > 0.5 ? 1 : -1),
                            corridorWidthPx: 18 + rand() * 4,
                            driftPx: 12 + rand() * 6,
                            startHeadingDegrees: this.resolveAirShowFlightHeadingDegrees(bomberFlight)
                        }), 0.22, bomberIndex, survivingBombers.length);
                    })
                ];
                const bomberIngressRoleSpeeds = this.resolveAirShowRoleSpeedMap({
                    bomber: HexMapRenderer.AIR_SHOW_BOMBER_SPEED_PX_PER_MS
                ...
                if (bomberAttackEntries.length > 0) {
                    ...
                    activeAttackEntries.forEach((entry, attackIndex) => {
                        ...
                        phaseAssignments.push(...this.buildAirShowFlightAssignments(entry.interceptorFlight, interceptorPath, 0.3, attackIndex, activeAttackEntries.length));
                        tracerBursts.push(...this.buildAirShowDynamicTracerVolley(phaseAssignments, entry.interceptorFlight, entry.bomberFlight, {
                            emitter: "nose",
                            width: 0.18,
                            lifetimeMs: 30,
                    ...
                    updateFlightAnchors([...survivingBombers, ...interceptorFlights, ...escortFlights]);
                    await Promise.all([
                        ...survivingBombers.map((bomberFlight) => this.syncAirShowFlightStrength(bomberFlight, Math.max(0, bomberFlight.spec.finalStrength ?? bomberFlight.currentStrength), { x: 0.95, y: 0.2 })),
                        ...interceptorFlights.map((flight) => this.syncAirShowFlightStrength(flight, Math.max(0, flight.spec.finalStrength ?? flight.currentStrength), { x: -0.75, y: 0.8 }))
                    ]);
                    updateFlightAnchors([...survivingBombers, ...interceptorFlights]);
                }
            }
            ...
            if (postPassBombers.length > 0) {
                ...
                const cancelStrikeRunFlak = bomberTargetRuns.map(({ bomberFlight, targetCenter }) => {
                    ...
                    return scheduleTimedFlakBursts(targetCenter, strikeRunDurationMs, bomberBursts);
                });
            ...
            if (egressFlights.length > 0) {
                ...
                await Promise.all(egressFlights.flatMap((flight) => flight.actors.map((actor) => this.fadeOutActor(actor, 300))));
            }
            // Calculate final outcome
            const finalSurvivors = egressFlights.map(f => f.spec.id);
            const finalDestroyed = []; // Track destroyed if needed
     ...
     */
    async playLinkedStrikePackage(scene) {
        if (!this.svgElement) {
            return;
        }
        const layer = this.ensureCombatEffectsLayer();
        ...
        const roles = Array.from(new Set(Array.from(allParticipants.values()).map(f => f.role)));
        logAirShowPackageStart(packageId, "LinkedStrikePackage", "HexMapRenderer", missionIds, roles, scene.targetHexKey);
        ...
        const survivingIds = Array.from(allParticipants.values())
            ...
            .map(f => f.id);
        const destroyedIds = []; // None destroyed in stub
     ...
     */
    async animateAircraftRoundTrip(fromKey, toKey, scenarioType, legDurationMs = 2200, pauseMs = 300, strength, laneOffsetPx = 0, faction, role = "interceptor") {
     ...
     */
    async animateAircraftSortie(fromKey, targetKey, returnKey, scenarioType, options = {}) {
        if (!this.svgElement) {
        ...
        const ingressDirection = this.normalizeAircraftVector(shiftedTarget.cx - shiftedStart.cx, shiftedTarget.cy - shiftedStart.cy, 1, 0);
        const egressDirection = this.normalizeAircraftVector(shiftedReturn.cx - shiftedTarget.cx, shiftedReturn.cy - shiftedTarget.cy, -ingressDirection.x, -ingressDirection.y);
        ...
        const sharedTurnSpeed = Math.min(ingressDistance / Math.max(1, ingressDurationMs || 1), egressDistance / Math.max(1, egressDurationMs || 1)) * 0.92;
        ...
        let targetPassError = null;
        let targetPassPromise = null;
        const triggerTargetPass = (centerX, centerY) => {
            ...
            await new Promise((resolve) => {
                ...
                const step = (now) => {
                    ...
                    if (inIngress || egressDurationMs <= 0) {
                        ...
                        const point = this.interpolateAircraftHermitePoint(shiftedStart, shiftedTarget, ingressTangent, targetIngressTangent, ingressProgress);
                        const tangent = this.interpolateAircraftHermiteDerivative(shiftedStart, shiftedTarget, ingressTangent, targetIngressTangent, ingressProgress);
                        centerX = point.cx;
                        centerY = point.cy;
                        tangentX = tangent.dx;
                        tangentY = tangent.dy;
                        ...
                        const point = this.interpolateAircraftHermitePoint(shiftedTarget, shiftedReturn, targetEgressTangent, egressTangent, egressProgress);
                        const tangent = this.interpolateAircraftHermiteDerivative(shiftedTarget, shiftedReturn, targetEgressTangent, egressTangent, egressProgress);
                        centerX = point.cx;
                        centerY = point.cy;
                        tangentX = tangent.dx;
                        tangentY = tangent.dy;
    ...
    async playDogfight(hexKey) {
        ...
        const scheduleCleanup = (node, delayMs) => {
    ...
    async playBomberDefensePass(hexKey) {
        const effectsLayer = this.ensureCombatEffectsLayer();
        const hexElement = this.hexElementMap.get(hexKey);
        const center = hexElement ? this.extractHexCenter(hexElement) : null;
        if (!effectsLayer || !center) {
     ...
     */
    toggleSelectionGlow(shouldShow, hexKey) {
    ...
    async playArcedProjectile(attackerHexKey, defenderHexKey, durationMs = 520, options) {
        const layer = this.ensureCombatEffectsLayer();
    ...
    render(svg, canvas, data) {
        ...
        let viewportRoot = svg.querySelector("#viewportRoot");
      ...
      `;
            ...
            viewportRoot = svg.querySelector("#viewportRoot");
        ...
        let effectsLayer = viewportRoot.querySelector(".combat-effects-layer");
        if (!effectsLayer) {
        ...
        this.combatEffectsLayer = this.viewportRoot?.querySelector(".combat-effects-layer");
    ...
    ensureCombatAnimationOverlay() {
        if (!this.canvasElement) {
            return null;
        }
        let host = this.canvasElement.querySelector(".combat-animation-overlay-host");
        if (!host) {
        ...
        let overlay = host.querySelector(".combat-animation-overlay");
        if (!overlay) {
    ...
    ensureQueuedTargetMarkerLayer() {
        ...
        if (!layer || !layer.isConnected) {
            layer = viewportRoot.querySelector(".queued-target-marker-layer");
        }
        if (!layer) {
    ...
    buildCrosshairMarkerShape(color) {
        const group = document.createElementNS(SVG_NS, "g");
    ...
    buildParachuteMarkerShape(color) {
        const group = document.createElementNS(SVG_NS, "g");
    ...
    resolveQueuedTargetMarkerOffset(index, totalAtHex) {
        if (totalAtHex <= 1) {
            return { dx: 0, dy: 0 };
        }
        const offsets = [
    ...
    resolveViewportRootMatrix() {
        if (!this.viewportRoot) {
            return { a: 1, b: 0, c: 0, d: 1, e: 0, f: 0 };
        }
        const transformList = this.viewportRoot.transform;
    ...
    syncCombatAnimationOverlayLayout() {
    ...
    bindCombatAnimationOverlayTransformObserver() {
    ...
    cacheHexReferences() {
        ...
        const hexCells = Array.from(this.svgElement.querySelectorAll(".hex-cell"));
        hexCells.forEach((cell) => {
            ...
            const polygon = cell.querySelector("polygon.hex-tile");
            if (polygon) {
                this.hexPolygonMap.set(hexKey, polygon);
            }
            const unitGroup = cell.querySelector("g.unit-stack");
            if (unitGroup) {
    ...
    resolveStackDecorationAnchor(cx, cy, variant, stacked) {
        if (!stacked) {
            return { cx, cy };
        }
        return variant === "corners"
    ...
    positionUnitStack(group, cx, cy) {
        const images = Array.from(group.querySelectorAll("image.unit-icon"));
    ...
    resolveAngleDegFromVector(dx, dy) {
        if (dx === 0 && dy === 0) {
            return 0;
        }
    ...
    resolveHexEdgeGeometry(cx, cy, facing) {
    ...
    appendFortificationPanels(container, totalLength) {
        const panelCount = 3;
        const gap = 3;
        const panelHeight = 6;
    ...
    appendTankTrapPanels(container, totalLength) {
        const trapCount = 3;
        const gap = 6;
        const usableLength = Math.max(18, totalLength);
    ...
    ensureFacingGroup(group) {
        const existing = group.querySelector("g.unit-stack-facing");
        if (existing) {
            return existing;
        }
        const facingGroup = document.createElementNS(SVG_NS, "g");
        facingGroup.classList.add("unit-stack-facing");
        const images = Array.from(group.childNodes).filter((node) => {
    ...
    ensureDecorationGroup(group) {
        const existing = group.querySelector("g.unit-stack-decorations");
        if (existing) {
    ...
    renderUnitDecorations(group, cx, cy, unit) {
    ...
    renderEntrenchmentPips(cx, cy, entrenchment) {
        const group = document.createElementNS(SVG_NS, "g");
    ...
    renderStatusPip(x, y, status) {
        ...
        if (status === "sentry") {
            ...
            crosshair.setAttribute("d", `M ${x} ${y - 4.2} L ${x} ${y - 2.8} M ${x + 4.2} ${y} L ${x + 2.8} ${y} M ${x} ${y + 4.2} L ${x} ${y + 2.8} M ${x - 4.2} ${y} L ${x - 2.8} ${y}`);
    ...
    applyFacingAngleToGroup(group, cx, cy, angleDeg) {
        const formationGroups = Array.from(group.querySelectorAll(":scope > g.unit-stack-formation"));
    ...
    rebindHexInteractions() {
        ...
        this.boundDelegatedClickHandler = (event) => {
            if (!this.hexClickHandler)
                return;
            // Check if the click was on a sentry pip - if so, dispatch a custom event and stop propagation
            const target = event.target;
            const sentryPip = target.closest(".unit-status-pip[data-status='sentry'][data-clickable='true']");
            if (sentryPip) {
                // Find the hex this sentry pip belongs to
                const hexCell = sentryPip.closest(".hex-cell");
            ...
            const hexCell = target.closest(".hex-cell");
            if (!hexCell)
                return;
     ...
     */
    applyHexSelection(key, silent = false) {
    ...
    toggleHexSelectionClass(hexKey, enabled) {
            ...
            if (polygon) {
                ...
                if (points) {
                    ...
                    if (coordinates.length > 0) {
                        const total = coordinates.reduce((accum, [x, y]) => ({ cx: accum.cx + x, cy: accum.cy + y }), { cx: 0, cy: 0 });
     ...
     */
    setTacticalHighlights(moveKeys, attackKeys) {
     ...
     */
    toggleIdleUnitHighlight(hexKey, enabled) {
        if (enabled) {
    ...
    toggleHexHighlightClass(hexKey, className, enabled) {
        const group = this.hexElementMap.get(hexKey);
        const polygon = this.hexPolygonMap.get(hexKey);
        if (enabled) {
    ...
    renderDebugMarker(hexKey, options) {
        const cell = this.hexElementMap.get(hexKey);
        if (!cell) {
            return;
        }
     ...
     */
    trackHexReconStatus(key, status) {
    ...
    isSupplyTruckUnit(unit) {
        return unit.type === "Supply_Truck";
    }
    resolveRenderableStackMembers(members) {
     ...
     */
    renderUnitStack(hexKey, members) {
        const cell = this.hexElementMap.get(hexKey);
        ...
        const primaryReconStatus = typeof primaryMember.reconStatus === "boolean"
        ...
        try {
            const def = unitTypesData[primaryMember.unit.type];
            if (def?.class) {
        ...
        visibleMembers.forEach((member, index) => {
            const variant = index === 0 ? "diamond" : "corners";
            const reconStatus = typeof member.reconStatus === "boolean"
                ? (member.reconStatus ? "spotted" : "visible")
                : (member.reconStatus ?? "visible");
            const spriteHref = reconStatus === "spotted" ? UNKNOWN_CONTACT_SPRITE : getSpriteForScenarioType(member.unit.type, member.faction);
        ...
        if (storedAngle !== null && visibleMembers.length === 1) {
            ...
            this.hexUnitFacingAngleMap.set(hexKey, this.resolveFacingAngleDeg(this.normalizeFacing(primaryMember.unit.facing)));
        }
     ...
     */
    renderUnit(hexKey, unit, faction, reconStatus = "visible") {
    ...
    renderHexModifications(hexKey, modifications) {
    ...
    refreshHexModificationOverlay(hexKey) {
        const cell = this.hexElementMap.get(hexKey);
        if (!cell) {
            return;
        }
        ...
        const facings = modifications
            ...
            .filter((facing) => facing !== null && facing !== undefined);
        if (facings.length > 0) {
        ...
        switch (modification.type) {
            ...
            default: {
                ...
                if (tile && offset && this.scenarioData) {
                    group.innerHTML = this.roadRenderer.drawRoadOverlay(cx, cy, tile, offset.col, offset.row, this.scenarioData.tiles, this.scenarioData.tilePalette, {
                        ...
                        neighborHasRoad: ({ tile: neighborTile, col, row }) => (this.roadRenderer.hasRoad(neighborTile) ||
    ...
    resolveClearedPathStrokeWidth(level) {
    ...
    getClearPathLevelForHexKey(hexKey) {
     ...
     */
    renderHex(hex, minX, minY, margin, data) {
    ...
    renderTerrainFeatureOverlay(tile, cx, cy, clipId) {
    ...
    renderShallowCrossingOverlay(cx, cy, clipId) {
    ...
    renderFordOverlay(cx, cy, clipId) {
    ...
    renderRubbleBridgeOverlay(cx, cy, clipId) {
     ...
     */
    normalizeReconStatus(recon) {
        const normalized = recon.toLowerCase();
        const validStatuses = ["unknown", "spotted", "identified", "visible"];
        if (validStatuses.includes(normalized)) {
            return normalized;
        }
        return "unknown";
    }
    rehydrateAftermathOverlays() {
        this.aftermathByHexKey.forEach((_entry, hexKey) => {
            this.syncAftermathOverlay(hexKey);
        });
    }
    markHexWrecked(hexKey, unitClass, fireTurns = 2) {
    ...
    markHexDamaged(hexKey, unitClass, strengthAfter, turns = 2) {
        ...
        const smokeLevel = isCritical ? 2 : 1;
    ...
    createWreckFragment(group, x, y, width, height, rotationDeg, fill, opacity = 0.86) {
    ...
    createWreckWheel(group, x, y, radius, opacity = 0.72) {
    ...
    createScatterDebris(group, rand, cx, cy, count, spreadX, spreadY, minSize, maxSize) {
    ...
    createWreckShape(hexKey, wreckClass, scenarioType, cx, cy) {
        const g = document.createElementNS(SVG_NS, "g");
    ...
    createMoveGhost(spriteHref, width, height) {
     ...
     */
    resolveAircraftFormationLayout(strength, role = "interceptor") {
     ...
     */
    createAircraftFormationGhost(spriteHref, iconSize, strength, role = "interceptor") {
    ...
    createAircraftSpriteGhosts(spriteHref, iconSize, strength, role = "interceptor") {
        // Apply role-based size multiplier
        const sizeMultiplier = role === "bomber"
            ? HexMapRenderer.AIRCRAFT_BOMBER_SIZE_MULTIPLIER
            : HexMapRenderer.AIRCRAFT_FIGHTER_SIZE_MULTIPLIER;
        const scaledIconSize = iconSize * sizeMultiplier;
        const layout = strength === undefined || strength === null
            ? [{ ox: 0, oy: 0, scale: 1 }]
    ...
    normalizeAircraftVector(dx, dy, fallbackX = 1, fallbackY = 0) {
    ...
    resolveAircraftHeadingDegrees(dx, dy, fallbackDegrees = 0) {
    ...
    interpolateAircraftHeadingDegrees(currentDegrees, targetDegrees, blend = 1) {
    ...
    sampleAircraftWaypointPath(points, progress) {
        return sampleAirShowWaypointPath(points, progress);
    }
    positionAircraftImageGhost(ghost, size, centerX, centerY, headingDegrees = 0) {
    ...
    playAirTracerExchange(start, end, options = {}) {
        const effectsLayer = this.ensureCombatEffectsLayer();
        if (!effectsLayer) {
            return;
        }
        ...
        const step = (now) => {
            ...
            const rise = this.clamp(progress / 0.08, 0, 1);
            const decay = progress < 0.42 ? 1 : 1 - (progress - 0.42) / 0.58;
            ...
            glow.style.opacity = `${0.12 * opacity}`;
            tracer.style.opacity = `${0.96 * opacity}`;
            if (progress >= 1) {
                glow.remove();
                tracer.remove();
                return;
    ...
    resolveAirShowVisibleActorCount(strength) {
        if (strength <= 0) {
            return 0;
        }
    ...
    buildAirShowRuntimeFlight(layer, spec, fallbackOrigin, defaultHeadingDegrees) {
        ...
        const ghosts = this.createAircraftSpriteGhosts(spriteHref, HexMapRenderer.AIRCRAFT_GHOST_ICON_SIZE, spec.strengthBefore, spec.role);
        ...
        const actors = ghosts.map((ghostSpec, index) => {
            layer.appendChild(ghostSpec.image);
            ...
            const headingDegrees = defaultHeadingDegrees +
        ...
        const roleLabel = spec.role === "escort" && spec.combatRole === "cap"
            ? "opposition/cap"
    ...
    resolveAirShowSceneAnchor(center, role, index, total, sideBias = 1) {
    ...
    resetAirShowFlightToSceneAnchor(flight, anchor, headingTarget) {
        ...
        flight.actors.forEach((actor) => {
            const headingDegrees = baseHeadingDegrees
            ...
            this.positionAircraftImageGhost(actor.image, actor.size, actor.position.cx, actor.position.cy, headingDegrees);
        });
    ...
    normalizeAirShowSceneFlightAnchors(corridor, sceneKind, interceptorFlights, escortFlights, bomberFlights, hqAxis) {
        ...
        const positionFlights = (flights, role, fallbackAlongSign) => {
            const orderedFlights = flights
                ...
                flight,
                originalIndex,
                projection: this.resolveAirShowCorridorCoordinates(corridor, this.averageAirShowPosition(flight.actors) ?? flight.anchor)
            }))
                .sort((left, right) => {
    ...
    offsetAirShowPoint(point, dx, dy) {
        return {
            cx: point.cx + dx,
            cy: point.cy + dy
        };
     ...
     */
    resolveAirShowMinimumSpacing(spriteSizeA, spriteSizeB, roleA, roleB) {
     ...
     */
    resolveAirShowCollisionFreePositions(actors, targetPositions, maxIterations = 3) {
        ...
        for (let iteration = 0; iteration < maxIterations; iteration++) {
            ...
            for (let i = 0; i < activeActors.length; i++) {
                for (let j = i + 1; j < activeActors.length; j++) {
                    ...
                    const minSpacing = this.resolveAirShowMinimumSpacing(actorA.size, actorB.size, actorA.role, actorB.role);
     ...
     */
    resolveAirShowAltitudeLaneOffsets(flights, basePositions) {
     ...
     */
    resolveAirShowExpandedCombatEllipse(center, flights, baseRadiusPx) {
        const totalActors = flights.reduce((sum, f) => sum + f.actors.filter(a => a.active).length, 0);
    ...
    interpolateAirShowPoint(start, end, progress) {
    ...
    projectAirShowCorridorPoint(corridor, alongPx, lateralPx = 0) {
        return {
    ...
    resolveAirShowCorridorCoordinates(corridor, point) {
    ...
    fadeInActor(actor, durationMs = 400) {
        return new Promise((resolve) => {
    ...
    fadeOutActor(actor, durationMs = 300) {
        return new Promise((resolve) => {
    ...
    buildAirShowCurvedPath(start, end, arcPx = 0, driftPx = 0, startHeadingDegrees) {
    ...
    buildAirShowIngressStagingPath(start, stagePoint, nextFocusPoint, options = {}) {
        ...
        const bridgeExitHeadingDegrees = this.resolveAirShowPathHeadingDegrees([start, ...bridgePoints], options.startHeadingDegrees);
        ...
        return this.pruneAirShowEarlyTurnWaypoints(this.pruneAirShowSharpTurns([start, ...bridgePoints, commitPoint, settlePoint, stagePoint], 112, 2), {
            maxTurnDeg: 48,
            strongTurnDeg: 96,
    ...
    resolveAirShowWaypointTurnDegrees(previous, current, next) {
        return this.resolveAirShowVectorAngleDegrees({ x: current.cx - previous.cx, y: current.cy - previous.cy }, { x: next.cx - current.cx, y: next.cy - current.cy });
    }
    pruneAirShowTurnWaypoint(path, waypointIndex, maxTurnDeg) {
    ...
    pruneAirShowEarlyTurnWaypoints(path, options = {}) {
    ...
    profileAirShowPathTurns(path) {
        ...
        const firstTurnDeg = path.length >= 3
    ...
    pruneAirShowSharpTurns(path, maxTurnDeg, maxWaypointsToRemove = 2) {
        const prunedPath = [...path];
        let removedWaypoints = 0;
     ...
     */
    clampPointToViewportBounds(point, center, maxHorizontalPx = 600, maxVerticalPx = 400) {
    ...
    buildAirShowPursuitPath(start, target, options = {}) {
        ...
        const driftPx = options.driftPx ?? 0;
        const clampPoint = (point) => this.clampPointToViewportBounds(point, target);
        const entryPoint = clampPoint({
            cx: start.cx + fx * (length * 0.2) + nx * lateralSign * (entryLateralPx * 0.42) + fx * driftPx * 0.08,
            cy: start.cy + fy * (length * 0.2) + ny * lateralSign * (entryLateralPx * 0.42) + fy * driftPx * 0.08
        });
        const mergePoint = clampPoint({
            cx: start.cx + fx * (length * 0.38) + nx * lateralSign * mergeLateralPx + fx * driftPx * 0.2,
            cy: start.cy + fy * (length * 0.38) + ny * lateralSign * mergeLateralPx + fy * driftPx * 0.2
        });
        ...
        const overshootPoint = clampPoint({
            cx: target.cx +
                fx * (overshootPx * 0.28 + breakForwardPx * 0.08) +
                nx * lateralSign * Math.max(attackOffsetPx * 0.72, breakLateralPx * 0.18),
            cy: target.cy +
                fy * (overshootPx * 0.28 + breakForwardPx * 0.08) +
                ny * lateralSign * Math.max(attackOffsetPx * 0.72, breakLateralPx * 0.18)
        });
        const exitPoint = clampPoint({
            cx: target.cx + fx * (overshootPx * 0.72 + breakForwardPx * 0.62) + nx * lateralSign * (breakLateralPx * 0.56),
            cy: target.cy + fy * (overshootPx * 0.72 + breakForwardPx * 0.62) + ny * lateralSign * (breakLateralPx * 0.56)
        });
        const leadPoint = this.buildAirShowHeadingLeadPoint(start, entryPoint, {
            startHeadingDegrees: options.startHeadingDegrees,
            lateralSign,
            leadForwardPx: Math.min(Math.max(30, length * 0.12), Math.max(48, length * 0.18)),
        ...
        if (this.resolveAirShowVectorAngleDegrees(headingForward, routeForward) <= 30) {
            return authoredPath;
        }
        ...
        const commitLead = this.buildAirShowHeadingLeadPoint(commitPoint, firstPoint, {
            startHeadingDegrees: this.resolveAircraftHeadingDegrees(commitPoint.cx - start.cx, commitPoint.cy - start.cy, options.startHeadingDegrees),
            lateralSign,
        ...
        return this.pruneAirShowEarlyTurnWaypoints([start, commitPoint, commitLead, ...authoredPath.slice(1)].filter((point, index, points) => {
    ...
    buildAirShowBreakTurnPath(start, threat, options = {}) {
        ...
        const clampPoint = (point) => this.clampPointToViewportBounds(point, threat);
        const setupPoint = clampPoint({
            cx: start.cx + fx * (length * 0.18) + nx * lateralSign * (entryLateralPx * 0.44),
            cy: start.cy + fy * (length * 0.18) + ny * lateralSign * (entryLateralPx * 0.44)
        });
        const guardPoint = clampPoint({
            cx: threat.cx - fx * guardForwardPx + nx * lateralSign * (guardLateralPx * 0.54),
            cy: threat.cy - fy * guardForwardPx + ny * lateralSign * (guardLateralPx * 0.54)
        });
        ...
        const exitPoint = clampPoint({
            cx: threat.cx + fx * (exitForwardPx * 0.74) - nx * lateralSign * (exitLateralPx * 0.62),
            cy: threat.cy + fy * (exitForwardPx * 0.74) - ny * lateralSign * (exitLateralPx * 0.62)
        });
        const trailPoint = clampPoint({
            cx: threat.cx + fx * (exitForwardPx * 0.78 + trailForwardPx * 0.72) - nx * lateralSign * (exitLateralPx * 0.74),
            cy: threat.cy + fy * (exitForwardPx * 0.78 + trailForwardPx * 0.72) - ny * lateralSign * (exitLateralPx * 0.74)
        });
        const leadPoint = this.buildAirShowHeadingLeadPoint(start, setupPoint, {
            startHeadingDegrees: options.startHeadingDegrees,
            lateralSign,
            leadForwardPx: Math.min(Math.max(24, length * 0.1), Math.max(38, length * 0.16)),
            leadLateralPx: entryLateralPx * 0.14,
            clampCenter: threat
        });
        const authoredPath = [
        ...
        const routeNormal = { x: -routeForward.y, y: routeForward.x };
        const headingForward = this.resolveAirShowHeadingVector(options.startHeadingDegrees);
        if (this.resolveAirShowVectorAngleDegrees(headingForward, routeForward) <= 32) {
            return authoredPath;
        }
    ...
    buildAirShowMergePassPath(start, focus, corridor, options) {
        ...
        const bridgeExitHeadingDegrees = this.resolveAirShowPathHeadingDegrees([start, ...entryBridgePoints], options.startHeadingDegrees);
        ...
        return this.pruneAirShowEarlyTurnWaypoints(this.pruneAirShowSharpTurns(smoothedPath, 96, 2), {
            maxTurnDeg: 52,
            strongTurnDeg: 96,
    ...
    buildAirShowBomberTransitPath(start, end, options = {}) {
        return this.buildAirShowBomberContinuationPath(start, end, {
    ...
    buildAirShowBomberMonotonicPath(start, end, options = {}) {
        const dx = end.cx - start.cx;
        const dy = end.cy - start.cy;
        const length = Math.max(1, Math.hypot(dx, dy));
        const fx = dx / length;
        ...
        const pointAlongRoute = (ratio, lateralScale, forwardDriftScale) => clampPoint({
            cx: start.cx +
        ...
        const finalApproachBackPx = Math.min(Math.max(4, length * 0.06), Math.max(8, length * 0.1));
        ...
        const smoothedPath = this.pruneAirShowEarlyTurnWaypoints(this.pruneAirShowSharpTurns(enforcedPath, 92, 1), {
            maxTurnDeg: 42,
            strongTurnDeg: 96,
    ...
    buildAirShowBomberBreakawayPath(start, end, options = {}) {
        const dx = end.cx - start.cx;
        const dy = end.cy - start.cy;
        const length = Math.max(1, Math.hypot(dx, dy));
    ...
    buildAirShowBomberRunPath(start, end, options = {}) {
        const dx = end.cx - start.cx;
        const dy = end.cy - start.cy;
        const length = Math.max(1, Math.hypot(dx, dy));
        const fx = dx / length;
    ...
    buildAirShowBomberContinuationPath(start, end, options = {}) {
        return this.buildAirShowBomberMonotonicPath(start, end, {
            lateralSign: options.lateralSign,
            corridorWidthPx: options.corridorWidthPx ?? 12,
            driftPx: options.driftPx ?? 8,
    ...
    buildAirShowBomberTargetRunPath(start, end, options = {}) {
        return this.buildAirShowBomberMonotonicPath(start, end, {
            lateralSign: options.lateralSign,
            corridorWidthPx: options.corridorWidthPx ?? 6,
            driftPx: 2,
            earlyRatio: 0.28,
            midRatio: 0.58,
            lateRatio: 0.84,
            earlyLateralScale: 0.03,
            midLateralScale: 0.015,
            lateLateralScale: 0.008,
            finalLateralScale: 0.004
        });
    }
    buildAirShowDisengagePath(start, end, options = {}) {
        ...
        const breakawayHeading = this.resolveAirShowPathHeadingDegrees([start, ...entryBridgePoints, breakawayLead], options.startHeadingDegrees);
    ...
    enforceAirShowMonotonicRoutePath(path, start, end, options = {}) {
        if (path.length <= 2) {
            return [...path];
        }
        const dx = end.cx - start.cx;
        ...
        const nx = -fy;
        const ny = fx;
        const clampPoint = (point) => options.clampCenter
            ? this.clampPointToViewportBounds(point, options.clampCenter, options.maxHorizontalPx ?? 430, options.maxVerticalPx ?? 300)
            : point;
        ...
        interior.forEach((point, index) => {
            ...
            const maxAllowedAlongPx = Math.max(minAllowedAlongPx, length - minAlongStepPx * (remainingInterior + 1));
            const correctedAlongPx = this.clamp(deviationX * fx + deviationY * fy, minAllowedAlongPx, maxAllowedAlongPx);
            const correctedLateralPx = this.clamp(deviationX * nx + deviationY * ny, -maxLateralPx, maxLateralPx);
            ...
            if (Math.hypot(correctedPoint.cx - corrected[corrected.length - 1].cx, correctedPoint.cy - corrected[corrected.length - 1].cy) >= 2) {
    ...
    buildAirShowDogfightPassPath(start, focus, corridor, options) {
        const laneIndex = options.laneIndex ?? 0;
        const sideSign = options.sideSign >= 0 ? 1 : -1;
        ...
        const clampPoint = (point) => this.clampPointToViewportBounds(point, corridor.center, 430, 300);
        const focusPoint = clampPoint(focus);
        const pointFromFocus = (alongPx, lateralPx) => clampPoint({
        ...
        const buildEntryTurn = (target, lateralSign, carryForwardPx, leadForwardPx) => {
            ...
            const buildLeadPoint = (bridgePoints) => {
                const bridgeExitPoint = bridgePoints[bridgePoints.length - 1] ?? start;
                const bridgeExitHeadingDegrees = this.resolveAirShowPathHeadingDegrees([start, ...bridgePoints], options.startHeadingDegrees);
            ...
            while (entryBridgePoints.length > 0) {
                const previous = entryBridgePoints.length >= 2 ? entryBridgePoints[entryBridgePoints.length - 2] : start;
            ...
            const evaluatePathTurns = (path) => {
                ...
                for (let index = 1; index < path.length - 1; index += 1) {
                    ...
                    maxTurnDeg = Math.max(maxTurnDeg, this.resolveAirShowWaypointTurnDegrees(previous, current, next));
                }
            ...
            const buildMonotonicApproachPath = () => {
                ...
                const routeNormal = { x: -routeForward.y, y: routeForward.x };
                const clampInline = (point) => this.clampPointToViewportBounds(point, corridor.center, 430, 300);
            ...
            let resolvedEntryPath = this.pruneAirShowTurnWaypoint(smoothedEntryPath, smoothedEntryPath.length - 2, 56);
            const prependHeadingCommit = (path) => {
                ...
                const commitDistancePx = Math.min(Math.max(52, routeDistance * 0.22), Math.max(86, routeDistance * 0.34));
                ...
                const commitLead = this.buildAirShowHeadingLeadPoint(commitPoint, firstPoint, {
                    startHeadingDegrees: this.resolveAircraftHeadingDegrees(commitPoint.cx - start.cx, commitPoint.cy - start.cy, options.startHeadingDegrees),
                    lateralSign,
                    leadForwardPx: Math.min(Math.max(32, routeDistance * 0.14), Math.max(48, routeDistance * 0.2)),
                    leadLateralPx: 10,
                    clampCenter: corridor.center,
                    maxHorizontalPx: 430,
                    maxVerticalPx: 300
                });
                return this.pruneAirShowEarlyTurnWaypoints([start, commitPoint, commitLead, ...path.slice(1)].filter((point, index, points) => {
                    const previous = index === 0 ? null : points[index - 1];
                    if (!previous) {
                        return true;
                    }
            ...
            if (entryStats.firstTurnDeg > 54 ||
                ...
                resolvedEntryPath = this.pruneAirShowTurnWaypoint(smoothedMonotonicPath, smoothedMonotonicPath.length - 2, 60);
            }
        ...
        if (reengage) {
            ...
            const approachEntry = pointFromFocus(sideSign * Math.max(18, entrySeparationPx * 0.1), sideSign * Math.max(54, entrySeparationPx * 0.36 + laneSpreadPx * 0.12));
            const approachTurn = buildEntryTurn(approachEntry, sideSign, 52, 42);
            // Commit pass: monotonic sweep through the focal zone
            const commitPoint = pointFromFocus(sideSign * Math.max(8, crossSeparationPx * 0.45), sideSign * Math.max(8, crossSeparationPx * 0.6 + laneIndex * 4));
            // Break turn: sharp exit arc away from focal zone, same lateral direction — no reversal
            const breakApex = pointFromFocus(sideSign * Math.max(44, turnRadiusPx * 0.3), -sideSign * Math.max(52, turnRadiusPx * 0.34 + laneSpreadPx * 0.12));
            // Rejoin arc: sweeps outward continuing the break direction
            const rejoinArc = pointFromFocus(sideSign * Math.max(92, overshootPx * 0.52), -sideSign * Math.max(70, turnRadiusPx * 0.46 + laneSpreadPx * 0.14));
            // Egress arc: exits the engagement zone cleanly
            const egressEnd = pointFromFocus(sideSign * Math.max(138, overshootPx * 0.8), -sideSign * Math.max(40, turnRadiusPx * 0.24 + laneSpreadPx * 0.08));
            const smoothedReengage = this.pruneAirShowEarlyTurnWaypoints([...approachTurn, commitPoint, breakApex, rejoinArc, egressEnd], {
                maxTurnDeg: 48,
                strongTurnDeg: 104,
        ...
        const turnInPoint = pointFromFocus(-sideSign * Math.max(24, entrySeparationPx * 0.15), sideSign * Math.max(74, entrySeparationPx * 0.48 + laneSpreadPx * 0.14 + passSign * 4));
        const entryTurn = buildEntryTurn(turnInPoint, sideSign, 58, 48);
        const mergePoint = pointFromFocus(-sideSign * Math.max(6, crossSeparationPx * 0.3), sideSign * Math.max(22, crossSeparationPx * 1.4 + laneIndex * 22));
        const crossingPoint = pointFromFocus(sideSign * Math.max(10, crossSeparationPx * 0.52), -sideSign * Math.max(10, crossSeparationPx * 0.92 - 6 + laneIndex * 18));
        // Break turn exit: sweeps away from focal zone in consistent direction — no coil reversal
        const breakExit = pointFromFocus(sideSign * Math.max(50, overshootPx * 0.28), -sideSign * Math.max(62, turnRadiusPx * 0.42 + laneSpreadPx * 0.1 + passSign * 6));
        // Egress continues the break direction monotonically
        const egressPoint = pointFromFocus(sideSign * Math.max(104, overshootPx * 0.6), -sideSign * Math.max(46, turnRadiusPx * 0.32 + laneSpreadPx * 0.08));
        return this.pruneAirShowSharpTurns([...entryTurn, mergePoint, crossingPoint, breakExit, egressPoint], 118, 2);
    }
    resolveAirShowHeadingVector(headingDegrees) {
        return this.normalizeAircraftVector(Math.cos(((headingDegrees - 90) * Math.PI) / 180), Math.sin(((headingDegrees - 90) * Math.PI) / 180), 0, -1);
    }
    resolveAirShowFlightHeadingDegrees(flight) {
        const activeActors = flight.actors.filter((actor) => actor.active);
        if (activeActors.length === 0) {
            return 0;
        }
        const vector = activeActors.reduce((acc, actor) => {
    ...
    resolveSceneBomberSpecs(scene) {
        if (Array.isArray(scene.bombers) && scene.bombers.length > 0) {
            return scene.bombers.map((bomber) => ({
                ...bomber,
        ...
        if (scene.bomber) {
            return [
                {
                    ...scene.bomber,
                    targetHexKey: scene.bomber.targetHexKey ?? scene.bomberTargetHexKey ?? null
                }
            ];
        }
        return [];
    }
    resolveAirShowBomberTargetHexKey(bomber, scene) {
        return bomber.targetHexKey ?? scene.bomberTargetHexKey ?? null;
    }
    resolveAirShowBomberTargetCenter(bomber, scene) {
    ...
    resolveAirShowBomberFlakBursts(scene, bomberId) {
    ...
    resolveAirShowBomberPassEntries(scene, flightMap) {
    ...
    resolveAirShowCorridorSideSign(point, corridor, fallback = 1) {
        const lateralOffset = (point.cx - corridor.center.cx) * corridor.normal.x +
    ...
    resolveAirShowRouteSideSign(start, end, startHeadingDegrees, fallback = 1) {
        const dx = end.cx - start.cx;
        const dy = end.cy - start.cy;
        const length = Math.max(1, Math.hypot(dx, dy));
    ...
    buildAirShowTargetRunEscortPath(start, targetApproach, corridor, options = {}) {
        const laneIndex = options.laneIndex ?? 0;
        const sideSign = options.sideSign ?? 1;
        const destination = this.clampPointToViewportBounds({
            cx: targetApproach.cx +
    ...
    resolveAirShowBomberIngressBandPlan(sceneKind) {
    ...
    resolveAirShowIngressBandHoldTarget(corridor, current, sceneKind, role, laneIndex, jitterAlongPx = 0, jitterLateralPx = 0) {
        ...
        const alongSign = Math.abs(currentProjection.alongPx) > 6
            ? (currentProjection.alongPx >= 0 ? 1 : -1)
            : role === "escort" ? 1 : -1;
        return this.clampPointToViewportBounds(this.projectAirShowCorridorPoint(corridor, alongSign * Math.abs(plan.alongPx) + laneIndex * (plan.alongStepPx ?? 34) + jitterAlongPx, plan.lateralPx + laneIndex * (plan.lateralStepPx ?? 48) + jitterLateralPx), corridor.center, 430, 300);
    }
    resolveAirShowBomberIngressBandHoldTarget(corridor, current, sceneKind, laneIndex, bomberCount, jitterAlongPx = 0, jitterLateralPx = 0) {
        ...
        const targetAbsAlong = sceneKind === "airToAir"
            ? (() => {
        ...
        if (bomberCount <= 1) {
            const lateralBias = Math.abs(currentProjection.lateralPx) > 24
        ...
        return this.clampPointToViewportBounds(this.projectAirShowCorridorPoint(corridor, alongSign * targetAbsAlong + laneIndex * (plan.alongStepPx ?? 34) + jitterAlongPx, targetLateralPx), corridor.center, 820, 300);
    }
    ...
    resolveAirShowBomberClashTransitTarget(corridor, bomberCurrent, beat, laneIndex) {
        ...
        const desiredLaneLateralPx = laneIndex * 44;
        const targetLateralPx = this.clamp(currentProjection.lateralPx * (beat === 0 ? 0.48 : 0.18)
            + desiredLaneLateralPx * (beat === 0 ? 0.52 : 0.82), desiredLaneLateralPx - 28, desiredLaneLateralPx + 28);
        ...
        const targetAlongPx = this.clamp(Math.max(stagedAlongPx + laneIndex * 10, currentProjection.alongPx + (beat === 0 ? 130 : 160)), currentProjection.alongPx + minAdvancePx, currentProjection.alongPx + maxAdvancePx);
        return this.clampPointToViewportBounds(this.projectAirShowCorridorPoint(corridor, targetAlongPx, targetLateralPx), corridor.center, 430, 300);
    }
    resolveAirShowBomberIngressContinuationTarget(corridor, bomberCurrent, targetApproach, laneIndex) {
        const currentProjection = this.resolveAirShowCorridorCoordinates(corridor, bomberCurrent);
        const targetProjection = this.resolveAirShowCorridorCoordinates(corridor, targetApproach);
        const desiredLaneLateralPx = targetProjection.lateralPx + laneIndex * 4;
        const targetLateralPx = this.clamp(currentProjection.lateralPx * 0.34 + desiredLaneLateralPx * 0.66, desiredLaneLateralPx - 20, desiredLaneLateralPx + 20);
        const remainingAlongPx = targetProjection.alongPx - currentProjection.alongPx;
        // Per North Star Spec §Scenario 5 Phase 4: bombers continue at V/2 during the
        // CAP-vs-bombers lull — they must never freeze. Enforce a minimum forward advance
        // sized for a ~3000ms phase at V/2 so the motion is visibly continuous even when
        // bombers have overshot their nominal target approach in earlier beats.
        const minContinuousAdvancePx = 170; // ≈ V/2 × 3000ms phase duration
        if (remainingAlongPx <= 24) {
            return this.clampPointToViewportBounds(this.projectAirShowCorridorPoint(corridor, currentProjection.alongPx + Math.max(minContinuousAdvancePx, remainingAlongPx * 0.5), targetLateralPx), corridor.center, 430, 300);
        }
        // Bombers should continue steadily toward their actual approach line and still leave
        // a short forward reserve for the defense/target-run handoff. This keeps the whole
        // run continuous instead of overshooting to a generic corridor point and reversing.
        const reserveBeforeApproachPx = this.clamp(remainingAlongPx * 0.26, 72, 144);
        const minAdvancePx = Math.min(this.clamp(remainingAlongPx * 0.46, 132, 220), remainingAlongPx - 20);
        const maxAdvancePx = Math.min(this.clamp(remainingAlongPx * 0.72, 220, 360), remainingAlongPx - 12);
        const desiredAlongPx = targetProjection.alongPx - reserveBeforeApproachPx;
        const targetAlongPx = this.clamp(desiredAlongPx, currentProjection.alongPx + Math.max(minContinuousAdvancePx, minAdvancePx), currentProjection.alongPx + Math.max(minContinuousAdvancePx + 40, maxAdvancePx));
        return this.clampPointToViewportBounds(this.projectAirShowCorridorPoint(corridor, targetAlongPx, targetLateralPx), corridor.center, 430, 300);
    }
    resolveAirShowBomberDefenseContinuationTarget(corridor, bomberCurrent, targetApproach, laneIndex) {
        const currentProjection = this.resolveAirShowCorridorCoordinates(corridor, bomberCurrent);
        const targetProjection = this.resolveAirShowCorridorCoordinates(corridor, targetApproach);
        const desiredLaneLateralPx = targetProjection.lateralPx + laneIndex * 4;
        const lateralDeltaPx = desiredLaneLateralPx - currentProjection.lateralPx;
        const maxLateralStepPx = this.clamp(Math.abs(targetProjection.alongPx - currentProjection.alongPx) * 0.12, 8, 18);
        const targetLateralPx = currentProjection.lateralPx + this.clamp(lateralDeltaPx, -maxLateralStepPx, maxLateralStepPx);
        const alongDeltaPx = targetProjection.alongPx - currentProjection.alongPx;
        const alongSign = Math.abs(alongDeltaPx) > 4 ? Math.sign(alongDeltaPx) : 1;
        const advancePx = this.clamp(Math.abs(alongDeltaPx) * 0.82, 96, 176);
        const targetAlongPx = alongSign >= 0
            ? Math.min(targetProjection.alongPx, currentProjection.alongPx + advancePx)
            : Math.max(targetProjection.alongPx, currentProjection.alongPx - advancePx);
        return this.clampPointToViewportBounds(this.projectAirShowCorridorPoint(corridor, targetAlongPx, targetLateralPx), corridor.center, 430, 300);
    }
    resolveAirShowBomberTargetRunExitPoint(corridor, bomberCurrent, targetCenter, targetApproach, laneIndex) {
        ...
        const targetAlongPx = Math.max(currentProjection.alongPx + 42, targetCenterProjection.alongPx + overshootPx);
        const desiredLaneLateralPx = targetApproachProjection.lateralPx + laneIndex * 2;
        const targetLateralPx = this.clamp(currentProjection.lateralPx * 0.24 + desiredLaneLateralPx * 0.76, desiredLaneLateralPx - 14, desiredLaneLateralPx + 14);
        return this.clampPointToViewportBounds(this.projectAirShowCorridorPoint(corridor, targetAlongPx, targetLateralPx), corridor.center, 430, 300);
    }
    buildAirShowBomberInterceptPassPath(start, corridor, options) {
        const laneIndex = options.laneIndex ?? 0;
        ...
        const clampPoint = (point) => this.clampPointToViewportBounds(point, corridor.center, 420, 280);
        const entryStart = clampPoint({
            cx: start.cx + laneIndex * 3.2,
            cy: start.cy + attackSideSign * laneIndex * 1.2
        });
        const pointOnCorridor = (alongPx, lateralPx) => clampPoint(this.projectAirShowCorridorPoint(corridor, alongPx, lateralPx));
        const stageAlongPx = this.clamp(currentProjection.alongPx + (options.passStartAlongPx - currentProjection.alongPx) * 0.34, Math.min(currentProjection.alongPx, options.passStartAlongPx) - 10, Math.max(currentProjection.alongPx, options.passStartAlongPx) + 10);
        const stagePoint = pointOnCorridor(stageAlongPx, laneSpreadPx + attackSideSign * 72);
        const gunPoint = pointOnCorridor(options.passStartAlongPx + passDirection * 12, laneSpreadPx + attackSideSign * 12);
        const crossingPoint = pointOnCorridor(options.passEndAlongPx + passDirection * 14, laneIndex * 10 - attackSideSign * 4);
        const extendPoint = pointOnCorridor(options.passEndAlongPx + passDirection * 56, laneIndex * 12 - attackSideSign * 26);
        const exitPoint = pointOnCorridor(options.passEndAlongPx + passDirection * 92, laneIndex * 14 - attackSideSign * 52);
        const evaluatePathEntry = (path) => {
            let maxTurnDeg = 0;
            for (let index = 1; index < path.length - 1; index += 1) {
                const previous = path[index - 1];
                const current = path[index];
        ...
        const buildMonotonicInterceptEntryPath = () => {
            ...
            const routeNormal = { x: -routeForward.y, y: routeForward.x };
            const clampInline = (point) => this.clampPointToViewportBounds(point, corridor.center, 420, 280);
        ...
        const prependHeadingCommit = (path) => {
            ...
            const commitDistancePx = Math.min(Math.max(32, routeDistance * 0.14), Math.max(58, routeDistance * 0.24));
            const commitPoint = this.clampPointToViewportBounds({
                cx: start.cx +
                    headingForward.x * commitDistancePx +
                    routeNormal.x * attackSideSign * Math.min(12, routeDistance * 0.04),
            ...
            const commitLead = this.buildAirShowHeadingLeadPoint(commitPoint, firstPoint, {
                ...
                leadForwardPx: Math.min(Math.max(18, routeDistance * 0.1), Math.max(30, routeDistance * 0.15)),
                leadLateralPx: 6,
                clampCenter: corridor.center,
                maxHorizontalPx: 420,
                maxVerticalPx: 280
            });
            const candidatePath = this.pruneAirShowEarlyTurnWaypoints([start, commitPoint, commitLead, ...path.slice(1)].filter((point, index, points) => {
                const previous = index === 0 ? null : points[index - 1];
                if (!previous) {
                    return true;
                }
            ...
            if (candidateStats.firstTurnDeg > 52
        ...
        const bridgeExitHeadingDegrees = this.resolveAirShowPathHeadingDegrees([entryStart, ...entryBridgePoints], options.startHeadingDegrees);
        ...
        const earlyPruned = this.pruneAirShowEarlyTurnWaypoints(this.pruneAirShowTurnWaypoint(path, headingLeadIndex, 52), {
            maxTurnDeg: 44,
            strongTurnDeg: 88,
            maxFirstSegmentPx: 72,
        ...
        if (entryStats.firstTurnDeg > 54
            ...
            || entryStats.firstPointRecedes) {
            const monotonicPath = buildMonotonicInterceptEntryPath();
            const smoothedMonotonicPath = this.pruneAirShowEarlyTurnWaypoints(monotonicPath, {
                maxTurnDeg: 46,
                strongTurnDeg: 92,
                maxFirstSegmentPx: 78,
            ...
            resolvedPath = this.pruneAirShowSharpTurns(smoothedMonotonicPath, 112, 2);
        }
        return prependHeadingCommit(resolvedPath);
    }
    resolveAirShowPathForwardVector(routeForward, startHeadingDegrees, minRouteDot = 0.14) {
        if (typeof startHeadingDegrees !== "number") {
            return routeForward;
        }
        const headingForward = this.resolveAirShowHeadingVector(startHeadingDegrees);
        ...
        return this.normalizeAircraftVector(headingForward.x * (1 - routeBlend) + routeForward.x * routeBlend, headingForward.y * (1 - routeBlend) + routeForward.y * routeBlend, routeForward.x, routeForward.y);
    }
    buildAirShowHeadingLeadPoint(start, toward, options = {}) {
        const dx = toward.cx - start.cx;
        const dy = toward.cy - start.cy;
        const distance = Math.max(1, Math.hypot(dx, dy));
        const routeForward = this.normalizeAircraftVector(dx, dy, 0, -1);
        const routeNormal = { x: -routeForward.y, y: routeForward.x };
        const headingForward = this.resolveAirShowPathForwardVector(routeForward, options.startHeadingDegrees);
        const alignment = this.clamp((headingForward.x * routeForward.x + headingForward.y * routeForward.y + 1) * 0.5, 0, 1);
        const blendedForward = this.normalizeAircraftVector(headingForward.x * (0.16 + alignment * 0.34) + routeForward.x * (0.84 - alignment * 0.34), headingForward.y * (0.16 + alignment * 0.34) + routeForward.y * (0.84 - alignment * 0.34), routeForward.x, routeForward.y);
        const requestedForwardPx = options.leadForwardPx ?? Math.max(30, distance * 0.22);
        const forwardPx = Math.min(requestedForwardPx * (0.34 + alignment * 0.66), Math.max(12, distance * (0.22 + alignment * 0.34)));
        const lateralSign = options.lateralSign ?? 0;
        const requestedLateralPx = Math.abs(options.leadLateralPx ?? 0);
        const lateralPx = Math.min(requestedLateralPx, Math.max(0, distance * (0.08 + alignment * 0.08))) *
            lateralSign;
        const rawPoint = {
        ...
        const progressAlongRoute = (rawPoint.cx - start.cx) * routeForward.x + (rawPoint.cy - start.cy) * routeForward.y;
        const clampedAlongRoute = this.clamp(progressAlongRoute, distance * (0.1 + alignment * 0.08), distance * (0.38 + alignment * 0.22));
        const point = {
    ...
    buildAirShowHeadingCarryPoint(start, options) {
        ...
        const headingForward = typeof options.startHeadingDegrees === "number"
            ...
            : routeForward;
        const alignment = this.clamp((headingForward.x * routeForward.x + headingForward.y * routeForward.y + 1) * 0.5, 0, 1);
        const forward = this.normalizeAircraftVector(headingForward.x * (0.1 + alignment * 0.56) + routeForward.x * (0.9 - alignment * 0.56), headingForward.y * (0.1 + alignment * 0.56) + routeForward.y * (0.9 - alignment * 0.56), routeForward.x, routeForward.y);
        const carryForwardPx = Math.min((options.carryForwardPx ?? 48) * (0.14 + alignment * 0.86), Math.max(8, distance * (0.08 + alignment * 0.26)));
        const point = {
    ...
    resolveAirShowPathHeadingDegrees(points, fallbackHeadingDegrees) {
    ...
    buildAirShowPhaseEntryBridge(start, toward, options) {
        const dx = toward.cx - start.cx;
        const dy = toward.cy - start.cy;
        const distance = Math.max(1, Math.hypot(dx, dy));
        const routeForward = this.normalizeAircraftVector(dx, dy, 0, -1);
        ...
        const carryDistancePx = Math.min(Math.max(options.carryForwardPx ?? 56, distance * (0.2 + reversalFactor * 0.12)), Math.max(96, distance * (0.34 + reversalFactor * 0.12)));
        const lateralArcPx = Math.min(Math.max(46, distance * (0.14 + reversalFactor * 0.12)), Math.max(98, distance * (0.3 + reversalFactor * 0.08)));
        const joinAlongPx = Math.min(Math.max(52, distance * (0.18 + reversalFactor * 0.1)), Math.max(92, distance * (0.34 + reversalFactor * 0.08)));
        const joinLateralPx = Math.min(lateralArcPx * (0.18 + reversalFactor * 0.18), Math.max(20, distance * 0.16));
        const handleInPx = Math.min(Math.max(30, joinAlongPx * (0.4 + reversalFactor * 0.12)), Math.max(56, distance * (0.18 + reversalFactor * 0.06)));
        ...
        const cubicPointAt = (progress) => {
            ...
            const t2 = t * t;
            return clampPoint({
                cx: oneMinusT2 * oneMinusT * carryPointB.cx +
                    3 * oneMinusT2 * t * startControl.cx +
                    3 * oneMinusT * t2 * joinControl.cx +
                    t2 * t * joinPoint.cx,
                cy: oneMinusT2 * oneMinusT * carryPointB.cy +
        ...
        const samples = [
            carryPointA,
            carryPointB,
            ...Array.from({ length: stepCount }, (_, index) => cubicPointAt((index + 1) / stepCount))
        ];
        ...
        while (simplifiedSamples.length > 1) {
            const previous = simplifiedSamples.length >= 2
    ...
    measureAirShowPathLength(points) {
        let lengthPx = 0;
    ...
    scaleAirShowAssignmentDeviationFromChord(assignment, deviationScale) {
        if (!Number.isFinite(deviationScale) || deviationScale <= 0 || assignment.points.length < 3) {
            return assignment;
        }
        ...
        const chordDx = end.cx - start.cx;
        const chordDy = end.cy - start.cy;
        const chordLength = Math.hypot(chordDx, chordDy);
        if (chordLength < 0.0001) {
            return assignment;
        }
        const cumulativeDistances = [0];
        for (let index = 1; index < assignment.points.length; index += 1) {
            const previous = assignment.points[index - 1];
            const current = assignment.points[index];
            if (!previous || !current) {
                cumulativeDistances.push(cumulativeDistances[index - 1] ?? 0);
                continue;
            }
            cumulativeDistances.push((cumulativeDistances[index - 1] ?? 0) + Math.hypot(current.cx - previous.cx, current.cy - previous.cy));
        }
        ...
        const normalizedChord = {
            x: chordDx / chordLength,
            y: chordDy / chordLength
        };
        const normalizedNormal = {
            x: -normalizedChord.y,
            y: normalizedChord.x
        };
        const points = assignment.points.map((point, index) => {
            if (index === 0) {
                return start;
            }
            if (index === assignment.points.length - 1) {
                return end;
            }
            const progress = (cumulativeDistances[index] ?? 0) / totalDistance;
            const baseline = {
                cx: start.cx + chordDx * progress,
                cy: start.cy + chordDy * progress
            };
            const deviation = {
                x: point.cx - baseline.cx,
                y: point.cy - baseline.cy
            };
            const deviationAlong = deviation.x * normalizedChord.x + deviation.y * normalizedChord.y;
            const deviationLateral = deviation.x * normalizedNormal.x + deviation.y * normalizedNormal.y;
            const scaledAlong = deviationAlong * this.clamp(0.78 + deviationScale * 0.22, 0.72, 1.32);
            const scaledLateral = deviationLateral * deviationScale;
            return {
                cx: baseline.cx +
                    normalizedChord.x * scaledAlong +
                    normalizedNormal.x * scaledLateral,
                cy: baseline.cy +
                    normalizedChord.y * scaledAlong +
                    normalizedNormal.y * scaledLateral
            };
        });
        return {
            ...assignment,
    ...
    retargetAirShowAssignmentPathLength(assignment, targetPathLengthPx) {
        if (!Number.isFinite(targetPathLengthPx) || targetPathLengthPx <= 0 || assignment.points.length < 3) {
            return assignment;
        }
        const start = assignment.points[0];
        const end = assignment.points[assignment.points.length - 1];
        ...
        const chordLengthPx = Math.hypot(end.cx - start.cx, end.cy - start.cy);
        const minimumTargetLengthPx = Math.max(chordLengthPx + 2, chordLengthPx * 1.01);
        const clampedTargetLengthPx = Math.max(minimumTargetLengthPx, targetPathLengthPx);
        const currentPathLengthPx = this.measureAirShowPathLength(assignment.points);
        if (!Number.isFinite(currentPathLengthPx) || currentPathLengthPx <= 0) {
            return assignment;
        }
        if (Math.abs(currentPathLengthPx - clampedTargetLengthPx) <= 1.5) {
            return assignment;
        }
        let lowerScale = 1;
        let upperScale = 1;
        if (clampedTargetLengthPx < currentPathLengthPx) {
            lowerScale = 0;
        }
        else {
            upperScale = 1.25;
            let attempts = 0;
            while (attempts < 8) {
                const candidate = this.scaleAirShowAssignmentDeviationFromChord(assignment, upperScale);
                const candidateLengthPx = this.measureAirShowPathLength(candidate.points);
                if (candidateLengthPx >= clampedTargetLengthPx || upperScale >= 5) {
                    break;
                }
                upperScale *= 1.35;
                attempts += 1;
        ...
        let bestErrorPx = Math.abs(currentPathLengthPx - clampedTargetLengthPx);
        for (let iteration = 0; iteration < 12; iteration += 1) {
            const candidateScale = (lowerScale + upperScale) * 0.5;
            const candidate = this.scaleAirShowAssignmentDeviationFromChord(assignment, candidateScale);
            const candidateLengthPx = this.measureAirShowPathLength(candidate.points);
            const candidateErrorPx = Math.abs(candidateLengthPx - clampedTargetLengthPx);
            if (candidateErrorPx < bestErrorPx) {
                bestAssignment = candidate;
                bestErrorPx = candidateErrorPx;
            }
            if (candidateLengthPx < clampedTargetLengthPx) {
    ...
    resolveAirShowRoleSpeedMap(overrides = {}) {
        return new Map([
    ...
    normalizeAirShowPhaseRoleSpeeds(assignments, durationMs, roleTargetSpeeds = this.resolveAirShowRoleSpeedMap()) {
        let normalizedAssignments = [...assignments];
        roleTargetSpeeds.forEach((targetSpeedPxPerMs, role) => {
            const relevantAssignments = normalizedAssignments.filter((assignment) => assignment.actor.role === role);
            if (relevantAssignments.length === 0 || targetSpeedPxPerMs <= 0 || durationMs <= 0) {
                return;
            }
            const targetPathLengthPx = targetSpeedPxPerMs * durationMs;
            normalizedAssignments = normalizedAssignments.map((assignment) => {
                if (assignment.actor.role !== role) {
                    return assignment;
                }
                return this.retargetAirShowAssignmentPathLength(assignment, targetPathLengthPx);
            });
        });
    ...
    finalizeAirShowPhaseAssignments(assignments, durationMs, progressSamplePoints, smoothEntryRadiusPx, roleTargetSpeeds = this.resolveAirShowRoleSpeedMap()) {
        const normalizedAssignments = this.normalizeAirShowPhaseRoleSpeeds(assignments, durationMs, roleTargetSpeeds);
        const spacedAssignments = this.resolveAirShowPhaseSpacing(normalizedAssignments, progressSamplePoints);
        const shapedAssignments = typeof smoothEntryRadiusPx === "number"
            ...
            : spacedAssignments;
        return this.normalizeAirShowPhaseRoleSpeeds(shapedAssignments, durationMs, roleTargetSpeeds);
    }
    resolveAirShowPhaseDurationFromRoleSpeeds(assignments, roleTargetSpeeds, fallbackMs, minMs, maxMs, primaryRoles) {
        ...
        const candidateDurations = rolesToMeasure
            ...
            const meanPathLengthPx = relevantAssignments.reduce((sum, assignment) => sum + this.measureAirShowPathLength(assignment.points), 0)
            ...
            .filter((durationMs) => typeof durationMs === "number" && Number.isFinite(durationMs) && durationMs > 0);
    ...
    resolveAirShowRolePhaseDurationMs(assignments, roles, targetSpeedPxPerMs, fallbackMs, minMs, maxMs) {
    ...
    resolveAirShowFighterPhaseDurationMs(assignments, fallbackMs, minMs, maxMs) {
    ...
    sampleAirShowAssignmentAtProgress(assignment, progress) {
        ...
        const progressOffsetWeight = clampedProgress <= 0 || clampedProgress >= 1
            ? 0
            : Math.sin(clampedProgress * Math.PI);
        const sample = this.sampleAircraftWaypointPath(assignment.points, this.clamp(clampedProgress + (assignment.progressOffset ?? 0) * progressOffsetWeight, 0, 1));
        ...
        return {
            ...
            headingDegrees: this.resolveAircraftHeadingDegrees(sample.derivative.dx, sample.derivative.dy, assignment.actor.headingDegrees),
     ...
     */
    resolveAirShowPhaseSpacing(assignments, progressSamplePoints = [0.3, 0.5, 0.7] // Check spacing at multiple points along paths
    ) {
        ...
        for (const progress of progressSamplePoints) {
            ...
            const resolvedPositions = this.resolveAirShowCollisionFreePositions(allActors, positionsAtProgress, 2 // Max iterations per progress point
            );
    ...
    smoothAirShowAssignmentEntries(assignments, maxTurnDeg = 48) {
        return assignments.map((assignment) => {
            ...
            const adjustedFirstSegmentPx = Math.min(firstSegmentPx, Math.max(18, routeDistance * 0.28));
            ...
            if (adjustedPoints.length >= 4) {
                ...
                if (third) {
                    ...
                    if (this.resolveAirShowWaypointTurnDegrees(adjustedFirstPoint, second, third)
                        > maxTurnDeg * 2.2) {
                        const secondForward = this.normalizeAircraftVector(secondRouteDx, secondRouteDy, 0, -1);
                        adjustedPoints[2] = {
                            cx: adjustedFirstPoint.cx +
    ...
    selectAirShowTracerActors(sourceFlight, targetFlight, assignments, progress, emitter = "nose", constraints = {}) {
        const assignmentsByActorId = this.buildAirShowAssignmentLookup(assignments);
        let bestSource = null;
        let bestTarget = null;
    ...
    resolveAirShowEmitterPoint(actor, emitter) {
        if (emitter === "center") {
    ...
    rotateAirShowVector(vector, degrees) {
    ...
    resolveAirShowTracerBurstGeometry(actor, burst) {
        const burstCount = Math.max(1, burst.burstCount ?? 1);
        const emitterPoint = burst.emitter === "center"
            ? actor.position
            : this.resolveAirShowEmitterPoint(actor, burst.emitter);
        const sourceHeadingDegrees = ((actor.headingDegrees % 360) + 360) % 360;
        const baseForward = this.normalizeAircraftVector(Math.cos(((sourceHeadingDegrees - 90) * Math.PI) / 180), Math.sin(((sourceHeadingDegrees - 90) * Math.PI) / 180), 0, -1);
        ...
        const fanHalfAngleDeg = Math.max(0, burst.fanHalfAngleDeg
    ...
    buildAirShowTracerVolley(source, target, options = {}) {
        const density = Math.max(4, options.burstCount ?? (options.emitter === "center" ? 6 : 7));
        const timingCount = Math.max(6, density);
        const timings = options.timings
            ?? Array.from({ length: timingCount }, (_, index) => 0.38 + (index / Math.max(1, timingCount - 1)) * 0.42);
        const perBurstSegments = 1;
        const baseSpreadPx = Math.min(options.spreadPx ?? (options.emitter === "center" ? 3 : 2), options.emitter === "center" ? 3 : 2);
        const baseFanHalfAngleDeg = Math.min(options.fanHalfAngleDeg ?? (options.emitter === "center" ? 3 : 2), options.emitter === "center" ? 3 : 2);
    ...
    buildAirShowDynamicTracerVolley(assignments, sourceFlight, targetFlight, options = {}) {
        ...
        return timings.flatMap((progress, volleyIndex) => {
            const probeProgresses = [
                ...
                .filter((candidate, index, array) => array.findIndex((value) => Math.abs(value - candidate) < 0.001) === index);
            let pair = null;
    ...
    resolveAirShowVectorAngleDegrees(left, right) {
    ...
    shouldRenderAirShowTracerBurst(source, targetPoint, burst) {
        ...
        const thresholdDeg = burst.emitter === "center"
    ...
    playAirShowTracerBurst(burst) {
        ...
        geometry.segments.forEach((segment, index) => {
            ...
            for (let pulseIndex = 0; pulseIndex < pulseCount; pulseIndex += 1) {
                ...
                }, index * 20 + pulseIndex * 22);
            }
        });
    }
    averageAirShowPosition(actors) {
    ...
    describeInspectionAirShowFlight(flight) {
        return {
            id: flight.spec.id,
    ...
    applyInspectionAirShowAssignments(assignments) {
        assignments.forEach((assignment) => {
            ...
            this.positionAircraftImageGhost(assignment.actor.image, assignment.actor.size, assignment.actor.position.cx, assignment.actor.position.cy, assignment.actor.headingDegrees);
        });
    }
    syncAirShowFlightStrengthForInspection(flight, targetStrength) {
     ...
     */
    updateCombatVolumePosition(actor, volume, targetX, targetY, blend) {
     ...
     */
    updateBomberCorridorPosition(actor, corridor, progress, lateralOffsetPx) {
     ...
     */
    clampToViewport(x, y, centerX, centerY, marginH = 600, marginV = 400) {
        return {
            x: Math.max(centerX - marginH, Math.min(centerX + marginH, x)),
            y: Math.max(centerY - marginV, Math.min(centerY + marginV, y))
        };
    }
    selectAirShowActor(flight, index, preferTail = false) {
        if (!flight) {
            return null;
        }
        const activeActors = flight.actors.filter((actor) => actor.active);
    ...
    async runAirShowPhase(assignments, durationMs, tracerBursts = [], options = {}) {
        ...
        await new Promise((resolve) => {
            ...
            const step = (now) => {
                ...
                assignments.forEach((assignment) => {
                    const sample = this.sampleAirShowAssignmentAtProgress(assignment, easedProgress);
                    assignment.actor.headingDegrees = this.interpolateAircraftHeadingDegrees(assignment.actor.headingDegrees, sample.headingDegrees, assignment.headingBlend ?? 0.34);
                    assignment.actor.position = sample.position;
                    this.positionAircraftImageGhost(assignment.actor.image, assignment.actor.size, sample.position.cx, sample.position.cy, assignment.actor.headingDegrees);
                });
    ...
    syncAirShowPhaseVisibility(assignments, sceneActors = []) {
        const assignedActorIds = new Set(assignments.map((assignment) => assignment.actor.id));
        const actorsToSync = sceneActors.length > 0
            ? sceneActors
    ...
    async syncAirShowFlightStrength(flight, targetStrength, escapeVector) {
        ...
        const assignments = removedActors.map((actor, index) => {
            const diveEnd = {
    ...
    resolveAircraftSortieTurnVector(ingressDirection, egressDirection, laneOffsetPx) {
        const blendedDirection = this.normalizeAircraftVector(ingressDirection.x + egressDirection.x, ingressDirection.y + egressDirection.y, 0, 0);
        ...
        return this.normalizeAircraftVector(-ingressDirection.y * bankSign, ingressDirection.x * bankSign, -ingressDirection.y, ingressDirection.x);
    }
    interpolateAircraftHermitePoint(start, end, startTangent, endTangent, progress) {
        const t = this.clamp(progress, 0, 1);
        const t2 = t * t;
        const t3 = t2 * t;
    ...
    interpolateAircraftHermiteDerivative(start, end, startTangent, endTangent, progress) {
        const t = this.clamp(progress, 0, 1);
        const t2 = t * t;
        const dh00 = 6 * t2 - 6 * t;
    ...
    positionAircraftGhost(ghost, isFormation, iconSize, centerX, centerY, headingDegrees = 0) {
        if (isFormation) {
    ...
    async animateAircraftGhostArcSegment(ghost, isFormation, iconSize, start, end, durationMs, onProgress, bendDirection = 1) {
        const dx = end.cx - start.cx;
        const dy = end.cy - start.cy;
        const distance = Math.max(1, Math.hypot(dx, dy));
        const nx = -dy / distance;
    ...
    cleanupMoveGhost(ghost, original, restoreOpacity) {
        ghost.remove();
     ...
     */
    async playFlashOverlay(center, radius, intensity = 0.6, durationMs = 140) {
    ...
    chooseImpactCount(attackerClass) {
        switch (attackerClass) {
            case "tank":
            case "artillery":
                return 3;
    ...
    chooseSparkCount(defenderClass) {
    ...
    getUnitTypeDefinition(unitType) {
        if (!unitType) {
            return undefined;
        }
        return unitTypesData[unitType];
    }
    getImpactWeaponRating(attackerType, attackerClass) {
    ...
    chooseImpactSparkScale(attackerType, attackerClass) {
    ...
    chooseImpactSparkBurstCount(attackerType, attackerClass) {
        const impactRating = this.getImpactWeaponRating(attackerType, attackerClass);
        if (attackerType === "Bomber") {
            return 3;
        }
     ...
     */
    async playCombatAnimation(animationType, hexKey, offsetX = 0, offsetY = 0, scale = 1, soundRequest) {
     ...
     */
    getEffectDedupeWindowMs(animationType) {
     ...
     * Plays a combat effect directly at viewport coordinates, which keeps airbursts and future freeform effects off the hex grid.
     */
    async playCombatAnimationAt(animationType, x, y, scale = 1, soundRequest, terrainTint, dedupeKey) {
        const effectsLayer = this.ensureCombatEffectsLayer();
        if (!effectsLayer) {
     ...
     */
    async playFlakBurstAt(x, y, count = 1, scale = 1.08, dedupeKey) {
        ...
        const burstPromises = Array.from({ length: burstCount }).map((_, index) => {
            ...
            return new Promise((resolve) => {
                window.setTimeout(() => {
                    void this.playCombatAnimationAt("flakBurst", x + offsetX, y + offsetY, burstScale, false, undefined, dedupeKey === false ? false : dedupeKey).then(() => resolve());
                }, index * 28);
            });
        });
     ...
     */
    async playAirDamageSmokeTrailAt(x, y, scale = 0.72, dedupeKey) {
     ...
     */
    getWeaponEffectType(hexKey) {
        const scenarioType = this.getUnitScenarioTypeAt(hexKey);
        if (!scenarioType) {
            return "small_arms"; // Fallback
        }
        const unitDef = unitTypesData[scenarioType];
        if (!unitDef || !unitDef.weaponEffectType) {
            return "small_arms"; // Fallback
        }
        return unitDef.weaponEffectType;
    }
    resolveWeaponSoundClass(attackerHexKey, attackerType, attackerClass) {
        if (attackerType === "Bomber") {
            return "large_bomb";
        }
        if (attackerType === "Ground_Attack") {
    ...
    createArmorImpactSoundRequest(attackerHexKey, attackerType, attackerClass, gainMultiplier = 0.75) {
        return {
    ...
    async playWeaponSoundBurst(attackerHexKey, burstCount, intervalMs, gainMultiplier) {
        const soundClass = this.resolveWeaponSoundClass(attackerHexKey, this.getUnitScenarioTypeAt(attackerHexKey), this.getUnitClassAt(attackerHexKey));
        const soundBursts = Array.from({ length: Math.max(1, burstCount) }).map((_, index) => new Promise((resolve) => {
            window.setTimeout(() => {
    ...
    chooseDirectFireImpactProfile(attackerHexKey, targetIsHardTarget, defenderIsAir) {
        if (defenderIsAir) {
            return {
    ...
    chooseMuzzleFlashProfile(attackerHexKey) {
     ...
     */
    async playArtillerySupportImpact(defenderHexKey, targetClass) {
    ...
    async playRecoilNudge(attackerHexKey, defenderHexKey, magnitudePx = 5) {
        const attackerCell = this.hexElementMap.get(attackerHexKey);
        const defenderCell = this.hexElementMap.get(defenderHexKey);
    ...
    async playHitShake(hexKey, magnitudePx = 4) {
        const group = this.hexUnitImageMap.get(hexKey);
        if (!group) {
            return;
        }
    ...
    async playLegacySparkBurst(defenderHexKey, rayCount = 8, durationMs = 160) {
        const layer = this.ensureCombatEffectsLayer();
        if (!this.svgElement || !layer) {
            return;
        }
     ...
     */
    async playSparkBurst(defenderHexKey, options = {}) {
        const defenderClass = options.defenderClass;
        if (defenderClass !== "tank" && defenderClass !== "vehicle") {
            return this.playLegacySparkBurst(defenderHexKey, options.rayCount ?? this.chooseSparkCount(defenderClass), options.durationMs ?? 160);
        }
        const burstCount = this.clamp(Math.round(options.burstCount ?? this.chooseImpactSparkBurstCount(options.attackerType, options.attackerClass)), 1, 3);
        ...
        const impactSoundRequest = this.createArmorImpactSoundRequest(options.attackerHexKey, options.attackerType, options.attackerClass);
        const burstPromises = Array.from({ length: burstCount }).map((_, index) => new Promise((resolve) => {
            window.setTimeout(() => {
                ...
                void this.playCombatAnimation("impactHits", defenderHexKey, offsetX, offsetY, scale, {
                    ...impactSoundRequest,
    ...
    async playArtilleryImpactBurst(defenderHexKey, targetIsHardTarget, options) {
        ...
        const burstPromises = impactOffsets.map(([offsetX, offsetY], index) => new Promise((resolve) => {
            window.setTimeout(() => {
    ...
    async playRocketArtillerySalvo(attackerHexKey, defenderHexKey, targetIsHardTarget) {
        const volleyCenters = [
        ...
        const volleyPromises = volleyCenters.map((center, index) => new Promise((resolve) => {
            window.setTimeout(() => {
                void Promise.all([
     ...
     */
    async playAttackSequence(attackerHexKey, defenderHexKey, targetIsHardTarget) {
        ...
        const impactPromises = impactProfile.impactOffsets.map(([ox, oy], index) => new Promise((resolve) => {
            window.setTimeout(() => {
...
HexMapRenderer.AIR_SHOW_BOMBER_SPEED_PX_PER_MS = 0.0575; // 5.75px per 100ms
HexMapRenderer.AIR_SHOW_FIGHTER_SPEED_PX_PER_MS = 0.115; // 11.5px per 100ms
// Role-based size multipliers: bombers are 2x fighter size
HexMapRenderer.AIRCRAFT_FIGHTER_SIZE_MULTIPLIER = 0.75;
HexMapRenderer.AIRCRAFT_BOMBER_SIZE_MULTIPLIER = 1.5;
// Role-based spacing multipliers (proportional to size to prevent overlap)
HexMapRenderer.AIRCRAFT_FIGHTER_SPACING_MULTIPLIER = 0.75;
HexMapRenderer.AIRCRAFT_BOMBER_SPACING_MULTIPLIER = 1.5;
...
HexMapRenderer.AIRCRAFT_SAME_ROLE_SPACING_FACTOR = 0.8;
HexMapRenderer.AIRCRAFT_DIFF_ROLE_SPACING_FACTOR = 1.0;
HexMapRenderer.AIRCRAFT_MAX_DENSITY_BEFORE_EXPANSION = 6; // aircraft count threshold
...
HexMapRenderer.OFF_MAP_DISTANCE_PX = 1000;
