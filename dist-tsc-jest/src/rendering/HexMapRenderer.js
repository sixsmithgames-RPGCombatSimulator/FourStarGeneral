...
export class HexMapRenderer {
    ...
    resolveAirShowContestedBomberPhaseDurations(bomberFlights, corridor, approachProfilesByBomberId, scene, randomForLabel, fighterIngressDurationMs) {
        ...
        const weightedBomberIngressDurationMs = Math.max(1, Math.round(defaultDurations.bomberIngressDurationMs * 0.062));
        const weightedBomberDefenseDurationMs = Math.max(1, Math.round(defaultDurations.bomberDefenseDurationMs * 1.45));
    ...
    retimeContestedFighterIngressPlan(plan, governedDurationMs) {
        ...
        const speedBalancedAssignments = trimmedPlan.assignments.map((assignment) => {
            ...
            const targetSpeedPxPerMs = roleSpeeds.get(assignment.actor.role) ?? HexMapRenderer.AIR_SHOW_FIGHTER_SPEED_PX_PER_MS;
            const maxPathLengthPx = targetSpeedPxPerMs * safeGovernedDurationMs * 1.18;
            if (!Number.isFinite(currentPathLengthPx)
        ...
        for (let index = 0; index < pointCount; index += 1) {
            ...
            if (index < flashPointCount) {
                window.setTimeout(() => {
                    ...
                    void this.playFlakBurstAt(point.cx + (jitter01(index, 19) - 0.5) * 18, point.cy + (jitter01(index, 23) - 0.5) * 12, flashCount, burstScale, false);
                }, flashDelayMs);
            }
            if (!singlePuffWave && index < flashPointCount && (index + Math.round(jitter01(index, 29) * 7)) % 4 === 0) {
                window.setTimeout(() => {
                    void this.playFlakBurstAt(point.cx + (jitter01(index, 31) - 0.5) * 24, point.cy + (jitter01(index, 37) - 0.5) * 18, 1, scale * (0.46 + jitter01(index, 41) * 0.2), false);
