...
export class HexMapRenderer {
    ...
    renderUnitStack(hexKey, members) {
        ...
        visibleMembers.forEach((member, index) => {
            ...
            layout.forEach((spec, posIndex) => {
                const resolvedHref = reconStatus === "spotted"
    ...
    buildFringeHexMarkup(realAxialKeys, hexes, minX, minY, margin) {
        ...
        for (const ring of fringeGroups) {
            for (const { q, r, fill, opacity } of ring) {
                ...
                polygons.push(`<polygon points="${points}" fill="${fill}" fill-opacity="${opacity}" stroke="none" style="pointer-events:none;" />`);
            }
        }
    ...
    resolveAircraftSpriteLayoutSpecs(iconSize, strength, role = "interceptor") {
        const sizeMultiplier = role === "bomber"
            ? HexMapRenderer.AIRCRAFT_BOMBER_SIZE_MULTIPLIER
            : HexMapRenderer.AIRCRAFT_FIGHTER_SIZE_MULTIPLIER;
        const scaledIconSize = iconSize * sizeMultiplier;
    ...
    resolveHqAxis(playerHqKey, botHqKey) {
        ...
        return resolveAirShowHqAxis(playerHq, botHq, mapBounds, HexMapRenderer.OFF_MAP_DISTANCE_PX);
    }
    ...
    buildAirShowRuntimeFlightFromPlan(layer, flight) {
        ...
        const actors = flight.actors.map((plannedActor) => {
            ...
            this.positionAircraftImageGhost(image, plannedActor.size, plannedActor.position.cx, plannedActor.position.cy, plannedActor.headingDegrees);
    ...
    clampAirShowHqOriginToCorridorDistance(corridor, hqOrigin, maxAlongPx, minAlongPx = 440) {
    ...
    pruneAirShowEntryWindowSharpTurns(path, maxTurnDeg, maxWaypointsToRemove = 2, entryWaypointCount = 3) {
        const prunedPath = [...path];
        let removedWaypoints = 0;
        while (prunedPath.length >= 4 && removedWaypoints < maxWaypointsToRemove) {
    ...
    softenAirShowEntryWindowTurns(path, maxTurnDeg = 118, options = {}) {
        const entryWindowPrunedPoints = this.pruneAirShowEntryWindowSharpTurns(path, maxTurnDeg, options.maxWaypointsToRemove ?? 2, options.entryWaypointCount ?? 3);
        const resolvedPoints = entryWindowPrunedPoints.length >= 2 ? [...entryWindowPrunedPoints] : [...path];
        if (resolvedPoints.length >= 3) {
            ...
            if (resolvedStart && resolvedFirst && resolvedSecond) {
                const resolvedFirstTurnDeg = this.resolveAirShowWaypointTurnDegrees(resolvedStart, resolvedFirst, resolvedSecond);
                if (resolvedFirstTurnDeg > maxTurnDeg) {
                    ...
                    const firstSegmentPx = Math.hypot(resolvedFirst.cx - resolvedStart.cx, resolvedFirst.cy - resolvedStart.cy);
                    ...
                    const blendFactor = this.clamp((resolvedFirstTurnDeg - maxTurnDeg) / (options.blendRangeDeg ?? 36), options.minBlendFactor ?? 0.4, options.maxBlendFactor ?? 0.75);
                    resolvedPoints[1] = {
    ...
    softenAirShowExitWindowTurns(path, maxTurnDeg = 118, options = {}) {
        if (path.length < 3) {
            return [...path];
        }
        return this.softenAirShowEntryWindowTurns([...path].reverse(), maxTurnDeg, {
    ...
    sanitizeAirShowEntryPath(path, options = {}) {
        ...
        const entryWindowPruned = this.pruneAirShowEntryWindowSharpTurns(earlyPruned, options.maxSharpTurnDeg ?? 124, options.maxWaypointsToRemove ?? 2);
        return this.pruneAirShowSharpTurns(entryWindowPruned, options.maxSharpTurnDeg ?? 124, options.maxWaypointsToRemove ?? 2);
    }
    /**
    ...
    buildAirShowBomberTargetRunPath(start, targetCenter, options = {}) {
        const clampPoint = (point) => this.clampPointToViewportBounds(point, targetCenter, 430, 300);
        const forward = typeof options.startHeadingDegrees === "number"
            ? this.resolveAirShowHeadingVector(options.startHeadingDegrees)
        ...
        const normalizeSignedAngleRad = (radians) => {
            let value = radians;
            while (value <= -Math.PI) {
                value += Math.PI * 2;
            }
        ...
        const rotateQuarterTurn = (vector, sign) => sign >= 0
        ...
        const bestArc = candidateCenters
            ...
            const tangentAtStart = this.normalizeAircraftVector(tangentStartVector.x, tangentStartVector.y, forward.x, forward.y);
    ...
    resolveAirShowBomberApproachProfiles(bomberFlights, corridor, bomberTargetCentersById, averageBomberTargetCenter, randomForLabel) {
        return new Map(bomberFlights.map((bomberFlight, index) => {
    ...
    buildContestedBomberMasterPaths(bomberFlights, corridor, approachProfilesByBomberId, randomForLabel) {
        return new Map(bomberFlights.map((bomberFlight) => {
            ...
            const path = this.buildAirShowBomberContinuationPath(start, standoffPoint, {
                lateralSign: this.resolveAirShowRouteSideSign(start, standoffPoint, this.resolveAirShowFlightHeadingDegrees(bomberFlight), rand() > 0.5 ? 1 : -1),
    ...
    resolveAirShowContestedBomberPhaseDurations(bomberFlights, corridor, approachProfilesByBomberId, scene, randomForLabel, fighterIngressDurationMs) {
        ...
        const maxFighterIngressDurationMs = Math.max(1, Math.min(3200, Math.round(canonicalPreTargetDurationMs * 0.32)));
        ...
        const weightedEscortMergeDurationMs = Math.max(1, Math.round(defaultDurations.escortMergeDurationMs * 0.92));
        const weightedEscortScrambleDurationMs = Math.max(1, Math.round(defaultDurations.escortScrambleDurationMs * 0.56));
        const weightedBomberIngressDurationMs = Math.max(1, Math.round(defaultDurations.bomberIngressDurationMs * 0.22));
        const weightedBomberDefenseDurationMs = Math.max(1, Math.round(defaultDurations.bomberDefenseDurationMs * 1.16));
    ...
    buildContestedBomberPhaseSliceAssignments(bomberFlights, masterPathsByBomberId, phaseDurations, label) {
        ...
        return bomberFlights.flatMap((bomberFlight, bomberIndex) => {
            ...
            const activeDurationMs = Math.max(1, Math.round(masterPathLengthPx / HexMapRenderer.AIR_SHOW_BOMBER_SPEED_PX_PER_MS));
            ...
            const activeEndTimeMs = this.clamp(elapsedBeforeMs + phaseDurationMs - ingressDelayMs, activeStartTimeMs, activeDurationMs);
            const flightStartProgress = activeDurationMs > 0 ? this.clamp(activeStartTimeMs / activeDurationMs, 0, 1) : startProgress;
            const flightEndProgress = activeDurationMs > 0 ? this.clamp(activeEndTimeMs / activeDurationMs, flightStartProgress, 1) : endProgress;
            ...
            return this.buildAirShowFlightAssignments(bomberFlight, slicedPath, 0.22, bomberIndex, bomberFlights.length, {
                phaseStartAnchor: slicedPath[0]
            }).map((assignment) => ({
                ...assignment,
                distanceBudgetPx: slicedPathLengthPx,
                progressTimeline: phaseMovementStartMs > 1
                    ? [
                        { timeMs: 0, progress: 0 },
    ...
    resolveAirShowBomberAdvanceDestination(corridor, current, standoffPoint, distanceBudgetPx, reserveAheadPx, minimumAdvancePx = 0) {
        ...
        const targetLateralPx = plannedAdvancePx <= 0
            ? currentProjection.lateralPx
            : currentProjection.lateralPx + this.clamp((standoffProjection.lateralPx - currentProjection.lateralPx) * Math.max(0.3, lateralRatio), -maxLateralStepPx, maxLateralStepPx);
    ...
    buildAirShowBomberAdvancePlan(bomberFlights, corridor, approachProfilesByBomberId, phaseDurationMs, reserveAheadDurationMs, label, randomForLabel) {
        ...
        const assignments = bomberFlights.flatMap((bomberFlight, bomberIndex) => {
            ...
            return this.buildAirShowFlightAssignments(bomberFlight, candidatePath, 0.22, bomberIndex, bomberFlights.length).map((assignment) => ({
                ...assignment,
    ...
    resolveAirShowBomberIngressBandWaypointPlan(sceneKind) {
    ...
    resolveAirShowBomberIngressBandWaypoint(corridor, current, sceneKind, laneIndex, jitterAlongPx = 0, jitterLateralPx = 0) {
    ...
    buildAirShowBomberInterceptPassPath(start, corridor, options) {
        ...
        const pointOnCorridor = (alongPx, lateralPx) => clampPoint(this.projectAirShowCorridorPoint(corridor, alongPx, lateralPx));
        const stageAlongPx = this.clamp(currentProjection.alongPx + (options.passStartAlongPx - currentProjection.alongPx) * 0.48, Math.min(currentProjection.alongPx, options.passStartAlongPx) - 10, Math.max(currentProjection.alongPx, options.passStartAlongPx) + 10);
        const stagePoint = pointOnCorridor(stageAlongPx, laneSpreadPx + attackSideSign * 58);
        const gunPoint = pointOnCorridor(options.passStartAlongPx + passDirection * 12, laneSpreadPx + attackSideSign * 16);
        ...
        const buildStagedInterceptEntryPath = () => {
        ...
        const finalizeInterceptEntryCandidate = (candidatePath, options) => this.pruneAirShowSharpTurns(this.pruneAirShowEarlyTurnWaypoints([...candidatePath], {
            maxTurnDeg: options.maxTurnDeg,
            strongTurnDeg: options.strongTurnDeg,
            maxFirstSegmentPx: options.maxFirstSegmentPx,
            maxWaypointsToRemove: options.maxWaypointsToRemove
        }), options.sharpTurnDeg, options.sharpTurnRemovals);
        const scoreInterceptEntryPath = (candidatePath) => {
            const stats = evaluatePathEntry(candidatePath);
            return {
                path: [...candidatePath],
                score: stats.firstTurnDeg * 2.2
                    + stats.maxTurnDeg
    ...
    truncateAirShowPathByLength(points, targetPathLengthPx) {
    ...
    extendAirShowPathWithCarry(points, targetPathLengthPx, options = {}) {
        if (points.length < 2) {
            return [...points];
        }
        const currentPathLengthPx = this.measureAirShowPathLength(points);
    ...
    extendAirShowPhaseAssignmentsForSpeed(assignments, durationMs, roleTargetSpeeds, options) {
        return assignments.map((assignment) => {
            ...
            const basePoints = extendAt === "start"
                ...
                : [...assignment.points];
            const extensionMode = options.extensionModeByRole?.[assignment.actor.role]
                ?? options.extensionMode;
            const extendedPoints = extensionMode === "carry"
            ...
            return {
                ...
                progressTimeline: options.remapProgressTimelineForExtension
                    ? this.remapAirShowProgressTimelineForExtendedPath(assignment.progressTimeline, currentPathLengthPx, this.measureAirShowPathLength(extendedPoints), extendAt === "start")
                    : assignment.progressTimeline
            };
        });
    }
    remapAirShowProgressTimelineForExtendedPath(timeline, originalPathLengthPx, extendedPathLengthPx, extendedAtStart) {
        ...
        return timeline.map((keyframe) => ({
            timeMs: keyframe.timeMs,
            progress: this.clamp((addedStartLengthPx + this.clamp(keyframe.progress, 0, 1) * originalPathLengthPx)
                / extendedPathLengthPx, 0, 1)
        }));
    }
    applyAirShowAssignmentDistanceBudget(assignment, durationMs, roleTargetSpeeds) {
        ...
        const implicitBudgetPx = explicitBudgetPx === null && roleTargetSpeedPxPerMs > 0 && durationMs > 0
    ...
    applyAirShowPhaseMotionBudgets(assignments, durationMs, roleTargetSpeeds = this.resolveAirShowRoleSpeedMap()) {
        return assignments.map((assignment) => this.applyAirShowAssignmentDistanceBudget(assignment, durationMs, roleTargetSpeeds));
    }
    ...
    prepareAirShowPhaseAssignments(assignments, durationMs, progressSamplePoints, smoothEntryRadiusPx, roleTargetSpeeds = this.resolveAirShowRoleSpeedMap(), options = {}) {
        ...
        if ((options.previousAssignments?.length ?? 0) > 0) {
            finalizedAssignments = this.applyAirShowPhaseMotionBudgets(this.harmonizeAirShowPhaseEntryWithPreviousPhase(options.previousAssignments ?? [], options.previousDurationMs ?? durationMs, finalizedAssignments, durationMs, options.entryTurnLimitDeg ?? 108, new Set(options.directTurnHomeRoles ?? [])), durationMs, roleTargetSpeeds);
        }
        ...
        if (softenedEntryRoles.size > 0) {
            ...
            const softenedEntryAssignments = finalizedAssignments.map((assignment) => {
                ...
                const softenedPoints = this.softenAirShowEntryWindowTurns(assignment.points, options.softenEntryTurnLimitDeg ?? 104, {
                    maxWaypointsToRemove: 3,
                ...
                else if (softenedPoints.some((point, index) => {
        ...
        if (softenedExitRoles.size > 0) {
            ...
            const softenedExitAssignments = finalizedAssignments.map((assignment) => {
                ...
                const softenedPoints = this.softenAirShowExitWindowTurns(assignment.points, options.softenExitTurnLimitDeg ?? 104, {
                    maxWaypointsToRemove: 3,
        ...
        if ((options.previousAssignments?.length ?? 0) > 0) {
            ...
            finalizedAssignments = finalizedAssignments.map((assignment) => {
                ...
                const previousBoundary = this.resolveAirShowAssignmentBoundaryState(previousAssignment, options.previousDurationMs ?? durationMs, "end");
                if (!previousBoundary) {
                    return assignment;
                }
            ...
            if (typeof smoothEntryRadiusPx === "number") {
                finalizedAssignments = this.applyAirShowPhaseMotionBudgets(this.smoothAirShowAssignmentEntries(finalizedAssignments, this.clamp(Math.round(smoothEntryRadiusPx * 0.9), 42, 68)), durationMs, roleTargetSpeeds);
            }
        }
        if (options.sanitizeEntryTurns) {
            ...
            const sanitizedAssignments = finalizedAssignments.map((assignment) => {
                ...
                else if (sanitizedPoints.some((point, index) => {
                    const originalPoint = assignment.points[index];
                    return !originalPoint || Math.hypot(point.cx - originalPoint.cx, point.cy - originalPoint.cy) > 0.5;
                })) {
                    sanitizedAssignmentsChanged = true;
            ...
            if (sanitizedAssignmentsChanged) {
                finalizedAssignments = this.applyAirShowPhaseMotionBudgets(sanitizedAssignments, durationMs, roleTargetSpeeds);
            }
        }
    ...
    resolveAirShowPhaseDurationFromRoleSpeeds(assignments, roleTargetSpeeds, fallbackMs, minMs, maxMs, primaryRoles) {
        ...
        const candidateDurations = rolesToMeasure
            ...
            const longestPathLengthPx = relevantAssignments.reduce((longest, assignment) => Math.max(longest, this.measureAirShowPathLength(assignment.points)), 0);
    ...
    resolveAirShowAssignmentPathProgressAtTime(assignment, timeMs, durationMs, phaseProgressOverride) {
    ...
    sampleAirShowAssignmentAtTime(assignment, timeMs, durationMs, phaseProgressOverride) {
        const pathProgress = this.resolveAirShowAssignmentPathProgressAtTime(assignment, timeMs, durationMs, phaseProgressOverride);
        return {
    ...
    resolveAirShowAssignmentBoundaryState(assignment, durationMs, edge) {
        ...
        const resolvedHeadingDegrees = Math.hypot(forwardDx, forwardDy) > 0.5
    ...
    harmonizeAirShowPhaseEntryWithPreviousPhase(previousAssignments, previousDurationMs, currentAssignments, currentDurationMs, maxTurnDeg = 108, directTurnHomeRoles = new Set()) {
        const previousAssignmentsByActorId = this.buildAirShowAssignmentLookup(previousAssignments);
        const preserveBridgeEntryActorIds = new Set();
        const resolvePathEntryTurnDeg = (start, previousForward, points) => {
            const first = points.find((point) => Math.hypot(point.cx - start.cx, point.cy - start.cy) > 0.5);
            if (!first) {
                return 0;
            }
            const second = points.find((point) => point !== first && Math.hypot(point.cx - first.cx, point.cy - first.cy) > 0.5)
                ?? first;
            const startVector = {
                x: first.cx - start.cx,
                y: first.cy - start.cy
        ...
        const chooseBestEntryPath = (start, previousForward, candidatePaths) => {
        ...
        const adjustedAssignments = currentAssignments.map((assignment) => {
            ...
            let routeForward = this.normalizeAircraftVector(routeToTarget.x, routeToTarget.y, currentForward.x, currentForward.y);
            let routeAlignment = previousBoundary.forward.x * routeForward.x + previousBoundary.forward.y * routeForward.y;
            ...
            const overallRouteForward = this.normalizeAircraftVector(overallRouteToTarget.x, overallRouteToTarget.y, routeForward.x, routeForward.y);
            const overallRouteAlignment = previousBoundary.forward.x * overallRouteForward.x + previousBoundary.forward.y * overallRouteForward.y;
            if (contestedInterceptorRolesPresent
                && (overallRouteAlignment <= 0.24 || routeAlignment <= 0.18 || strongBoundaryShock)
                && directTurnHomeRoles.has(assignment.actor.role)) {
                const turnHomeCross = previousBoundary.forward.x * overallRouteToTarget.y
                    - previousBoundary.forward.y * overallRouteToTarget.x;
                const preferredTurnSign = turnHomeCross >= 0 ? 1 : -1;
                const turnHomePath = chooseBestEntryPath(alignedStartPoint, previousBoundary.forward, [preferredTurnSign, -preferredTurnSign].map((turnSign) => this.softenAirShowEntryWindowTurns(this.sanitizeAirShowEntryPath(this.buildAirShowDisengagePath(alignedStartPoint, finalTarget, {
            ...
            if (overallRouteAlignment <= -0.2 && directTurnHomeRoles.has(assignment.actor.role) && assignment.points.length >= 4) {
                bridgeTargetIndex =
                    assignment.points.length <= 4
                        ...
                        : this.clamp(Math.round((assignment.points.length - 1) * (assignment.points.length >= 7 ? 0.55 : 0.5)), 3, assignment.points.length - 1);
                bridgeTarget = assignment.points[bridgeTargetIndex] ?? bridgeTarget;
                routeToTarget = {
                    x: bridgeTarget.cx - alignedStartPoint.cx,
                    y: bridgeTarget.cy - alignedStartPoint.cy
                };
                routeForward = this.normalizeAircraftVector(routeToTarget.x, routeToTarget.y, currentForward.x, currentForward.y);
                routeAlignment =
            ...
            const carryForwardPx = preserveBridgeEntryActorIds.has(assignment.actor.id)
                ? this.clamp(Math.round(Math.max(firstSegmentPx * 0.84, startCarryDistancePx * 0.4)), 28, 84)
                : this.clamp(Math.round(Math.max(firstSegmentPx, startCarryDistancePx * 0.42)), 24, 88);
            const bridgePoints = chooseBestEntryPath(alignedStartPoint, previousBoundary.forward, [preferredBridgeSign, -preferredBridgeSign].map((bridgeSign) => this.buildAirShowPhaseEntryBridge(alignedStartPoint, bridgeTarget, {
                startHeadingDegrees: previousBoundary.headingDegrees,
                sideSign: bridgeSign,
                carryForwardPx
            })))
            ...
            const resolvedAdjustedPoints = preserveBridgeEntryActorIds.has(assignment.actor.id)
                ? this.pruneAirShowEntryWindowSharpTurns(adjustedPoints, isDirectTurnHomeRole ? 108 : 118, isDirectTurnHomeRole ? 3 : 2, isDirectTurnHomeRole ? 5 : 4)
                : adjustedPoints;
        ...
        return adjustedAssignments.map((assignment) => preserveBridgeEntryActorIds.has(assignment.actor.id)
            ? assignment
     ...
     */
    resolveAirShowPhaseSpacing(assignments, durationMs, progressSamplePoints = [0.3, 0.5, 0.7] // Check spacing at multiple points along paths
    ) {
        ...
        for (const progress of progressSamplePoints) {
            ...
            resolvedAssignments.forEach((assignment, index) => {
                const sampled = this.sampleAirShowAssignmentAtTime(assignment, progress * durationMs, durationMs);
    ...
    resolveAirShowTracerBurstGeometry(actor, burst, targetPoint) {
        ...
        const headingForward = this.normalizeAircraftVector(Math.cos(((sourceHeadingDegrees - 90) * Math.PI) / 180), Math.sin(((sourceHeadingDegrees - 90) * Math.PI) / 180), 0, -1);
        const targetDirectedForward = targetPoint
            ? this.normalizeAircraftVector(targetPoint.cx - emitterPoint.cx, targetPoint.cy - emitterPoint.cy, headingForward.x, headingForward.y)
            : null;
        ...
        const segments = Array.from({ length: burstCount }, (_, index) => {
            ...
            const forwardBiasPx = typeof targetDistancePx === "number" && burstCount > 1
            ...
            const segmentLengthPx = this.clamp(typeof targetPoint === "object" && targetPoint
                ? Math.min(streakLengthPx * (0.88 - Math.abs(fanT) * 0.08), Math.max(14, Math.hypot(targetPoint.cx - start.cx, targetPoint.cy - start.cy) * 0.95))
                : streakLengthPx, 14, Math.max(18, streakLengthPx));
            return {
                start,
                end: {
    ...
    buildAirShowTracerVolley(source, target, options = {}) {
        const timingCount = Array.isArray(options.timings) && options.timings.length > 0
    ...
    sampleAirShowFlightCentroidAtProgress(flight, assignments, progress) {
        const assignmentsByActorId = this.buildAirShowAssignmentLookup(assignments);
        ...
        return timings.flatMap((progress, volleyIndex) => {
            ...
            if (!pair && options.fallbackToNearest) {
                ...
                if (sourceActor && sourceCentroid && targetCentroid) {
                    const fallbackTarget = options.fallbackTarget === "midpoint"
                        ? {
    ...
    resolveAirShowBomberDefensePassAttackEntries(bomberAttackEntries, interceptorFlights, survivingBombers) {
        const activeInterceptors = interceptorFlights.filter((flight) => flight.actors.some((actor) => actor.active));
        if (activeInterceptors.length === 0 || survivingBombers.length === 0) {
            return [];
        }
        const resolveBomberLossScore = (flight) => {
        ...
        const prioritizedBombers = [...survivingBombers].sort((left, right) => resolveBomberLossScore(right) - resolveBomberLossScore(left));
        const resolveFlightCurrentPoint = (flight) => this.averageAirShowPosition(flight.actors) ?? flight.anchor;
        const resolvePreferredBomberFlight = (interceptorFlight, candidateBombers, fallbackBombers) => {
            ...
            return bomberPool.reduce((best, candidate) => {
                ...
                const bestDistancePx = Math.hypot(bestPoint.cx - interceptorPoint.cx, bestPoint.cy - interceptorPoint.cy);
                const candidateDistancePx = Math.hypot(candidatePoint.cx - interceptorPoint.cx, candidatePoint.cy - interceptorPoint.cy);
        ...
        const usedInterceptorIds = new Set();
        const assignedBomberIds = new Set();
        activeAttackEntries
            .sort((left, right) => resolveBomberLossScore(right.bomberFlight) - resolveBomberLossScore(left.bomberFlight))
            .forEach((entry) => {
        ...
        const unusedInterceptors = activeInterceptors.filter((flight) => !usedInterceptorIds.has(flight.spec.id));
        const remainingUnassignedBombers = prioritizedBombers.filter((flight) => !assignedBomberIds.has(flight.spec.id));
        unusedInterceptors.forEach((interceptorFlight) => {
            const bomberFlight = resolvePreferredBomberFlight(interceptorFlight, remainingUnassignedBombers, prioritizedBombers);
            if (!bomberFlight) {
                return;
            }
            resolvedEntries.push({
            ...
            const remainingBomberIndex = remainingUnassignedBombers.findIndex((flight) => flight.spec.id === bomberFlight.spec.id);
        ...
        return activeInterceptors.map((interceptorFlight) => {
            const bomberFlight = resolvePreferredBomberFlight(interceptorFlight, remainingFallbackBombers, prioritizedBombers)
                ?? prioritizedBombers[0];
            const remainingBomberIndex = remainingFallbackBombers.findIndex((flight) => flight.spec.id === bomberFlight.spec.id);
            if (remainingBomberIndex >= 0) {
    ...
    buildAirShowBomberDefensePassTracerBursts(assignments, interceptorFlight, bomberFlight, options = {}) {
    ...
    resolveAirShowFlakBurstWave(corridor, targetCenter, burst) {
        ...
        const smokePuffCount = isSinglePuff
            ...
            : Math.max(Math.round(puffCount * 0.45), Math.min(Math.max(puffCount + 3, Math.round(puffCount * 1.2)), requestedSmokePuffCount));
        ...
        let points = Array.from({ length: puffCount }, (_, index) => {
            ...
            const alongJitter = Math.cos(angle) * alongSpreadPx * (0.14 + radial * 0.64)
                + (nextRandom() - 0.5) * alongSpreadPx * 0.16;
            const lateralJitter = Math.sin(angle) * lateralSpreadPx * (0.18 + radial * 0.72)
    ...
    async runAirShowPhase(assignments, durationMs, tracerBursts = [], options = {}) {
        ...
        assignments.forEach((assignment) => {
            ...
            assignment.actor.headingDegrees = this.interpolateAircraftHeadingDegrees(assignment.actor.headingDegrees, initialSample.headingDegrees, 0.68);
            assignment.actor.position = {
        ...
        await new Promise((resolve) => {
            ...
            const step = (now) => {
                ...
                assignments.forEach((assignment) => {
                    const phaseProgressOverride = assignment.progressTimeline && assignment.progressTimeline.length >= 2
                        ? undefined
                        : easedProgress;
                    const sample = this.sampleAirShowAssignmentAtTime(assignment, elapsed, durationMs, phaseProgressOverride);
    ...
    syncAirShowPhaseVisibility(assignments, sceneActors = [], visibleActorIds) {
    ...
    playAirShowFlakWave(wave, scale = 1.08, _smokeScale = 0.92) {
        ...
        const waveSeed = wave.points.reduce((seed, point, index) => (seed
            + Math.round(point.cx * 17)
            + Math.round(point.cy * 23)
            + index * 1013904223
            + wave.puffCount * 97
            + wave.flashCount * 131) >>> 0, 2166136261);
        const jitter01 = (index, salt) => {
        ...
        for (let index = 0; index < pointCount; index += 1) {
            ...
            if (!singlePuffWave && index < smokePointCount) {
                window.setTimeout(() => {
                    void this.playCombatAnimationAt("flakSmokePuff", point.cx + (jitter01(index, 37) - 0.5) * 18, point.cy - 5 + (jitter01(index, 41) - 0.5) * 14, _smokeScale * (0.88 + jitter01(index, 43) * 0.2), false, undefined, false);
                }, flashDelayMs + 220 + Math.round(jitter01(index, 47) * 140));
                if (jitter01(index, 53) > 0.88) {
                    window.setTimeout(() => {
                        void this.playCombatAnimationAt("flakSmokePuff", point.cx + (jitter01(index, 59) - 0.5) * 24, point.cy - 8 + (jitter01(index, 61) - 0.5) * 18, _smokeScale * (0.76 + jitter01(index, 67) * 0.2), false, undefined, false);
...
HexMapRenderer.AIR_SHOW_FIGHTER_SPEED_PX_PER_MS = AIR_SHOW_POLICY_FIGHTER_SPEED_PX_PER_MS;
HexMapRenderer.AIR_SHOW_BOMBER_STANDOFF_DISTANCE_PX = HEX_HEIGHT * 2;
