 ...
 */
const slicedFrameCache = new Map();
function validateLeadingFrameUniqueness(sourceLabel, frameDataUrls) {
    if (frameDataUrls.length < 3) {
        return;
    }
    const leadingFrames = frameDataUrls.slice(0, 3);
    if (new Set(leadingFrames).size === 1) {
        throw new Error(`[SpriteSheet] Leading cached frames for ${sourceLabel} resolved to identical encoded outputs. ` +
...
//   }
// }
export async function sliceSpriteSheet(image, columns, rows, frameCount, anchorX = 0.5, anchorY = 0.5) {
    ...
    if (!Number.isInteger(sourceCellWidth) || !Number.isInteger(sourceCellHeight)) {
        const error = new Error(`[SpriteSheet] ❌ ASSET/SPEC MISMATCH - Image dimensions ${image.naturalWidth}×${image.naturalHeight} ` +
    ...
    if (frameCount > maxFrames) {
        const error = new Error(`[SpriteSheet] ❌ ASSET/SPEC MISMATCH - frameCount=${frameCount} exceeds grid capacity of ${columns}×${rows}=${maxFrames} cells.`);
        console.error(error.message);
        throw error;
    }
    ...
    for (let i = 0; i < frameCount; i++) {
        ...
        if (canvas.width !== 300 || canvas.height !== 150) {
            throw new Error(`[SpriteSheet] Frame ${i} canvas is not pristine. ` +
    ...
    console.log(`[SpriteSheet] Sliced ${frameCount} frames at ${actualFrameWidth}x${actualFrameHeight}px each (1:1 from source, no scaling); ` +
...
function largeExplosionFrameDuration(frameIndex) {
    if (frameIndex < 3)
        return 40;
    if (frameIndex < 9)
        return 58;
...
function smallExplosionFrameDuration(frameIndex) {
    if (frameIndex < 4)
        return 34;
    if (frameIndex < 10)
        return 46;
...
function impactHitsFrameDuration(frameIndex) {
    if (frameIndex < 4)
        return 18;
    if (frameIndex < 10)
        return 24;
 ...
 */
export function resolveSpriteSheetSpec(spec, imageDimensions) {
 ...
 */
export function getSpriteSheetFrameDuration(spec, frameIndex, totalFrames) {
 ...
 */
export function getSpriteSheetFrameOpacity(spec, frameIndex, totalFrames) {
...
const spriteSheetImageCache = new Map();
export function loadSpriteSheetImage(imagePath) {
...
function requiresLoadedImage(spec) {
    return spec.frameWidth == null || spec.frameHeight == null;
}
export async function resolveSpriteSheetSpecAsync(spec) {
...
export class SpriteSheetAnimation {
     ...
     */
    configure(spec, frames, svgParent, x, y, scale = 1) {
...
export class SpriteSheetAnimator {
     ...
     */
    async ensureSlicedFrames(spec) {
        const cacheKey = spec.imagePath;
        const cached = slicedFrameCache.get(cacheKey);
        if (cached) {
            return cached;
        }
        const pending = loadSpriteSheetImage(spec.imagePath).then((asset) => sliceSpriteSheet(asset.image, spec.columns, spec.rows, spec.frameCount, spec.anchorX, spec.anchorY)).catch((error) => {
