...
const allocationEntries = allocationOptions;
/**
 * Mapping helper that enables constant-time lookups by key during quantity adjustments.
 */
export const ALLOCATION_BY_KEY = Object.freeze(Object.fromEntries(allocationEntries.map((option) => [option.key, option])));
/**
...
const allocationKeySet = new Set(allocationEntries.map((option) => option.key));
/**
 * Runtime guard shielding downstream callers from typos when receiving user input or parsing saves.
 */
export function isAllocationKey(value) {
