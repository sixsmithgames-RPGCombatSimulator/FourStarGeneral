 ...
 */
const CAMPAIGN_GENERATED_PROFILE = {
...
export function validateScenarioSource(source, missionKey) {
    ...
    const isCampaignGenerated = typeof record?.["campaignTemplateKey"] === "string";
    ...
    if (profile && !profile.allowedMissionKeys.includes(missionKey)) {
        issues.push(`Scenario ${scenarioName} is not approved for mission key ${missionKey}. Allowed mission keys: ${profile.allowedMissionKeys.join(", ")}.`);
    }
...
export function assertScenarioSourceValid(source, missionKey) {
    ...
    throw new Error(`[scenarioValidation] Validation failed for mission ${missionKey} (${scenarioLabel}):\n- ${result.issues.join("\n- ")}`);
}
function validateObjectives(record, issues, missionKey, scenarioName, size, profile) {
    ...
    if (profile && objectivesValue.length < profile.minObjectiveCount) {
        issues.push(`Scenario ${scenarioName ?? missionKey} only defines ${objectivesValue.length} objectives; at least ${profile.minObjectiveCount} are required.`);
    }
    const objectiveAxials = [];
    ...
    if (Number.isFinite(minSpacing) && minSpacing < profile.minObjectiveSpacing) {
        issues.push(`Scenario ${scenarioName ?? missionKey} clusters objectives too tightly. Minimum spacing is ${minSpacing}; require at least ${profile.minObjectiveSpacing}.`);
    }
}
function validateDeploymentZones(record, issues, missionKey, scenarioName, size, tilePalette, tiles) {
    ...
    const planningScenario = size && tilePalette && tiles
        ? {
            size,
    ...
    zonesValue.forEach((zone, index) => {
        ...
        if (capacity !== null && usableHexCount < capacity) {
            issues.push(`Deployment zone ${zoneKey} in scenario ${scenarioName ?? missionKey} declares capacity ${capacity} but only ${usableHexCount} usable hexes.`);
        }
            ...
            if (planningScenario && zoneLabel && capacity !== null && (zoneFaction === "Player" || zoneFaction === "Bot" || zoneFaction === "Ally")) {
                try {
                    const finalizedZone = finalizeDeploymentZone({
                        ...
                        capacity,
                        faction: zoneFaction,
                        hexes: hexes.map((hex) => [readInteger(hex[0]) ?? -1, readInteger(hex[1]) ?? -1])
                    ...
                    if (zoneDoctrine && finalizedZone.capacity < zoneDoctrine.minimumCapacity) {
                        issues.push(`Deployment zone ${zoneKey} in scenario ${scenarioName ?? missionKey} exposes capacity ${finalizedZone.capacity}; require at least ${zoneDoctrine.minimumCapacity}.`);
                    }
                    if (zoneDoctrine && metrics.frontage < zoneDoctrine.minimumFrontage) {
                        issues.push(`Deployment zone ${zoneKey} in scenario ${scenarioName ?? missionKey} exposes frontage ${metrics.frontage}; require at least ${zoneDoctrine.minimumFrontage}.`);
                    }
                    if (zoneDoctrine && metrics.depth < zoneDoctrine.minimumDepth) {
                        issues.push(`Deployment zone ${zoneKey} in scenario ${scenarioName ?? missionKey} exposes depth ${metrics.depth}; require at least ${zoneDoctrine.minimumDepth}.`);
                    }
                }
                catch (error) {
    ...
    if (playerCapacityTotal < deploymentProfile.validation.minimumPlayerZoneCapacityTotal) {
        issues.push(`Scenario ${scenarioName ?? missionKey} exposes player deployment capacity ${playerCapacityTotal}; require at least ${deploymentProfile.validation.minimumPlayerZoneCapacityTotal}.`);
    }
    if (maxPlayerFrontage < deploymentProfile.validation.minimumPlayerZoneFrontage) {
        issues.push(`Scenario ${scenarioName ?? missionKey} exposes player deployment frontage ${maxPlayerFrontage}; require at least ${deploymentProfile.validation.minimumPlayerZoneFrontage}.`);
    }
...
function validateRangeEnvelope(record, issues, missionKey, scenarioName, size, profile) {
    ...
    if (size.cols < profile.minCols) {
        issues.push(`Scenario ${scenarioName ?? missionKey} width ${size.cols} is below the profile minimum ${profile.minCols}.`);
    }
    if (size.rows < profile.minRows) {
        issues.push(`Scenario ${scenarioName ?? missionKey} depth ${size.rows} is below the profile minimum ${profile.minRows}.`);
    }
}
function readScenarioSize(record, issues, missionKey, scenarioName) {
...
function readTilePalette(record, issues, missionKey, scenarioName) {
...
function readTileRows(record, issues, missionKey, scenarioName, size) {
    ...
    rows.forEach((row, rowIndex) => {
        ...
        if (size && row.length !== size.cols) {
            issues.push(`Tile row ${rowIndex} in scenario ${scenarioName ?? missionKey} declares ${row.length} columns; expected ${size.cols}.`);
        }
        normalizedRows.push(row);
    });
    return normalizedRows;
}
function validateTileCoverage(issues, missionKey, scenarioName, size, tilePalette, tiles) {
    ...
    for (let row = 0; row < size.rows; row += 1) {
        ...
        for (let col = 0; col < size.cols; col += 1) {
            ...
            if (!readString(entryRecord["terrain"]) || !readString(entryRecord["terrainType"])) {
                issues.push(`Scenario ${scenarioLabel} tile [${row}][${col}] must reference a palette key or declare terrain and terrainType.`);
            }
        }
    }
}
function resolveTerrainTypeAt(col, row, tiles, tilePalette) {
...
function isWithinBounds(col, row, size) {
