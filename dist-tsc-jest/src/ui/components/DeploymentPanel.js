...
export class DeploymentPanel {
    constructor() {
        ...
        this.zoneMetaMap = new Map();
        this.zoneHexLookup = new Map();
        this.hexZoneIndex = new Map();
    ...
    bindReserveEvents() {
        this.reserveList.addEventListener("click", (event) => {
            ...
            const deployBtn = target.closest(".reserve-deploy");
            if (deployBtn) {
                const item = deployBtn.closest(".reserve-entry[data-unit-key]");
            ...
            const item = target.closest(".reserve-entry[data-unit-key]");
            if (!item || item.hasAttribute("data-deployment-disabled")) {
                return;
            }
            const unitKey = item.getAttribute("data-unit-key");
    ...
    renderDeploymentStatus() {
        ...
        if (this.queuedUnitKey && !this.selectedHexKey) {
            const queuedLabel = deploymentState.pool.find((entry) => entry.key === this.queuedUnitKey)?.label ?? "Queued unit";
     ...
     */
    renderUnitListItem(entry, total, locked) {
        const deploymentState = ensureDeploymentState();
     ...
     */
    renderUnitThumbnail(label, spritePath) {
        if (spritePath) {
    ...
    resolveStatusLabel(remaining, deployed, total) {
        if (remaining <= 0) {
            return "Exhausted";
        }
    ...
    handlePanelClick(event) {
        const target = event.target;
        const unitItem = target.closest("[data-unit-key]");
        ...
        const recallButton = target.closest("[data-recall-hex]");
        if (recallButton) {
            if (this.interactionsLocked) {
                return;
            }
        ...
        const zoneItem = target.closest("[data-zone-key]");
     ...
     */
    deployQueuedUnitIfReady(hexKey) {
