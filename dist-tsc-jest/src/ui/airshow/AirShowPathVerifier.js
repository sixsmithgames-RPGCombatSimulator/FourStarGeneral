...
function pointToSegmentDistance(point, segmentStart, segmentEnd) {
    ...
    const t = Math.max(0, Math.min(1, ((point.cx - segmentStart.cx) * dx +
        (point.cy - segmentStart.cy) * dy) / segmentLengthSq));
 ...
 */
export function verifyAssignmentPath(assignment, durationMs, sampleIntervalMs, maxAllowedDeviationPx, sampler) {
    const samples = [];
    const deviations = [];
    ...
    for (let i = 0; i <= sampleCount; i++) {
        ...
        if (deviationPx > maxAllowedDeviationPx) {
            deviations.push({
                timeMs,
                expectedPosition: assignment.points[Math.floor((timeMs / durationMs) * (assignment.points.length - 1))] ?? assignment.points[0],
 ...
 * @param sampler - Function to sample position at a given time
 */
export function verifyFighterConvergence(assignments, fightSpaceCenter, phaseDurationMs, sampleIntervalMs, convergenceThresholdPx, sampler) {
    const fighterAssignments = assignments.filter(a => a.actor.role === "interceptor" || a.actor.role === "escort");
    const convergenceTimes = new Map();
    ...
    for (const assignment of fighterAssignments) {
        ...
        for (let i = 0; i <= sampleCount; i++) {
            ...
            const distanceToCenter = Math.hypot(sample.position.cx - fightSpaceCenter.cx, sample.position.cy - fightSpaceCenter.cy);
