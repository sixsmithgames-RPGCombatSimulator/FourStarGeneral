...
function getEventInterceptors(event) {
    return Array.isArray(event.interceptors)
        ? (event.interceptors)
        : [];
}
function getEventEscorts(event) {
    return Array.isArray(event.escorts)
        ? (event.escorts)
        : [];
}
function updateStrengthFloor(current, candidate) {
    if (typeof candidate !== "number" || !Number.isFinite(candidate)) {
        return current;
    }
    return Math.min(current, candidate);
}
function buildExchangeKey(exchange) {
    return [
...
function createMutableFlightSpec(unitKey, scenarioType, faction, role, combatRole, originHexKey, strength, laneOffsetPx) {
    return {
        id: unitKey,
        scenarioType,
        faction,
...
function toResolvedFlightSpec(spec) {
    return {
        id: spec.id,
...
function findMatchingEventForFlight(flight, events, type) {
    return (events.find((event) => event.type === type && event.missionId === flight.missionId)
        ?? events.find((event) => event.type === type && event.bomber.unitKey === flight.unitKey)
        ?? null);
}
function toResolvedStrikeFlightSpec(entry, options) {
    ...
    return {
        ...
        originHexKey,
        strengthBefore: airToAirEvent?.bomberStrengthBefore
...
function resolveCoordinatedBomberStartDelayMs(fighterScenePresent, strikePlanCount, options) {
    ...
    const fighterIngressLeadMs = Math.max(configuredLeadMs, Math.round(options.fighterIngressDurationMs +
...
export function buildCoordinatedAirClusterPlaybackPlan(cluster, options) {
    if (cluster.length === 0) {
        return null;
    }
    const linkedStrikeOperations = cluster.filter((operation) => operation.kind === "linkedStrike");
    const standaloneStrikeOperations = cluster.filter((operation) => operation.kind === "flight" && operation.flight.kind === "strike");
    const standaloneEventOperations = cluster.filter((operation) => operation.kind === "event");
    ...
    const hasEscortFighterBattle = candidateCombatEvents.some((event) => getEventInterceptors(event).length > 0 && getEventEscorts(event).length > 0);
    const shouldCoordinate = strikeOperationCount > 1
    ...
    const flightsByUnitKey = new Map();
    ...
    const interceptorSpecs = new Map();
    const escortSpecs = new Map();
    const claimedOperationIndices = new Set();
    const upsertSceneFlight = (map, args) => {
        const current = map.get(args.unitKey);
        if (!current) {
            map.set(args.unitKey, createMutableFlightSpec(args.unitKey, args.scenarioType, args.faction, args.role, args.combatRole, args.originHexKey, args.strengthBefore, args.laneOffsetPx));
        }
        ...
        next.strengthAfterEscortPhase = updateStrengthFloor(next.strengthAfterEscortPhase, args.strengthAfterEscortPhase);
        next.finalStrength = updateStrengthFloor(next.finalStrength, args.finalStrength);
    };
    candidateCombatEvents.forEach((event) => {
        const ownerOperation = cluster.find((operation) => operation.kind === "linkedStrike"
        ...
        getEventInterceptors(event).forEach((participant, index) => {
            ...
            const strengthBefore = participant.strength
            ...
            upsertSceneFlight(interceptorSpecs, {
                ...
                role: "interceptor",
                combatRole: "cap",
                originHexKey: linkedFlight?.originKey
                    ?? options.resolveOriginKey(participant.unitKey, participant.faction),
                strengthBefore,
                strengthAfterEscortPhase: event.interceptorStrengthsAfterEscortPhase?.[index] ?? strengthBefore,
                finalStrength: event.interceptorFinalStrengths?.[index] ?? strengthBefore,
        ...
        getEventEscorts(event).forEach((participant, index) => {
            ...
            upsertSceneFlight(escortSpecs, {
                ...
                originHexKey: linkedFlight?.originKey
                    ?? options.resolveOriginKey(participant.unitKey, participant.faction),
                strengthBefore,
                strengthAfterEscortPhase: event.escortStrengthsAfterEscortPhase?.[index] ?? strengthBefore,
                finalStrength: event.escortFinalStrengths?.[index] ?? strengthBefore,
                laneOffsetPx: linkedFlight?.laneOffsetPx ?? 0
            });
        });
    });
    ...
    linkedStrikeOperations.forEach((operation) => {
        operation.escorts.forEach((escort) => {
            ...
            const fallbackStrength = escort.strength ?? options.resolveStrength(escort.unitKey, escort.faction);
            upsertSceneFlight(escortSpecs, {
    ...
    const combinedEscortExchanges = Array.from(new Map(candidateCombatEvents
        .flatMap((event) => (event.escortExchanges ?? []))
        .map((exchange) => [buildExchangeKey(exchange), exchange])).values());
    const coordinatedStrikeEntries = [
        ...linkedStrikeOperations.map((operation) => {
            claimedOperationIndices.add(operation.index);
            const airToAirEvent = operation.linkedEvents.find((event) => event.type === "airToAir") ?? null;
            const flakEvent = operation.linkedEvents.find((event) => event.type === "flak") ?? null;
            return {
                flight: operation.flight,
                airToAirEvent,
                flakEvent,
                destroyedBeforeTarget: airToAirEvent?.bomberDestroyed === true || flakEvent?.bomberDestroyed === true
            };
        }),
    ...
    const coordinatedBombers = coordinatedStrikeEntries.map((entry) => toResolvedStrikeFlightSpec(entry, options));
    const combinedBomberPassExchanges = Array.from(new Map(coordinatedStrikeEntries
        .flatMap((entry) => (entry.airToAirEvent?.bomberPassExchanges ?? []))
        .map((exchange) => [buildExchangeKey(exchange), exchange])).values());
    const combinedFlakBursts = coordinatedStrikeEntries.flatMap((entry) => buildResolvedAirShowFlakBursts(entry.flakEvent, {
    ...
    const scene = fighterScenePresent || coordinatedBombers.length > 0
        ? {
            kind: coordinatedBombers.length > 0 || candidateCombatEvents.some((event) => event.type === "airToAir")
                ? "airToAir"
                : "capClash",
    ...
    const { bomberStartDelayMs, fighterIngressLeadMs } = resolveCoordinatedBomberStartDelayMs(fighterScenePresent, coordinatedBombers.length, options);
