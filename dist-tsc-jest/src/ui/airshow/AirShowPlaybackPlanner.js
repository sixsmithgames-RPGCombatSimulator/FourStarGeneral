    ...
    const buildCorridorContestedAirShowPlan = () => {
        ...
        const resolveContinuousFlakBursts = (assignments, durationMs, phase) => {
            ...
            return bomberPlans.flatMap((plan) => {
                ...
                return scoped.flatMap((burst, burstIndex) => {
                    ...
                    const puffCount = Math.max(11, Math.min(16, Math.round(basePuffCount <= 1 ? (phase === "approach" ? 11 : 12) : basePuffCount * 0.68)));
                    const requestedSmokePuffCount = burst.smokePuffCount ?? Math.round(puffCount * 1.45);
                    const smokePuffCount = Math.max(puffCount + 3, Math.min(24, Math.round(requestedSmokePuffCount)));
                    const jitteredProgress = host.clamp(inRangeProgress + (jitter01(sampleIndex, burstIndex + 1) - 0.5) * (phase === "approach" ? 0.026 : 0.022), 0.08, phase === "approach" ? maxProgress : 0.84);
                    return [{
                            ...burst,
                            ...
                            puffCount,
                            smokePuffCount,
                            alongOffsetPx: host.clamp((burst.alongOffsetPx ?? 0) * 0.55 - 44 + jitter01(sampleIndex, burstIndex + 11) * 122, -110, 110),
                            lateralOffsetPx: host.clamp((burst.lateralOffsetPx ?? 0) * 0.5
                                + (jitter01(sampleIndex, burstIndex + 23) - 0.5) * 310
                                + (burstIndex - (scopedCount - 1) / 2) * 5, -210, 210),
                            alongSpreadPx: Math.max(58, Math.min(104, Math.round((burst.alongSpreadPx ?? 64) * (0.9 + jitter01(sampleIndex, burstIndex + 29) * 0.24)))),
                            lateralSpreadPx: Math.max(132, Math.min(178, Math.round((burst.lateralSpreadPx ?? 126) * (1.02 + jitter01(sampleIndex, burstIndex + 37) * 0.28)))),
        ...
        const resolveCorridorPhaseDurationMs = (label, assignments) => {
            ...
            const requiredRoles = fighterRolesForTimedPhase
                ?? (assignments.some((assignment) => assignment.actor.role === "bomber")
                    ...
                    ? ["bomber"]
                    : undefined);
            return host.resolveAirShowPhaseDurationFromRoleSpeeds(assignments, roleSpeeds, seedDurationByPhase.get(label) ?? 1, 1, 60000, requiredRoles);
        };
        ...
        const alignClashFightersThroughSharedFightSpace = (assignments, durationMs, fightSpaceCenter, convergenceTimeProgress) => {
            ...
            return assignments.map((assignment) => {
                ...
                const previousHeading = previousAssignment
                    ...
                    : null;
                const convergencePoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.axis.x * roleSide * 6 + corridor.normal.x * lane * 10, corridor.axis.y * roleSide * 6 + corridor.normal.y * lane * 10);
                ...
                const entryPoint = host.offsetAirShowPoint(carryPoint, (convergencePoint.cx - carryPoint.cx) * 0.44 + corridor.normal.x * roleSide * 10, (convergencePoint.cy - carryPoint.cy) * 0.44 + corridor.normal.y * roleSide * 10);
                const preMergePoint = host.offsetAirShowPoint(entryPoint, (convergencePoint.cx - entryPoint.cx) * 0.55 + corridor.normal.x * lane * 4, (convergencePoint.cy - entryPoint.cy) * 0.55 + corridor.normal.y * lane * 4);
                const exitPoint = host.offsetAirShowPoint(convergencePoint, (end.cx - convergencePoint.cx) * 0.38 + corridor.normal.x * roleSide * 10, (end.cy - convergencePoint.cy) * 0.38 + corridor.normal.y * roleSide * 10);
        ...
        const finalizeCorridorPhaseAssignments = (label, assignments, durationMs) => {
            ...
            const resolvedTimedAssignments = label === "escort-clash-merge"
                ? alignClashFightersThroughSharedFightSpace(preparedAssignments, durationMs, projectAirShowRailPoint(corridor, -420, 0), 0.54)
                : label === "escort-clash-scramble"
            ...
            const phasePatternAlignedAssignments = label === "bomber-defense-pass"
                ? finalSpeedAdjustedAssignments.map((assignment) => {
                    ...
                    const interceptFocus = host.sampleAirShowAssignmentAtTime(targetBomberAssignment, durationMs * 0.5, durationMs).position;
                    const exitFocus = host.sampleAirShowAssignmentAtTime(targetBomberAssignment, durationMs * 0.86, durationMs).position;
                    const interceptPoint = host.offsetAirShowPoint(interceptFocus, corridor.normal.x * lateralOffsetPx - corridor.axis.x * 32, corridor.normal.y * lateralOffsetPx - corridor.axis.y * 32);
                    const entryPoint = host.offsetAirShowPoint(start, (interceptPoint.cx - start.cx) * 0.46 + corridor.normal.x * lateralOffsetPx * 0.34, (interceptPoint.cy - start.cy) * 0.46 + corridor.normal.y * lateralOffsetPx * 0.34);
                    const exitPoint = host.offsetAirShowPoint(exitFocus, corridor.normal.x * lateralOffsetPx * 0.58 + corridor.axis.x * 150, corridor.normal.y * lateralOffsetPx * 0.58 + corridor.axis.y * 150);
                    return {
                        ...assignment,
