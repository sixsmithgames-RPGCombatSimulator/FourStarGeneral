    ...
    const buildCorridorContestedAirShowPlan = () => {
        ...
        const alignClashFightersThroughSharedFightSpace = (assignments, durationMs, fightSpaceCenter, convergenceTimeProgress) => {
            ...
            return assignments.map((assignment) => {
                ...
                const previousHeading = previousAssignment
                    ...
                    : null;
                const convergencePoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.axis.x * roleSide * 8 + corridor.normal.x * (lane * 20 + actorSlot * 9), corridor.axis.y * roleSide * 8 + corridor.normal.y * (lane * 20 + actorSlot * 9));
                ...
                const entryPoint = host.offsetAirShowPoint(carryPoint, (convergencePoint.cx - carryPoint.cx) * 0.36
                    + corridor.normal.x * (roleSide * 38 + lane * 14 + actorSlot * 8), (convergencePoint.cy - carryPoint.cy) * 0.36
                ...
                const preMergePoint = host.offsetAirShowPoint(entryPoint, (convergencePoint.cx - entryPoint.cx) * 0.62
                    + corridor.normal.x * (lane * 12 - roleSide * 16 + actorSlot * 6), (convergencePoint.cy - entryPoint.cy) * 0.62
                    + corridor.normal.y * (lane * 12 - roleSide * 16 + actorSlot * 6));
                const breakPoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.axis.x * (42 + lane * 8 + actorSlot * 5)
                    + corridor.normal.x * (lane * 30 - roleSide * 72 + actorSlot * 14), corridor.axis.y * (42 + lane * 8 + actorSlot * 5)
                    + corridor.normal.y * (lane * 30 - roleSide * 72 + actorSlot * 14));
                const rejoinPoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.axis.x * (122 + lane * 12)
                    + corridor.normal.x * (roleSide * 34 + lane * 6 + actorSlot * 12), corridor.axis.y * (122 + lane * 12)
                    + corridor.normal.y * (roleSide * 34 + lane * 6 + actorSlot * 12));
                const exitPoint = host.offsetAirShowPoint(rejoinPoint, (end.cx - rejoinPoint.cx) * 0.42, (end.cy - rejoinPoint.cy) * 0.42);
        ...
        const alignScrambleFightersThroughChasePocket = (assignments, durationMs, fightSpaceCenter) => {
            ...
            return assignments.map((assignment) => {
                ...
                const carryPoint = host.offsetAirShowPoint(start, currentForward.x * 66, currentForward.y * 66);
                const escortConvergenceScale = assignment.actor.role === "escort" ? 0.18 : 1;
                const pocketNormalPx = (lane * 14 + roleSide * 8 + actorSlot * 5) * escortConvergenceScale;
                const pocketAxisPx = (roleSide * 4 + actorSlot * 2) * escortConvergenceScale;
                const sweepAxisPx = (50 + lane * 7) * (assignment.actor.role === "escort" ? 0.82 : 1);
                const sweepNormalPx = (lane * 24 - roleSide * 30 + actorSlot * 8) * (assignment.actor.role === "escort" ? 0.24 : 1);
                ...
                const entryPoint = host.offsetAirShowPoint(carryPoint, (pocketPoint.cx - carryPoint.cx) * 0.58
                    + corridor.normal.x * roleSide * (assignment.actor.role === "escort" ? 0 : 12), (pocketPoint.cy - carryPoint.cy) * 0.58
                    + corridor.normal.y * roleSide * (assignment.actor.role === "escort" ? 0 : 12));
                const exitPoint = host.offsetAirShowPoint(sweepPoint, (end.cx - sweepPoint.cx) * 0.42, (end.cy - sweepPoint.cy) * 0.42);
