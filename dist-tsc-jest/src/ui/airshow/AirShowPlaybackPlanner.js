...
export function planResolvedAirCombatShowScene(host, scene) {
    ...
    const buildBomberEgressAssignments = (bomberFlightsForPhase) => bomberFlightsForPhase.flatMap((flight, index) => {
        ...
        const continuityPoint = host.offsetAirShowPoint(current, headingForward.x * 96 + corridor.normal.x * (rand() - 0.5) * 10, headingForward.y * 96 + corridor.normal.y * (rand() - 0.5) * 10);
    ...
    const buildCorridorContestedAirShowPlan = () => {
        ...
        const resolveBomberPhasePath = (plan, label) => {
            ...
            const startDistancePx = host.clamp(currentDistancePx, 0, lengthPx);
            ...
            const endDistancePx = host.clamp(desiredEndDistancePx, startDistancePx, lengthPx);
        ...
        const rebuildFighterIngressPathToMergeEndpoint = (assignment, durationMs) => {
            ...
            const sideSign = assignment.actor.formationIndex % 2 === 0 ? -1 : 1;
            const pairIndex = Math.floor(assignment.actor.formationIndex / 2);
            const roleLeadPx = assignment.actor.role === "interceptor" ? -70 : 72;
            const roleLateralBiasPx = assignment.actor.role === "interceptor" ? -48 : 48;
            const slotLeadPx = sideSign * 8;
            const slotLateralPx = sideSign * (38 + pairIndex * 9);
            ...
            let points = assignment.actor.role === "interceptor"
                ? (() => {
                    ...
                    const bendLateralPx = host.clamp(directDistancePx * 0.17, 78, 215);
                    return dedupePath([
                        start,
                        {
            ...
            const maxTravelPx = Math.max(96, durationMs * roleSpeedPxPerMs * 16);
            if (pathLengthPx > maxTravelPx + 1) {
                points = slicePathByDistanceRange(points, pathLengthPx - maxTravelPx, pathLengthPx);
                ...
                [
                    ...
                    if (label === "fighter-ingress") {
                        ...
                        const switchedGroup = fighterGroups.length > 1
                            ? fighterGroups[(groupIndex + (role === "interceptor" ? 1 : fighterGroups.length - 1)) % fighterGroups.length]
                            : group;
                        const switchedLane = switchedGroup.lane;
                        const switchedLaneOffsetPx = resolveAirShowRailLaneOffsetPx(switchedLane, role, (flight.spec.laneOffsetPx ?? 0) * 0.2);
        ...
        const resolveContinuousFlakBursts = (assignments, durationMs, phase) => {
            ...
            return bomberPlans.flatMap((plan, planIndex) => {
                ...
                const syntheticProgressSlots = phase === "approach"
                    ...
                    : [0.52, 0.6, 0.68, 0.76, 0.82];
                const syntheticMinProgress = phase === "approach" ? 0.14 : 0.18;
                const syntheticMaxProgress = phase === "approach" ? 0.84 : 0.82;
                const syntheticScopedBursts = syntheticProgressSlots.map((slotProgress, slotIndex) => ({
                    progress: host.clamp(slotProgress + bomberSlot * (phase === "approach" ? 0.022 : 0.018), syntheticMinProgress, syntheticMaxProgress),
                    count: 1,
                    puffCount: phase === "approach" ? 8 : 9,
                    smokePuffCount: phase === "approach" ? 14 : 16,
                    scale: 0.86,
                    smokeScale: 1.42,
                    alongOffsetPx: -88 + slotIndex * 28 + bomberSlot * 10,
                    lateralOffsetPx: bomberSlot * 112 + (slotIndex - 2) * 34,
                    alongSpreadPx: 92,
                    lateralSpreadPx: 192,
                ...
                const expandedScopedBursts = scoped.length <= 2
                    ? scoped.flatMap((burst) => {
                        const baseProgress = burst.progress ?? 0.5;
                        const volleyOffsets = phase === "approach" ? [-0.045, 0, 0.045] : [-0.038, 0, 0.038];
                        return volleyOffsets.map((offset, volleyIndex) => ({
                            ...burst,
                            progress: host.clamp(baseProgress + offset, phase === "approach" ? 0.12 : 0.14, phase === "approach" ? 0.88 : 0.84),
                            alongOffsetPx: (burst.alongOffsetPx ?? 0) + (volleyIndex - 1) * 16,
                            lateralOffsetPx: (burst.lateralOffsetPx ?? 0) + (volleyIndex - 1) * 28,
                            count: 1
                        }));
                    })
                    : scoped;
                ...
                return sourceBursts.flatMap((burst, burstIndex) => {
                    ...
                    const basePuffCount = burst.puffCount ?? (phase === "approach" ? 10 : 11);
                    const puffCount = Math.max(6, Math.min(9, Math.round(basePuffCount <= 1 ? (phase === "approach" ? 7 : 8) : basePuffCount * 0.56)));
                    const requestedSmokePuffCount = burst.smokePuffCount ?? Math.round(puffCount * 1.85);
                    const smokePuffCount = Math.max(puffCount + 5, Math.min(18, Math.round(requestedSmokePuffCount)));
                    const jitteredProgress = host.clamp(selectedSample.progress
                        + (jitter01(sampleIndex, burstIndex + 1) - 0.5)
                            * (usingSyntheticSource ? (phase === "approach" ? 0.018 : 0.016) : (phase === "approach" ? 0.03 : 0.026)), minProgress, phase === "approach" ? maxProgress : 0.84);
                    ...
                    return [{
                            ...
                            alongOffsetPx: host.clamp((burst.alongOffsetPx ?? 0) * 0.56 - 68 + jitter01(sampleIndex, burstIndex + 11) * 176, -168, 168),
                            lateralOffsetPx: host.clamp((burst.lateralOffsetPx ?? 0) * 0.52
                                + (jitter01(sampleIndex, burstIndex + 23) - 0.5) * 360
                                + bomberSlot * 36
                                + (burstIndex - (scopedCount - 1) / 2) * 26
                                + (jitter01(sampleIndex, planIndex + 71) - 0.5) * 64, -340, 340),
                            alongSpreadPx: Math.max(76, Math.min(136, Math.round((burst.alongSpreadPx ?? 84) * (1.02 + jitter01(sampleIndex, burstIndex + 29) * 0.3)))),
                            lateralSpreadPx: Math.max(148, Math.min(248, Math.round((burst.lateralSpreadPx ?? 148) * (1.08 + jitter01(sampleIndex, burstIndex + 37) * 0.34)))),
                            scale: Math.max(0.7, (burst.scale ?? 1) * (0.8 + jitter01(sampleIndex, burstIndex + 31) * 0.36)),
                            smokeScale: Math.max(1.46, (burst.smokeScale ?? 1) * (1.36 + jitter01(sampleIndex, burstIndex + 43) * 0.4)),
                            bomberUnitKey: plan.flight.spec.id,
        ...
        const resolveCorridorPhaseDurationMs = (label, assignments) => {
            ...
            return label === "fighter-ingress"
                ? host.clamp(resolvedDurationMs, 1, 8000)
                : resolvedDurationMs;
        };
        ...
        const alignClashFightersThroughSharedFightSpace = (assignments, durationMs, fightSpaceCenter, convergenceTimeProgress) => {
            ...
            return assignments.map((assignment) => {
                ...
                const previousHeading = previousAssignment
                    ...
                    : null;
                const convergencePoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.axis.x * roleSide * 3 + corridor.normal.x * (lane * 7 + actorSlot * 3), corridor.axis.y * roleSide * 3 + corridor.normal.y * (lane * 7 + actorSlot * 3));
                ...
                const carryDistancePx = Math.min(84, distanceToConvergencePx * 0.11);
                const carryPoint = host.offsetAirShowPoint(start, currentForward.x * carryDistancePx, currentForward.y * carryDistancePx);
                const entryPoint = host.offsetAirShowPoint(carryPoint, (convergencePoint.cx - carryPoint.cx) * 0.42
                    + corridor.normal.x * (roleSide * 26 + lane * 6 + actorSlot * 4), (convergencePoint.cy - carryPoint.cy) * 0.42
                    + corridor.normal.y * (roleSide * 26 + lane * 6 + actorSlot * 4));
                const preMergePoint = host.offsetAirShowPoint(entryPoint, (convergencePoint.cx - entryPoint.cx) * 0.58
                    + corridor.normal.x * (lane * 5 - roleSide * 9 + actorSlot * 2), (convergencePoint.cy - entryPoint.cy) * 0.58
                    + corridor.normal.y * (lane * 5 - roleSide * 9 + actorSlot * 2));
                const breakPoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.axis.x * (34 + lane * 5 + actorSlot * 3)
                    + corridor.normal.x * (lane * 12 - roleSide * 42 + actorSlot * 6), corridor.axis.y * (34 + lane * 5 + actorSlot * 3)
                    + corridor.normal.y * (lane * 12 - roleSide * 42 + actorSlot * 6));
                const rejoinPoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.axis.x * (88 + lane * 7)
                    + corridor.normal.x * (roleSide * 18 + actorSlot * 7), corridor.axis.y * (88 + lane * 7)
                    + corridor.normal.y * (roleSide * 18 + actorSlot * 7));
                ...
                const convergenceDistancePx = measurePathLength(points.slice(0, convergencePointIndex >= 0 ? convergencePointIndex + 1 : 5));
                const breakDistancePx = measurePathLength(points.slice(0, breakPointIndex >= 0 ? breakPointIndex + 1 : 6));
                return {
                    ...assignment,
                    points,
                    progressTimeline: [
        ...
        const alignScrambleFightersThroughChasePocket = (assignments, durationMs, fightSpaceCenter) => {
            ...
            return assignments.map((assignment) => {
                ...
                const carryPoint = host.offsetAirShowPoint(start, currentForward.x * 58, currentForward.y * 58);
                const pocketPoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.normal.x * (lane * 9 + roleSide * 5 + actorSlot * 4)
                    + corridor.axis.x * (roleSide * 3 + actorSlot * 2), corridor.normal.y * (lane * 9 + roleSide * 5 + actorSlot * 4)
                    + corridor.axis.y * (roleSide * 3 + actorSlot * 2));
                const sweepPoint = host.offsetAirShowPoint(fightSpaceCenter, corridor.axis.x * (42 + lane * 5)
                    + corridor.normal.x * (lane * 18 - roleSide * 24 + actorSlot * 4), corridor.axis.y * (42 + lane * 5)
                    + corridor.normal.y * (lane * 18 - roleSide * 24 + actorSlot * 4));
                const entryPoint = host.offsetAirShowPoint(carryPoint, (pocketPoint.cx - carryPoint.cx) * 0.52 + corridor.normal.x * roleSide * 12, (pocketPoint.cy - carryPoint.cy) * 0.52 + corridor.normal.y * roleSide * 12);
                const exitPoint = host.offsetAirShowPoint(sweepPoint, (end.cx - sweepPoint.cx) * 0.34, (end.cy - sweepPoint.cy) * 0.34);
                ...
                const pocketDistancePx = measurePathLength(points.slice(0, pocketPointIndex >= 0 ? pocketPointIndex + 1 : 4));
                const sweepDistancePx = measurePathLength(points.slice(0, sweepPointIndex >= 0 ? sweepPointIndex + 1 : 5));
                return {
                    ...assignment,
                    points,
                    progressTimeline: [
        ...
        const finalizeCorridorPhaseAssignments = (label, assignments, durationMs) => {
            ...
            const preparedAssignments = host.prepareAirShowPhaseAssignments(extendedAssignments, durationMs, [0.24, 0.5, 0.76], 42, roleSpeeds, {
            ...
            const phasePatternAlignedAssignments = label === "bomber-defense-pass"
                ? finalSpeedAdjustedAssignments.map((assignment) => {
                    ...
                    const fighterFlightIds = Array.from(new Set(finalSpeedAdjustedAssignments
                    ...
                    const fighterActorCount = Math.max(1, finalSpeedAdjustedAssignments.filter((candidate) => candidate.actor.flightId === assignment.actor.flightId
                        && (candidate.actor.role === "interceptor" || candidate.actor.role === "escort")).length);
                    const localLane = assignment.actor.formationIndex - (fighterActorCount - 1) / 2;
                    const lateralOffsetPx = (assignment.actor.role === "interceptor" ? -18 : 22) + localLane * 10;
                    const interceptFocus = host.sampleAirShowAssignmentAtTime(targetBomberAssignment, durationMs * 0.48, durationMs).position;
                    const exitFocus = host.sampleAirShowAssignmentAtTime(targetBomberAssignment, durationMs * 0.78, durationMs).position;
                    const interceptPoint = host.offsetAirShowPoint(interceptFocus, corridor.normal.x * lateralOffsetPx * 0.42 - corridor.axis.x * 8, corridor.normal.y * lateralOffsetPx * 0.42 - corridor.axis.y * 8);
                    const entryPoint = host.offsetAirShowPoint(start, (interceptPoint.cx - start.cx) * 0.38
                        + corridor.normal.x * lateralOffsetPx * 0.22
                        - corridor.axis.x * 16, (interceptPoint.cy - start.cy) * 0.38
                        + corridor.normal.y * lateralOffsetPx * 0.22
                        - corridor.axis.y * 16);
                    const crossingPoint = host.offsetAirShowPoint(interceptFocus, corridor.axis.x * 54 - corridor.normal.x * lateralOffsetPx * 0.32, corridor.axis.y * 54 - corridor.normal.y * lateralOffsetPx * 0.32);
                    const exitPoint = host.offsetAirShowPoint(exitFocus, -corridor.normal.x * lateralOffsetPx * 0.58 + corridor.axis.x * 118, -corridor.normal.y * lateralOffsetPx * 0.58 + corridor.axis.y * 118);
                    ...
                    const interceptDistancePx = measurePathLength(points.slice(0, interceptPointIndex >= 0 ? interceptPointIndex + 1 : 3));
                    const crossingDistancePx = measurePathLength(points.slice(0, crossingPointIndex >= 0 ? crossingPointIndex + 1 : 4));
                    return {
                        ...assignment,
                        points,
                        progressTimeline: [
            ...
            const bomberDefenseSpeedCappedAssignments = label === "bomber-defense-pass"
                ? fighterClashTravelCappedAssignments.map((assignment) => {
                    ...
                    const maxFighterCombatTravelPx = Math.max(24, durationMs * host.airShowFighterSpeedPxPerMs * 1.24);
                    const travelPx = measurePathLength(assignment.points);
            ...
            const bomberDefenseSpeedFlooredAssignments = label === "bomber-defense-pass"
                ? bomberDefenseSpeedCappedAssignments.map((assignment) => {
                    ...
                    const minimumFighterCombatTravelPx = Math.max(28, durationMs * host.airShowFighterSpeedPxPerMs * 0.6);
                    ...
                    const lateralDriftPx = sideSign * (20 + Math.abs(formationBias) * 5);
                    ...
                    const carryPoint = host.offsetAirShowPoint(end, extensionDirection.x * Math.min(68, extensionPx * 0.4)
                        + corridor.normal.x * lateralDriftPx * 0.42, extensionDirection.y * Math.min(68, extensionPx * 0.4)
                        + corridor.normal.y * lateralDriftPx * 0.42);
                    const extensionPoint = host.offsetAirShowPoint(end, extensionDirection.x * extensionPx + corridor.normal.x * lateralDriftPx, extensionDirection.y * extensionPx + corridor.normal.y * lateralDriftPx);
                    return {
                        ...assignment,
            ...
            return separatePhaseEndAssignments(bomberDefenseSpeedRestoredAssignments, 16);
        };
        ...
        const buildHomeSideTargetRunFighterPath = (assignment) => {
            if (assignment.actor.role !== "interceptor"
                && assignment.actor.role !== "escort") {
                return assignment;
            }
        ...
        const rawTargetRunAssignmentByActorId = new Map(rawTargetRunAssignments.map((assignment) => [assignment.actor.id, assignment]));
        const preparedTargetRunAssignments = host.prepareAirShowPhaseAssignments(rawTargetRunAssignments, targetRunDurationMs, [0.2, 0.5, 0.78], 44, roleSpeeds, {
            previousAssignments: previousPhaseAssignments,
            previousDurationMs: previousPhaseDurationMs,
        ...
        const targetRunAssignments = host.extendAirShowPhaseAssignmentsForSpeed(preparedTargetRunAssignments, targetRunDurationMs, roleSpeeds, {
            clampCenter: corridor.strike,
            orbitSignByRole: {
                interceptor: -1,
                escort: 1,
            ...
            return {
                ...assignment,
                points: continuityAnchoredPoints,
    ...
    if (survivingBombers.length > 0) {
        ...
        if (attackEntriesForShow.length > 0) {
            ...
            const maxCombatRoleSpeedScale = 1.2;
            bomberDefensePlaybackAssignments = bomberDefensePlaybackAssignments.map((assignment) => {
                if (assignment.actor.role !== "interceptor"
                    && assignment.actor.role !== "escort") {
                    return assignment;
    ...
    if (postPassBombers.length > 0) {
        ...
        const bomberStrikeRunAssignments = bomberTargetRuns.flatMap(({ bomberFlight, targetCenter, turnSideSign }, bomberIndex) => spreadBomberFormationAssignmentsForCorridor(host.buildAirShowFlightAssignments(bomberFlight, host.buildAirShowBomberTargetRunPath(host.averageAirShowPosition(bomberFlight.actors) ?? bomberFlight.anchor, targetCenter, {
            lateralSign: turnSideSign,
            corridorWidthPx: 10,
            startHeadingDegrees: host.resolveAirShowFlightHeadingDegrees(bomberFlight)
        }), 0.2, bomberIndex, bomberTargetRuns.length), corridor));
        ...
        const scopedPostPassFlakBursts = collectScopedBomberFlakBursts(postPassBombers);
        const scopedStrikeRunFlakBursts = remapFlakBurstsToPhase(scopedPostPassFlakBursts, {
        ...
        const syntheticStrikeRunFlakBursts = scopedPostPassFlakBursts.length > 0
            ? []
            : postPassBombers.flatMap((flight, bomberIndex) => {
                const bomberSlot = bomberIndex - (postPassBombers.length - 1) / 2;
                const targetHexKey = bomberSpecsById.get(flight.spec.id)?.targetHexKey
                    ?? scene.bomberTargetHexKey
                    ?? null;
                return [0.56, 0.66, 0.76].map((progress, burstIndex) => ({
                    progress: host.clamp(progress + bomberSlot * 0.014, 0.54, 0.86),
                    count: 1,
                    puffCount: 8 + (burstIndex % 2),
                    smokePuffCount: 14 + (burstIndex % 2),
                    alongOffsetPx: -72 + burstIndex * 36 + bomberSlot * 14,
                    lateralOffsetPx: bomberSlot * 108 + (burstIndex - 1) * 34,
                    alongSpreadPx: 98,
                    lateralSpreadPx: 188,
                    scale: 0.9,
                    smokeScale: 1.34,
                    bomberUnitKey: flight.spec.id,
                    targetHexKey,
                    batteryHexKey: null
                }));
            });
        const strikeRunFlakBursts = scopedStrikeRunFlakBursts.length > 0
            ...
            : Array.from(scopedPostPassFlakBursts.reduce((burstsByBomberId, burst) => {
                ...
                .concat(syntheticStrikeRunFlakBursts);
