...
export class MapViewport {
    constructor(mapElementSelector = "#battleHexMap") {
        ...
        this.bindPointerInteractions = () => {
            ...
            this.wheelEventTarget.addEventListener("pointerdown", this.handlePointerDown);
            this.wheelEventTarget.addEventListener("pointermove", this.handlePointerMove);
     ...
     */
    adjustZoom(delta) {
        this.transform.zoom = this.clamp(this.transform.zoom + delta, this.MIN_ZOOM, this.MAX_ZOOM);
        this.updateTransform();
    }
    /**
     * Adjusts zoom centered on the pointer position, preserving the screen location of the hovered map point.
     */
    adjustZoomAt(delta, clientX, clientY) {
        ...
        const { viewport, containerWidth, containerHeight, baseOffsetX, baseOffsetY, renderScale, contentOffsetX, contentOffsetY, elementWidth, elementHeight, zoom: currentZoom } = context;
    ...
    centerOn(x, y) {
        ...
        const { canvas, viewport, containerWidth, containerHeight, baseOffsetX, baseOffsetY, renderScale, contentOffsetX, contentOffsetY, elementWidth, elementHeight, zoom } = context;
    ...
    clampPanToViewport(context, zoom, panX, panY) {
        const { containerWidth, containerHeight, baseOffsetX, baseOffsetY, mapWidth, mapHeight, renderScale, contentOffsetX, contentOffsetY } = context;
        let nextPanX = panX;
        let nextPanY = panY;
