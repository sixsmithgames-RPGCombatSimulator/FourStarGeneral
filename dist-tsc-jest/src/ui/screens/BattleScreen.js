...
export class BattleScreen {
    ...
    async collectAirMissionFlights(arrivals, renderer) {
        const flights = [];
        const engine = this.battleState.ensureGameEngine();
     ...
     */
    buildLinkedStrikePackageScene(flight, linkedEvents, escortFlights, renderer, engine) {
        const destKey = this.resolvePreparedAirMissionDestKey(flight, engine) ?? flight.destKey;
        ...
        if (!destOffset)
            throw new Error(`Invalid hex key: ${destKey}`);
        ...
        const escorts = escortFlights.map((escort, index) => ({
        ...
        const interceptors = airToAirEvent
     ...
     */
    buildCombatTracers(escorts, interceptors, airToAirEvent) {
        if (!airToAirEvent?.escortExchanges)
            return [];
        const tracers = [];
     ...
     */
    buildDestroyedList(escorts, interceptors, airToAirEvent) {
        if (!airToAirEvent)
            return [];
        const destroyed = [];
        // Check escorts
