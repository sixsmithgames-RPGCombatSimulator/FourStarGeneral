...
export class BattleScreen {
     ...
     */
    promptAttackConfirmation(attacker, defender, options = {}) {
        if (!this.attackConfirmDialog || !this.attackConfirmBody) {
            ...
            void this.executePendingAttack(attacker, defender, options.attackerUnitId ?? this.selectedPlayerUnitId, options.defenderUnitId ?? null);
            return;
        }
      ...
      `;
        ...
        const detailsExpanded = this.attackConfirmBody.querySelector(".attack-preview-details")?.open ?? false;
        ...
        const damageMultiplierDescription = damageMultiplier === 10
            ? "Heavy payload strike (bombers vs ground)"
            : damageMultiplier === 4
                ? "Accelerated dogfight tempo"
                : "Standard payload";
        const suppressionMultiplierDescription = suppressionMultiplier === 10
        ...
        const signatureMultiplier = signatureMultipliers[defenderDef.combat.signature];
        ...
        const accuracyBreakdownLine = `Range table ${rangeTableAccuracy.toFixed(1)}% x Unit accuracy x${unitAccuracyScalar.toFixed(2)} (${attackerDef.accuracyBase}/${attackerProfile.accuracyReference}) = ${baseAccuracyPercent.toFixed(1)}%, ` +
    ...
    describeAttackProfile(unit, commandState) {
    ...
    handleAirPreviewRange(event) {
        if (!this.hexMapRenderer)
            return;
        const { origin, radius } = event.detail ?? { origin: null, radius: 0 };
    ...
    parseAxialKeyToOffsetHexKey(hexKey) {
        if (!hexKey) {
            return null;
        }
    ...
    getQueuedArtilleryForCallerHex(hexKey) {
        if (!this.battleState.hasEngine()) {
            return null;
        }
        return this.battleState.ensureGameEngine().getSupportSnapshot().queued.find((asset) => asset.type === "artillery" && asset.queuedByHex === hexKey) ?? null;
    }
    restartQueuedArtilleryTargeting(callerHexKey, callerLabel, assetId) {
    ...
    cancelQueuedAirMission(missionId, missionKind, targetHexKey) {
        const engine = this.battleState.ensureGameEngine();
    ...
    handleQueuedTargetMarkerClick(event) {
    ...
    resolveArtilleryTargetHexKeys(unit, hexKey) {
    ...
    resolveArtilleryActionState(unit, commandState, hexKey) {
    ...
    async executeQueuedArtilleryStrike(targetHexKey) {
    ...
    async playSupportImpacts(impacts) {
        ...
        for (let index = 0; index < impacts.length; index += 1) {
            ...
            const targetClass = impact.targetUnitType
                ? this.unitTypes[impact.targetUnitType]?.class
                : undefined;
            if (lastFocusedHexKey !== targetHexKey) {
                ...
                await new Promise((resolve) => window.setTimeout(resolve, lastFocusedHexKey ? 220 : 320));
    ...
    updateAirHudWidget() {
        try {
            ...
            if (!el)
                return;
            const setText = (id, v) => {
                const n = el.querySelector(`[data-airhud-${id}]`);
                if (n)
            ...
            const openBtn = el.querySelector("[data-airhud-open]");
            if (openBtn) {
    ...
    syncAirMissionLogs() {
        try {
            ...
            const linkedStrikeUnitKeys = new Set(reports
            ...
            const linkedEscortsByBomberKey = new Map();
            for (const report of reports) {
                if (report.kind === "escort"
            ...
            for (const r of reports) {
                ...
                const reportShowsCombatDetails = (r.interceptions ?? 0) > 0
                ...
                const isLinkedEscortReport = r.kind === "escort"
                    && !!r.escortTargetUnitKey
                    && linkedStrikeUnitKeys.has(r.escortTargetUnitKey);
                const shouldFoldIntoDetailedCombatLog = r.event !== "refitStarted"
                ...
                let action = "resolved";
                if (r.event === "refitStarted")
                    action = "refit started";
                else if (r.event === "refitCompleted")
                    action = "refit completed";
                ...
                if (r.event !== "refitStarted" && r.event !== "refitCompleted" && r.outcome) {
                    ...
                    const destroyedBeforeTarget = r.kind === "strike"
                ...
                if (r.outcome) {
                    details.outcomeDetails = r.outcome.details;
                }
                ...
                if (r.kind === "strike") {
                    ...
                    if (linkedEscorts.length > 0) {
                        const escortLabels = linkedEscorts.map((escortReport) => this.formatAirCombatantSummary(escortReport.unitLabel, escortReport.unitKey, escortReport.unitType, escortReport.faction, engine));
                        escortNote =
    ...
    async handleConfirmAttack() {
        ...
        try {
            ...
            await this.executePendingAttack(attackerAxial, defenderAxial, this.pendingAttack.attackerUnitId, this.pendingAttack.defenderUnitId);
            this.pendingAttack = null;
            this.currentAttackStance = null;
        }
        finally {
     ...
     */
    async executePendingAttack(attacker, defender, attackerUnitId, defenderUnitId) {
        this.closeSelectionIntelForAnimation();
        const engine = this.battleState.ensureGameEngine();
        try {
            const attackerOffset = CoordinateSystem.axialToOffset(attacker.q, attacker.r);
            ...
            if (this.hexMapRenderer) {
                try {
                    preview = engine.previewAttack(attacker, defender, this.currentAttackStance ?? undefined, attackerUnitId ?? undefined, defenderUnitId ?? undefined);
                    if (preview) {
                        const defenderDefinition = this.unitTypes?.[preview.defender.type];
            ...
            const resolution = engine.attackUnit(attacker, defender, this.currentAttackStance ?? undefined, attackerUnitId ?? undefined, defenderUnitId ?? undefined);
            if (resolution && this.hexMapRenderer) {
                ...
                if (!resolution.defenderDestroyed && defenderInflicted > 0) {
                    ...
                    const defenderDefinition = defenderType ? this.unitTypes?.[defenderType] : undefined;
                    this.hexMapRenderer.markHexDamaged(defenderHex, defenderDefinition?.class, resolution.defenderRemainingStrength, 2);
                }
                if (resolution.defenderDestroyed) {
                    const defenderType = preview?.defender.type ?? this.pendingAttack?.preview?.defender?.type;
                    const defenderDefinition = defenderType ? this.unitTypes?.[defenderType] : undefined;
                    this.hexMapRenderer.markHexWrecked(defenderHex, defenderDefinition?.class, 2);
                }
                ...
                if (resolution.retaliationOccurred &&
                    ...
                    const attackerDefinition = attackerType ? this.unitTypes?.[attackerType] : undefined;
                    this.hexMapRenderer.markHexDamaged(attackerHex, attackerDefinition?.class, attackerRemaining, 2);
                }
                if (resolution.retaliationOccurred && typeof attackerRemaining === "number" && attackerRemaining <= 0) {
                    const attackerType = preview?.attacker.type ?? this.pendingAttack?.preview?.attacker?.type;
                    const attackerDefinition = attackerType ? this.unitTypes?.[attackerType] : undefined;
                    this.hexMapRenderer.markHexWrecked(attackerHex, attackerDefinition?.class, 2);
                }
            }
            if (resolution?.retaliationOccurred && this.hexMapRenderer) {
                try {
                    const attackerDefinition = preview
                        ? this.unitTypes?.[preview.attacker.type]
                        : null;
     ...
     */
    async playBotTurnAnimations(botSummary) {
        this.closeSelectionIntelForAnimation();
        if (!this.hexMapRenderer) {
            this.renderEngineUnits();
            return;
        ...
        if (!canFocusCamera) {
            console.error("[BattleScreen] playBotTurnAnimations: mapViewport unavailable; skipping camera recentering.", { hasRenderer: !!this.hexMapRenderer });
        }
        ...
        for (const attack of botSummary.attacks) {
            ...
            if (this.hexMapRenderer) {
                if (attack.defenderDestroyed) {
                    const defenderDefinition = this.unitTypes?.[attack.defenderType];
                    this.hexMapRenderer.markHexWrecked(targetKey, defenderDefinition?.class, 2);
                }
                    ...
                    this.hexMapRenderer.markHexDamaged(targetKey, defenderDefinition?.class, defenderNow?.strength, 2);
                }
                ...
                if (typeof attackerStrengthAfter === "number" &&
                    ...
                    const attackerDefinition = this.unitTypes?.[attack.attackerType];
                    this.hexMapRenderer.markHexDamaged(attackerKey, attackerDefinition?.class, attackerStrengthAfter, 2);
                }
                if (typeof attackerStrengthAfter === "number" && attackerStrengthAfter <= 0) {
                    const attackerDefinition = this.unitTypes?.[attack.attackerType];
                    this.hexMapRenderer.markHexWrecked(attackerKey, attackerDefinition?.class, 2);
                }
            }
    ...
    updateStanceButtonStates() {
        ...
        const assaultState = assaultBtn?.querySelector(".stance-state");
        if (assaultState) {
        ...
        const suppressiveState = suppressiveBtn?.querySelector(".stance-state");
        if (suppressiveState) {
    ...
    promptFortificationFacing(hex, unitLabel, unitId, modificationType = "fortifications") {
    ...
    buildFortificationFacingPreviewMarkup(fortifiedFacings, modificationType) {
      ...
      <svg viewBox="0 0 220 220" class="fortification-facing-preview-svg" aria-label="Select a ${noun} edge">
        ...
        ${Object.entries(labelPositions).map(([edge, point]) => `
          <text
    ...
    handleFortificationDialogKeydown(event) {
        if (!this.fortificationFacingDialog) {
            return;
        }
        switch (event.key) {
    ...
    getFortificationDialogFocusableElements() {
        if (!this.fortificationFacingDialog) {
            return [];
        }
        const selectors = [
        ...
        return Array.from(this.fortificationFacingDialog.querySelectorAll(selectors)).filter((element) => {
    ...
    async handleConfirmFortificationFacing(facing) {
    ...
    completeTutorialPhase(phase, shouldAdvance = true) {
    ...
    bindPanelEvents() {
        ...
        this.deploymentPanel.on((event) => {
            ...
            switch (event.type) {
                case "deploy": {
                    ...
                    const hexKey = event.payload?.hexKey;
                    if (!unitKey || !hexKey) {
                        return;
                    }
                ...
                case "highlightZone": {
                    const zoneKey = event.payload?.zoneKey;
        ...
        this.deploymentPanelToggleButton?.addEventListener("click", () => {
            ...
            this.deploymentPanelToggleButton?.setAttribute("aria-expanded", this.deploymentPanelBody?.classList.contains("hidden") ? "false" : "true");
        });
        ...
        if (this.element.dataset.tutorialFocusBound !== "true") {
            ...
            document.addEventListener("tutorial:focusHex", ((event) => {
                if (!this.hexMapRenderer) {
                ...
                console.log("[BattleScreen][tutorial:focusHex] event received", {
                    ...
                    rendererInitialized: Boolean(this.hexMapRenderer.initialized)
                });
                // Extract hex key
                let hexKey = null;
    ...
    renderMissionStatus() {
        ...
        const stateBadge = (state) => {
            if (state === "completed")
                return '<span class="mission-pill mission-pill--success">Completed</span>';
            if (state === "failed")
                return '<span class="mission-pill mission-pill--danger">Failed</span>';
            if (state === "inProgress")
                return '<span class="mission-pill mission-pill--progress">In progress</span>';
    ...
    showMissionEndModal(outcome, reason) {
     ...
     */
    refreshIdleUnitHighlights(summary) {
        const renderer = this.hexMapRenderer;
        if (!renderer) {
        ...
        const nextHighlightKeys = new Set();
    ...
    async triggerAirOperations(summary) {
        void summary;
    ...
    async triggerAirMissionArrivals(summary) {
        void summary;
        const arrivals = this.battleState.consumeAirMissionArrivals();
    ...
    async playAirMissionArrivals(arrivals) {
        await this.playAirOperations(arrivals, []);
    }
    async triggerAirEngagements(summary) {
        void summary;
        const engagements = this.battleState.consumeAirEngagements();
    ...
    async playAirEngagements(events) {
        await this.playAirOperations([], events);
    }
    beginAirShowPlaybackCapture(arrivals, events, engine) {
        return {
            base: {
                version: 1,
    ...
    finalizeAirShowPlaybackCapture(context) {
    ...
    snapshotAirPlaybackOperation(operation) {
        if (operation.kind === "linkedStrike") {
            return {
                ...
                interceptorUnitKeys: Array.from(new Set(operation.linkedEvents.flatMap((event) => event.interceptors.map((participant) => participant.unitKey)))),
    ...
    snapshotCoordinatedAirClusterPlaybackPlan(plan) {
        return {
    ...
    recordActiveAirShowPlaybackCluster(cluster, coordinatedPlan) {
    ...
    recordActiveAirShowResolvedEventSceneCapture(event, locKey, linkedEscortFlights, missingLinkedEscortUnitKeys, bomberPassAvailable, scene) {
        if (!this.activeAirShowPlaybackCaptureContext) {
            return;
        }
    ...
    recordActiveAirShowPlaybackViolation(violation) {
        if (!this.activeAirShowPlaybackCaptureContext) {
            return;
        }
        this.activeAirShowPlaybackCaptureContext.violations.push(this.deepCloneValue(violation));
    }
    formatAirShowPlaybackError(error) {
        if (error instanceof Error) {
            return error.stack ?? error.message;
        }
        return String(error);
    }
    async playAirOperations(arrivals, events) {
        const renderer = this.hexMapRenderer;
        if (!renderer) {
            return;
        }
        ...
        try {
            ...
            linkedEscortFlights.forEach((escorts) => standaloneFlights.push(...escorts.filter((flight) => !claimedAirBattleUnitKeys.has(flight.unitKey))));
            const standaloneEvents = events.filter((event) => !claimedLinkedEvents.has(event));
            const playbackOperations = this.buildAirPlaybackOperations(linkedStrikeFlights, standaloneFlights, standaloneEvents, engine);
    ...
    async collectAirMissionFlights(arrivals) {
        const flights = [];
        const engine = this.battleState.ensureGameEngine();
    ...
    async playStandaloneAirMissionFlight(flight, renderer, engine, preFocused = false) {
        ...
        if (flight.kind === "strike") {
            await this.animateAircraftLeg(renderer, flight.originKey, destKey, flight.unitType, this.resolveBomberSortieIngressDurationMs(), undefined, 1, flight.strength, flight.laneOffsetPx, flight.faction, "bomber");
            await this.playResolvedAirStrikeImpact(flight, renderer, engine);
            await this.playDamagedAircraftReturn(renderer, destKey, flight.originKey, flight.unitType, 0, flight.strength, flight.laneOffsetPx, 0, flight.faction, this.resolveBomberSortieEgressDurationMs(), "bomber");
            return;
        }
        await this.animateAircraftLeg(renderer, flight.originKey, destKey, flight.unitType, this.resolveFighterSortieIngressDurationMs(), undefined, 1, flight.strength, flight.laneOffsetPx, flight.faction, "interceptor");
        await this.playDamagedAircraftReturn(renderer, destKey, flight.originKey, flight.unitType, 0, flight.strength, flight.laneOffsetPx, 0, flight.faction, this.resolveFighterSortieEgressDurationMs(), "interceptor");
    }
    resolveAirEngagementOffsetKey(squadronIdOrHexKey, faction, engine) {
    ...
    async playStandaloneAirEngagementEvent(event, renderer, engine, preFocused = false, laneOffsetPx = 0) {
        ...
        const bomberFrom = this.resolveAirEngagementOffsetKey(event.bomber.unitKey, event.bomber.faction, engine);
        await this.playMissionAirInterceptEvent(event, locKey, renderer, engine, laneOffsetPx, false, true, event.type === "capClash" ? 0 : this.resolveAirInterceptBomberArrivalDelayMs(), event.type !== "capClash", bomberFrom);
    }
    async playMissionStrikeOperation(flight, linkedEvents, escortFlights, renderer, engine, preFocused = false) {
        const destKey = this.resolvePreparedAirMissionDestKey(flight, engine) ?? flight.destKey;
        if (!preFocused) {
            await this.focusCameraOnHex(destKey);
            await this.waitForNextFrame();
        ...
        const totalAttrition = Math.max(0, Number(outcome?.meta?.flakAttrition ?? 0)) +
        ...
        if (airToAirEvent) {
            await this.playMissionAirInterceptEvent(airToAirEvent, interceptLocKey, renderer, engine, flight.laneOffsetPx, false, true, this.resolveAirInterceptBomberArrivalDelayMs(), true, flight.originKey, escortFlights, destKey, flakEvent);
            if (flakEvent) {
        ...
        await Promise.all([
            (async () => {
                await this.animateAircraftLeg(renderer, flight.originKey, destKey, flight.unitType, this.resolveBomberSortieIngressDurationMs(), (progress, centerX, centerY) => {
                    if (flakEvent) {
                        while (progress >= nextBurstProgress && nextBurstProgress <= flakWindowEnd) {
                ...
                }, flakEvent?.bomberDestroyed ? 0.84 : 1, bomberStrength, flight.laneOffsetPx, flight.faction, "bomber");
                if (!bomberDestroyedBeforeImpact) {
                    await this.playResolvedAirStrikeImpact(flight, renderer, engine);
                    await this.playDamagedAircraftReturn(renderer, destKey, flight.originKey, flight.unitType, totalAttrition, remainingStrength, flight.laneOffsetPx, 0, flight.faction, this.resolveBomberSortieEgressDurationMs(), "bomber");
                }
            })(),
            ...escortAnimations
        ]);
    }
    async playMissionAirInterceptEvent(event, locKey, renderer, engine, fallbackLaneOffsetPx = 0, skipEscortFlights = false, announceEvent = true, bomberArrivalDelayMs = 0, allowBomberDefensePass = true, bomberOriginKey = null, linkedEscortFlights = [], bomberTargetKey = null, flakEvent = null) {
        if (announceEvent) {
        ...
        event.interceptors.forEach((interceptor, index) => {
            ...
            if (from) {
                ...
                const initialStrength = interceptor.strength ?? this.resolveAirSquadronStrength(interceptor.unitKey, interceptor.faction, engine);
                participants.push({
                    ...
                    clockwise: false,
                    initialStrength,
                    strengthAfterEscortPhase: this.resolveAirEngagementPhaseStrength(event.interceptorStrengthsAfterEscortPhase, index, initialStrength),
        ...
        escortParticipants.forEach((escort, index) => {
            if (!skipEscortFlights) {
                ...
                if (from) {
                    const laneOffset = participantOffsets[participantIndex] ?? -fallbackLaneOffsetPx;
                    const initialStrength = escort.strength ?? this.resolveAirSquadronStrength(escort.unitKey, escort.faction, engine);
                    participants.push({
                        unitKey: escort.unitKey,
                        role: "escort",
                        phaseIndex: index,
                        ...
                        clockwise: true,
                        initialStrength,
                        strengthAfterEscortPhase: typeof escort.eventIndex === "number"
                            ? this.resolveAirEngagementPhaseStrength(event.escortStrengthsAfterEscortPhase, escort.eventIndex, initialStrength)
                            : initialStrength,
                        finalStrength: typeof escort.eventIndex === "number"
        ...
        this.recordActiveAirShowResolvedEventSceneCapture(event, locKey, linkedEscortFlights, diagnostics.linkedEscortMissingFromEventUnitKeys, bomberPassAvailable, scene);
        if (diagnostics.linkedEscortMissingFromEventUnitKeys.length > 0) {
            const message = `[AirSprite] Linked escort flights missing from resolved event ${event.missionId ?? event.type}: ` +
    ...
    async playResolvedAirStrikeImpact(flight, renderer, engine, playEffects = true) {
        ...
        const defenderClass = defenderType ? this.unitTypes?.[defenderType]?.class : undefined;
        if (outcome.defenderDestroyed) {
    ...
    async playDamagedAircraftReturn(renderer, fromKey, toKey, unitType, damage, strength, laneOffsetPx = 0, initialDelayMs = 120, faction, durationMs, role = "interceptor") {
    ...
    announceFlakEngagement(event) {
        ...
        if (flakEngagements && flakEngagements.length > 0) {
            flakEngagements.forEach((engagement, index) => {
                ...
                const summary = `${this.toTitleCase(engagement.batteryUnitType)} at ${batteryHex} opened fire on ${bomberLabel} during final approach. ` +
    ...
    announceAirInterceptEngagement(event) {
        ...
        this.markDetailedAirCombatPublished(engine?.turnNumber ?? null, [
            ...event.interceptors.map((interceptor) => interceptor.unitKey),
            ...event.escorts.map((escort) => escort.unitKey)
        ]);
        if ((Array.isArray(event.escortExchanges) && event.escortExchanges.length > 0) ||
            ...
            const lowerFactionLabel = (faction) => faction === "Player" ? "player" : faction === "Ally" ? "allied" : "enemy";
            const publishExchange = (summary, category, details) => {
                this.announceBattleUpdate(summary);
                this.publishActivityEvent({
                    category,
                    type: "log",
            ...
            (event.escortExchanges ?? []).forEach((exchange, index) => {
                ...
                const attackerCombatant = this.formatAirCombatantSummary(exchange.attackerLabel, exchange.attackerUnitKey, exchange.attackerUnitType, exchange.attackerFaction, engine);
                const defenderCombatant = this.formatAirCombatantSummary(exchange.defenderLabel, exchange.defenderUnitKey, exchange.defenderUnitType, exchange.defenderFaction, engine);
                ...
                const summary = `${phaseLabel} over ${location}: ${attackerLabel} hit ${lowerFactionLabel(exchange.defenderFaction)} ${defenderCombatant} ` +
            ...
            (event.bomberPassExchanges ?? []).forEach((exchange, index) => {
                ...
                const attackerCombatant = this.formatAirCombatantSummary(exchange.attackerLabel, exchange.attackerUnitKey, exchange.attackerUnitType, exchange.attackerFaction, engine);
                const defenderCombatant = this.formatAirCombatantSummary(exchange.defenderLabel, exchange.defenderUnitKey, exchange.defenderUnitType, exchange.defenderFaction, engine);
                const summary = `Bomber pass over ${location}: ${this.formatAirFactionLabel(exchange.attackerFaction)} ${attackerCombatant} hit ` +
        ...
        const strengthBefore = typeof event.bomberStrengthBefore === "number"
            ...
            : typeof event.bomber.strength === "number"
                ...
                : null;
        const strengthAfter = typeof event.bomberStrengthAfter === "number"
            ? Math.max(0, Math.round(event.bomberStrengthAfter))
            : null;
        const interceptDamage = strengthBefore !== null && strengthAfter !== null
        ...
        const interceptorNote = interceptorAttrition > 0
            ...
            : interceptorKills > 0
                ...
                : "";
        const escortDamageNote = escortAttrition > 0
    ...
    resolveAirSquadronMatch(unit, squadronId) {
        return unit.unitId === squadronId || `${unit.type}@${unit.hex.q},${unit.hex.r}` === squadronId;
    }
    resolveAirSquadronUnit(squadronId, faction, engine) {
    ...
    resolveAirSquadronLabel(squadronId, faction, engine) {
        if (!squadronId) {
            return "-";
        }
    ...
    formatAirCombatantSummary(snapshotLabel, squadronId, unitType, faction, engine) {
    ...
    formatAirFactionLabel(faction) {
        return faction === "Player" ? "Player" : faction === "Ally" ? "Allied" : "Enemy";
    }
    buildDetailedAirCombatTurnUnitKey(turnResolved, unitKey) {
        if (!unitKey) {
            return null;
        }
        return `${turnResolved}:${unitKey}`;
    }
    ensureDetailedAirCombatTurnUnitKeys() {
    ...
    markDetailedAirCombatPublished(turnResolved, unitKeys) {
    ...
    hasDetailedAirCombatPublished(turnResolved, unitKey) {
    ...
    inferStrikeOutcomeCause(details) {
    ...
    findScheduledAirMissionById(missionId, faction, engine) {
        return engine.getScheduledAirMissions(faction).find((entry) => entry.id === missionId) ?? null;
    }
    findLinkedStrikeMissionForEscort(protectedSquadronId, faction, engine) {
    ...
    resolvePreparedAirMissionTargetHex(flight, engine) {
    ...
    resolveAirPlaybackFocusHexForFlight(flight, engine) {
        return this.resolvePreparedAirMissionTargetHex(flight, engine) ?? this.offsetHexKeyToAxial(flight.destKey);
    }
    resolvePreparedAirMissionDestKey(flight, engine) {
        return this.toOffsetHexKey(this.resolvePreparedAirMissionTargetHex(flight, engine)) ?? (flight.destKey || null);
    }
    async playEscortCompanionFlight(flight, destKey, renderer) {
        await this.animateAircraftLeg(renderer, flight.originKey, destKey, flight.unitType, this.resolveFighterSortieIngressDurationMs(), undefined, 1, flight.strength, flight.laneOffsetPx, flight.faction, "interceptor");
        await this.playDamagedAircraftReturn(renderer, destKey, flight.originKey, flight.unitType, 0, flight.strength, flight.laneOffsetPx, this.scaleAirSequenceMs(120), flight.faction, this.resolveFighterSortieEgressDurationMs(), "interceptor");
    }
    resolveAirSquadronStrength(squadronId, faction, engine, fallback = 100) {
        if (!squadronId) {
            return fallback;
        }
        return this.resolveAirSquadronUnit(squadronId, faction, engine)?.strength ?? fallback;
    }
    buildAirLaneOffsets(count) {
        if (count <= 1) {
            return [0];
        }
    ...
    assignAirMissionFlightLanes(flights) {
    ...
    async animateAircraftLeg(renderer, fromKey, toKey, unitType, durationMs, onProgress, endProgress = 1, strength, laneOffsetPx = 0, faction, role = "interceptor") {
        if (typeof renderer.animateAircraftArc === "function") {
            await renderer.animateAircraftArc(fromKey, toKey, unitType, durationMs, onProgress, endProgress, strength, laneOffsetPx, faction, role);
            return;
        }
        if (typeof renderer.animateAircraftFlyover === "function") {
            await renderer.animateAircraftFlyover(fromKey, toKey, unitType, durationMs, onProgress, endProgress, strength, laneOffsetPx, faction, role);
        }
    }
    ...
    resolveInterceptorsAfterEscortPhase(event) {
        ...
        const possibleEscortKills = Math.min(event.interceptors.length, event.escorts.length, Math.max(0, Math.round(event.interceptorKills ?? 0)));
        return Math.max(0, event.interceptors.length - possibleEscortKills);
    }
    resolveEscortsAfterEscortPhase(event) {
    ...
    resolveAirEngagementPhaseStrength(phaseStrengths, index, fallbackStrength) {
    ...
    shouldPlayBomberDefensePass(event) {
        const interceptorsAfterEscortPhase = this.resolveInterceptorsAfterEscortPhase(event);
        if (interceptorsAfterEscortPhase > 0) {
            return true;
        }
        const bomberStrengthBefore = typeof event.bomberStrengthBefore === "number"
            ? Math.max(0, Math.round(event.bomberStrengthBefore))
            : typeof event.bomber.strength === "number"
                ? Math.max(0, Math.round(event.bomber.strength))
                : null;
        const bomberStrengthAfter = typeof event.bomberStrengthAfter === "number"
            ? Math.max(0, Math.round(event.bomberStrengthAfter))
            : null;
    ...
    async playAirInterceptPasses(event, locKey, renderer, bomberArrivalDelayMs = 0, allowBomberDefensePass = true) {
        ...
        const orbitRenderer = renderer;
        const playOrbitStage = async (units) => {
            if (typeof orbitRenderer.animateAircraftOrbitAt !== "function" || units.length === 0) {
                return;
            }
            await Promise.all(units.map((unit) => orbitRenderer.animateAircraftOrbitAt(locKey, unit.unitType, orbitDurationMs, Math.max(0, Math.round(unit.strength)))));
        };
        if (event.escorts.length > 0) {
            await playOrbitStage([
                ...event.escorts.map((escort, index) => ({
                    unitType: escort.unitType,
                    strength: this.resolveAirEngagementPhaseStrength(event.escortStrengthsAfterEscortPhase, index, escort.strength ?? 100)
                })),
                ...event.interceptors.map((interceptor, index) => ({
                    unitType: interceptor.unitType,
                    strength: this.resolveAirEngagementPhaseStrength(event.interceptorStrengthsAfterEscortPhase, index, interceptor.strength ?? 100)
                }))
            ]);
        ...
        if (event.escorts.length === 0 || this.shouldPlayBomberDefensePass(event)) {
            const gapBeforeBomberPass = event.escorts.length > 0
            ...
            await playOrbitStage(event.interceptors
                .map((interceptor, index) => ({
                unitType: interceptor.unitType,
                strength: this.resolveAirEngagementPhaseStrength(event.interceptorStrengthsAfterEscortPhase, index, interceptor.strength ?? 100)
            }))
                .filter((interceptor) => interceptor.strength > 0));
            if (typeof renderer.playBomberDefensePass === "function") {
                await renderer.playBomberDefensePass(locKey);
            }
            else {
                await renderer.playDogfight(locKey);
            }
    ...
    buildAirPlaybackOperations(linkedStrikeFlights, standaloneFlights, standaloneEvents, engine) {
        const operations = [];
        let index = 0;
    ...
    clusterAirPlaybackOperations(operations) {
    ...
    airPlaybackOperationsShareCluster(a, b) {
    ...
    async playAirPlaybackCluster(cluster, renderer, engine) {
        ...
        const eventOperations = cluster.filter((operation) => operation.kind === "event");
        const eventLaneOffsets = this.buildAirLaneOffsets(eventOperations.length);
        const laneOffsetsByIndex = new Map();
        ...
        await Promise.all(concurrentOperations.map(async (operation) => {
            if (operation.kind === "linkedStrike") {
                await this.playMissionStrikeOperation(operation.flight, [...operation.linkedEvents], operation.escorts, renderer, engine, Boolean(focusKey));
                return;
            }
            if (operation.kind === "flight") {
    ...
    freezeCamera() {
        ...
        const mapElement = document.querySelector("#battleHexMap");
        if (mapElement) {
    ...
    async focusCameraOnHex(hexKey) {
        ...
        const svgElement = document.getElementById("battleHexMap");
    ...
    constructor(screenManager, battleState, popupManager, hexMapRenderer, deploymentPanel, battleLoadout, reservePresenter, mapViewport, zoomPanControls, battleActivityLog = null, uiState = null) {
        ...
        this.baseCampAssignButton = null;
        this.deploymentPanelToggleButton = null;
        this.deploymentPanelBody = null;
        this.autoDeployEvenlyButton = null;
        this.autoDeployGroupedButton = null;
        this.battleMainContainer = null;
        this.attackConfirmDialog = null;
        this.attackConfirmAccept = null;
        this.attackConfirmCancel = null;
        this.attackConfirmBody = null;
        this.fortificationFacingDialog = null;
        this.fortificationFacingPreview = null;
        this.missionTitleElement = null;
        this.missionBriefingElement = null;
        this.missionObjectivesList = null;
        ...
        this.idleWarningLayer = null;
        this.idleWarningDialog = null;
        this.idleWarningList = null;
        this.idleContinueButton = null;
        this.idleEndTurnButton = null;
        this.idleWarningPreviousFocus = null;
        this.idleWarningKeyHandler = null;
        ...
        this.playerMoveHexes = new Set();
        this.playerAttackHexes = new Set();
        this.pendingAttack = null;
        ...
        window.battleScreenMapViewport = this.mapViewport;
    ...
    bindSelectionIntelOverlayActions() {
    ...
    bindIdleWarningDialog() {
        ...
        this.idleWarningKeyHandler = (event) => {
            if (event.key === "Escape") {
                event.preventDefault();
     ...
     */
    resolveBaseCampZoneKey(baseCampAxialKey) {
        const deploymentState = ensureDeploymentState();
     ...
     */
    planAutoDeployment(mode, baseCampAxialKey, zoneKey) {
        const deploymentState = ensureDeploymentState();
    ...
    *buildUnitQueue(mode) {
     ...
     */
    collectAvailableHexes(zoneHexes) {
        const deploymentState = ensureDeploymentState();
        const available = [];
     ...
     */
    resolveDistanceFromBase(baseCamp, offsetHexKey) {
     ...
     */
    executeAutoDeployment(engine, placements) {
     ...
     */
    finishDeploymentAfterAutoPlacement(_engine) {
        try {
     ...
     */
    prepareBattleState(enforceAllocations) {
        this.refreshScenario();
     ...
     */
    assertBattleReady(engine) {
        const deploymentState = ensureDeploymentState();
        if (!deploymentState.hasCommittedEntries()) {
            throw new Error("Commander allocations are missing. Return to the precombat screen and commit your deployment package before beginning the battle.");
        }
        ...
        if (reserveCount === 0 && placementCount === 0) {
            ...
            throw new Error(committedKeys.length === 0
     ...
     */
    showIdleWarning(summary, idleAxialKeys) {
        ...
        this.idleWarningPreviousFocus = document.activeElement ?? null;
    ...
    async executeTurnAdvance(_preflightSummary) {
        ...
        try {
            ...
            this.refreshIdleUnitHighlights(summary);
            this.announceBattleUpdate(`Turn ${summary.turnNumber} begins. Active faction: ${summary.activeFaction}. Phase: ${summary.phase}.`);
    ...
    handleEndMission() {
        ...
        this.announceBattleUpdate(outcomeAppliedToCampaign
     ...
     */
    updateGeneralServiceRecord(success) {
    ...
    computePlayerCasualties() {
        const initialUnitCount = this.scenario.sides.Player.units.length;
        if (!this.battleState || typeof this.battleState.hasEngine !== "function") {
            return 0;
        }
        if (!this.battleState.hasEngine()) {
            return 0;
    ...
    collectMissionStatistics() {
        ...
        if (typeof this.battleState.hasEngine !== "function" || !this.battleState.hasEngine()) {
     ...
     */
    calculateUnitDifference(initialUnits, currentUnits) {
     ...
     */
    calculateAmmunitionExpenditure() {
        try {
     ...
     */
    countLiveReservesForUnitKey(engine, unitKey) {
        const deploymentState = ensureDeploymentState();
        ...
        return engine.getReserveSnapshot().filter((reserve) => {
            ...
            return deploymentState.getUnitKeyForScenarioType(reserve.unit.type) === unitKey;
        }).length;
    }
    /**
     ...
     */
    isPlayerPlacementOccupyingHex(engine, hex) {
     ...
     */
    summarizeLiveReserveQueue(engine) {
     ...
     */
    seedEngineFromDeploymentState(engine) {
     ...
     */
    aggregateReserveRequests(blueprints) {
        const tally = new Map();
     ...
     */
    collapseDeploymentPanelForBattlePhase() {
     ...
     */
    showActivityLogAfterDeployment() {
     ...
     */
    reflectActivityLogState(collapsed) {
        if (!this.battleMainContainer) {
            return;
        }
        if (collapsed) {
    ...
    initializeBattleMap() {
        ...
        const svg = this.element.querySelector("#battleHexMap");
    ...
    resolveInitialPlayerDepotStock() {
    ...
    assertBotUnitsHydrated() {
        ...
        if (scenarioBotCount > 0 && engineBotCount === 0) {
            const summary = engine.getTurnSummary();
            throw new Error(`[BattleScreen] Bot units missing after initialization. scenarioBotCount=${scenarioBotCount}, engineBotCount=${engineBotCount}, phase=${summary.phase}, activeFaction=${summary.activeFaction}`);
        }
    }
    ...
    handleReserveCallupRequest(unitKey) {
        ...
        try {
            const deploymentState = ensureDeploymentState();
            const scenarioType = deploymentState.getScenarioTypeForUnitKey(unitKey);
            const unitTypeKey = (scenarioType ?? unitKey);
            const def = unitTypesSource[unitTypeKey];
    ...
    autoPlaceReserveNearestBase(unitKey) {
        const engine = this.battleState.ensureGameEngine();
    ...
    updateSelectionFeedback(key) {
        ...
        if (selectedMember) {
            ...
            const selectedUnitDefinition = this.unitTypes[selectedPlayerUnit.type];
            ...
            if (isAutomatedLogisticsUnit) {
                ...
                statusMessage += this.buildBattleActionSummary(this.playerMoveHexes.size, this.playerAttackHexes.size, ammoStatusMessage);
    ...
    async executeSelectionIntelAction(actionId) {
        if (!this.selectedHexKey) {
            return;
        }
    ...
    collectAirOperationsSummary(engine) {
        ...
        const livePlayerAirUnits = new Set();
        ...
        const participatingPlayerFlights = new Set();
        for (const report of reports) {
     ...
     */
    handleAttackDialogKeydown(event) {
        if (!this.attackConfirmDialog) {
            return;
        }
        switch (event.key) {
     ...
     */
    getAttackDialogFocusableElements() {
        if (!this.attackConfirmDialog) {
            return [];
        }
        const selectors = [
        ...
        return Array.from(this.attackConfirmDialog.querySelectorAll(selectors)).filter((element) => {
    ...
    computeDefaultSelectionKey() {
        ...
        throw new Error(`[BattleScreen] Unable to compute a default selection for mission ${(this.uiState?.selectedMission ?? "training")}: no registered player deployment hexes are available in scenario ${this.scenario.name}.`);
    }
    /**
     ...
     */
    resolveNavigationDelta(key, col) {
        switch (key) {
            case "ArrowUp":
            case "w":
            case "W":
     ...
     */
    ensureAnnouncementRegionAttributes() {
        if (!this.battleAnnouncements) {
            return;
        }
    ...
    reportDeploymentPanelError(error, options) {
     ...
     */
    resolvePlayerUnitSnapshot(hexKey, unitId) {
        const parsed = CoordinateSystem.parseHexKey(hexKey);
        if (!parsed) {
            return null;
        }
    ...
    getPlayerStackMembersAtHex(hexKey) {
        const parsed = CoordinateSystem.parseHexKey(hexKey);
        if (!parsed) {
            return [];
        }
    ...
    resolveSelectedPlayerStackMember(hexKey) {
    ...
    lookupPlayerUnitFuel(hexKey, unitId) {
    ...
    buildBattleIntelActions(hexKey, unit, commandState) {
    ...
    buildUnitTraitSummary(unit, definition) {
        const traits = new Set((definition.traits ?? []).map((trait) => this.formatIntelLabel(trait)));
        if (this.isEngineerBattleUnit(unit)) {
            traits.add("Engineer");
        }
        return Array.from(traits);
    }
    buildBattleActionSummary(moveOptions, attackOptions, ammoStatusMessage) {
    ...
    formatBattleResourceValue(value) {
    ...
    resolveBattleAttackAmmoCost(definition) {
    ...
    buildBattleAmmoStatusMessage(unit, definition) {
    ...
    describeHexModificationPlacement(modification) {
    ...
    describeHexModificationCollection(modifications) {
    ...
    formatHexModificationLabel(modification) {
        const facing = this.normalizeFortificationEdgeFacing(modification.facing);
        if ((modification.type === "fortifications" || modification.type === "tankTraps") && facing) {
    ...
    formatHexModificationCollectionLabel(modifications) {
        const fortifications = modifications.filter((modification) => modification.type === "fortifications");
        const tankTraps = modifications.filter((modification) => modification.type === "tankTraps");
        const others = modifications.filter((modification) => modification.type !== "fortifications" && modification.type !== "tankTraps");
    ...
    parseHexModificationAction(actionId) {
        switch (actionId) {
            case "fortifications":
            case "tankTraps":
            case "clearedPath":
     ...
     */
    resolveUnitLabelForHex(hexKey, unitId) {
        const unit = this.resolvePlayerUnitSnapshot(hexKey, unitId);
        return unit ? this.resolveUnitLabelForUnit(unit) : null;
    }
    resolveUnitLabelForUnit(unit) {
        const scenarioType = unit.type;
        const deploymentState = ensureDeploymentState();
    ...
    findPoolEntry(key, pool) {
        return pool.find((entry) => entry.key === key);
    }
    composeZoneCapacityMessage(hexKey, deploymentState) {
        const zoneKey = deploymentState.getZoneKeyForHex(hexKey);
        if (!zoneKey) {
            return "";
        }
    ...
    syncBaseCampAssignButton(phase, hasValidPlayerDeploymentHex) {
        if (!this.baseCampAssignButton) {
            return;
        }
    ...
    renderEngineUnits() {
        ...
        if (typeof renderer.renderHexModifications === "function" || typeof renderer.renderHexModification === "function") {
            const modificationsByHex = new Map();
        ...
        const renderStack = (hexKey, members) => {
        ...
        friendlyHexes.forEach((hex) => {
            const stackMembers = engine
                ...
                .filter((entry) => {
                const def = this.unitTypes[entry.unit.type];
                return def?.moveType !== "air";
            })
                .map((entry) => ({
                unit: entry.unit,
                faction: entry.faction === "Ally" ? "Ally" : "Player"
            }));
            if (stackMembers.length === 0) {
                return;
            }
        ...
        const enemyContacts = typeof engine.getEnemyContactSnapshot === "function"
        ...
        enemyContacts.forEach((contact) => {
            ...
            const def = this.unitTypes[renderUnit.type];
            if (def?.moveType === "air") {
                return;
            }
    ...
    buildEnemyContactRenderUnit(contact, liveUnits) {
        ...
        const scenarioType = (contact.unitType ?? liveUnit?.type ?? "Recon_Bike");
    ...
    normalizeContactStrengthEstimate(contact, liveUnit) {
    ...
    findEnemyContactAtHex(axial) {
        const engine = this.battleState.ensureGameEngine();
        const contacts = typeof engine.getEnemyContactSnapshot === "function"
            ? engine.getEnemyContactSnapshot()
            : [];
    ...
    formatScenarioUnitTypeLabel(unitType) {
        return unitType
            .replace(/_/g, " ")
     ...
     */
    buildUnitTypeDictionary() {
        return this.deepCloneValue(unitTypesSource);
    }
    /**
    ...
    normalizeTileInstance(entry) {
        return {
            ...
            features: entry.features?.map((feature) => feature)
        };
    }
    /**
...
BattleScreen.SOUND_ENABLED_STORAGE_KEY = "fsg-sound-enabled";
...
BattleScreen.PLAYER_MOVE_ANIMATION_MS = 350;
