...
export class CampaignScreen {
    ...
    `;
        ...
        const costEl = body.querySelector("#campaignRedeployCost");
        const cancelBtn = body.querySelector("#campaignRedeployCancel");
        const modeSelect = body.querySelector("#transportModeSelect");
        const calcAndRenderCost = () => {
            if (!modeSelect || !costEl)
                return;
            const selectedMode = modeSelect.value;
            const inputs = Array.from(body.querySelectorAll("[data-move-index]"));
            ...
            const mode = modes[selectedMode] ?? modes.foot;
            ...
            if (mode.capacity && transportCap) {
                ...
                const capType = mode.capacity.type;
                const available = (transportCap[capType] ?? 0) - (transportCap[`${capType}InTransit`] ?? 0);
      ...
      `;
        ...
        calcAndRenderCost();
        body.querySelectorAll("[data-move-index]").forEach((inp) => inp.addEventListener("input", () => calcAndRenderCost()));
        modeSelect?.addEventListener("change", () => calcAndRenderCost());
        if (!form || !cancelBtn || !modeSelect)
            return;
        form.onsubmit = (ev) => {
            ev.preventDefault();
            const inputs = Array.from(body.querySelectorAll("[data-move-index]"));
