...
import { ROSTER_FILE_NAME, generalRosterEntries, saveRosterToFile, loadRosterFromFile, getRosterCount, addGeneralToRoster, findGeneralById, removeGeneralFromRoster, ensureRosterInitialized } from "../../utils/rosterStorage";
import { REGION_OPTIONS, SCHOOL_OPTIONS, findRegionOption, findSchoolOption } from "../../data/commissioningOptions";
...
export class LandingScreen {
    attachPrecombatScreen(precombatScreen) {
        this.precombatScreen = precombatScreen;
    }
    attachCampaignScreen(campaignScreen) {
    ...
    constructor(screenManager, uiState) {
        ...
        this.missionHeadline = null;
        this.missionListSummary = null;
        this.generalAssignmentHeadline = null;
        this.generalAssignmentDetails = null;
        this.generalDetailPanel = null;
        this.generalDetailName = null;
        this.generalDetailSummary = null;
        this.generalDetailMissions = null;
        this.generalDetailVictories = null;
        this.generalDetailUnits = null;
        this.generalDetailCasualties = null;
        this.generalDetailBonuses = null;
        this.generalForm = null;
        this.generalRegionSelect = null;
        this.generalSchoolSelect = null;
        this.commissioningBenefits = null;
        this.commissioningBenefitsContent = null;
        this.clearGeneralSelectionButton = null;
        this.exportRosterButton = null;
        this.importRosterInput = null;
        this.enterPrecombatButton = null;
        this.feedback = null;
        this.generalRosterList = null;
        this.generalFormSection = null;
        this.commissionButton = null;
        this.difficultySelect = null;
        this.activeMissionButton = null;
        this.precombatScreen = null;
    ...
    populateCommissioningSelections() {
    ...
    populateSelect(select, options, isLocked) {
    ...
    buildUnlockCallout(title, sku, buttonLabel) {
        const wrapper = document.createElement("article");
        wrapper.className = "commissioning-benefit";
        const heading = document.createElement("p");
        heading.className = "commissioning-benefit__heading";
    ...
    cacheElements() {
        ...
        this.campaignButtons = this.campaignList
            ? Array.from(this.campaignList.querySelectorAll('[data-campaign-id]'))
            : [];
    }
    /**
    ...
    handleGeneralFormSubmit(event) {
        ...
        const name = formData.get("generalName")?.trim() ?? "";
        const regionKey = formData.get("regionKey")?.trim() ?? "";
        const schoolKey = formData.get("schoolKey")?.trim() ?? "";
        const commissionedAt = formData.get("commissionedAt")?.trim() ?? "";
        if (name.length === 0) {
     ...
     */
    async handleImportRoster(event) {
     ...
     */
    commissionGeneral(formData, region, school) {
    ...
    buildInitialStatBlock(region, school) {
        const base = {
            accBonus: 0,
            dmgBonus: 0,
            moveBonus: 0,
    ...
    applyStatAdjustments(target, adjustments) {
    ...
    renderRoster() {
        ...
        `;
            ...
            .join("");
        this.generalRosterList.querySelectorAll("[data-select-general]").forEach((button) => {
            button.addEventListener("click", () => {
                const generalId = button.dataset.selectGeneral ?? "";
                this.selectGeneral(generalId);
            });
        });
        this.generalRosterList.querySelectorAll("[data-view-general]").forEach((button) => {
            button.addEventListener("click", () => {
                const generalId = button.dataset.viewGeneral ?? "";
                this.viewGeneralProfile(generalId);
            });
        });
        this.generalRosterList.querySelectorAll("[data-retire-general]").forEach((button) => {
            button.addEventListener("click", () => {
     ...
     */
    syncCampaignButtonState(mission) {
    ...
    renderMissionList() {
        ...
        const canonicalOrder = ["training", "patrol", "patrol_river_watch", "assault", "assault_citadel_ridge"];
        ...
        this.missionButtons = Array.from(this.missionList.querySelectorAll('[data-mission]'));
     ...
     */
    updateGeneralDetailPanel(general, context) {
        if (!this.generalDetailPanel ||
     ...
     */
    formatStatAdjustments(adjustments) {
        const statLabels = {
            accBonus: "Accuracy",
            dmgBonus: "Damage",
            moveBonus: "Mobility",
        ...
        const parts = Object.keys(statLabels)
            .map((key) => {
