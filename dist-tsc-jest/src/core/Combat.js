 ...
 * document.
 */
export function pickFacingArmor(attackerHex, defenderHex, defenderFacing, defenderUnit, attackerClass) {
    if (combatBalance.penetration.topAttackClasses.has(attackerClass)) {
...
export function resolveAttack(request) {
    ...
    const facingArmor = pickFacingArmor(request.attackerCtx.hex, request.defenderCtx.hex, request.targetFacing, request.defender.unit, request.attacker.unit.class);
