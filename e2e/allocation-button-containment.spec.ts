...
test.describe("Allocation Button Containment", () => {
  ...
  async function verifyNoOverlappingItems(page: import("@playwright/test").Page, testName: string): Promise<void> {
    ...
    // Collect all visible item boxes
    for (let i = 0; i < items.length; i++) {
    ...
    for (let i = 0; i < boxes.length; i++) {
      for (let j = i + 1; j < boxes.length; j++) {
        ...
        // Allow items to be adjacent (gap of 0 is fine) but not truly overlapping
        const tolerance = 1; // 1px tolerance for subpixel rendering
        ...
        expect(
          trulyOverlapping,
          `${testName}: Allocation items ${itemA.index} and ${itemB.index} are overlapping (not just adjacent)`
        ).toBe(false);
      }
    }
  }
